...
SKIP_DIRS: Final[frozenset[str]] = frozenset({'.git', '__pycache__', '.mypy_cache', '.ruff_cache', '.pytest_cache', '.venv', 'venv', 'lazy', '.env', 'node_modules'})
...
class DocstringRemover(ast.NodeTransformer):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> ast.FunctionDef:
        ...
        if self._has_docstring(node) and (not self.remove_module_docstring or not self._is_module_level(node)) and (isinstance(node.body[0], ast.Expr) and isinstance(node.body[0].value, ast.Constant) and isinstance(node.body[0].value.value, str)):
    ...
    def visit_AsyncFunctionDef(self, node: ast.AsyncFunctionDef) -> ast.AsyncFunctionDef:
        ...
        if self._has_docstring(node) and isinstance(node.body[0], ast.Expr) and isinstance(node.body[0].value, ast.Constant) and isinstance(node.body[0].value.value, str):
            self.docstrings_removed += 1
            node.body = node.body[1:]
            if not node.body:
                node.body = [ast.Pass()]
    ...
    def visit_Module(self, node: ast.Module) -> ast.Module:
        ...
        if self.remove_module_docstring and self._has_docstring(node) and (isinstance(node.body[0], ast.Expr) and isinstance(node.body[0].value, ast.Constant) and isinstance(node.body[0].value.value, str)):
            self.docstrings_removed += 1
            node.body = node.body[1:]
        self.generic_visit(node)
        return node
    ...
    def _has_docstring(node: ast.AST) -> bool:
        ...
        return bool(getattr(node, 'body', [])) and isinstance(node.body[0], ast.Expr) and isinstance(node.body[0].value, ast.Constant) and isinstance(node.body[0].value.value, str)
...
def _remove_docstrings_from_source(source: str, remove_module_docstring: bool) -> tuple[str, int]:
    ...
    def _collect_docstring_lines(node: ast.AST) -> None:
        """_collect_docstring_lines –  collect docstring lines.
...
def process_single_file(path: Path, remove_module_docstring: bool=False, dry_run: bool=False) -> FileResult:
    ...
    try:
        ...
        try:
            ...
            return FileResult(path=str(path), is_error=True, error_message=f'Validation error: {e}')
        ...
        return FileResult(path=str(path), comments_removed=comment_remover.comments_removed, docstrings_removed=docstrings_removed)
    except Exception as e:
        return FileResult(path=str(path), is_error=True, error_message=str(e))

def process_wheel_file(wheel_path: Path, remove_module_docstring: bool=False, dry_run: bool=False) -> list[FileResult]:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            results.append(FileResult(path=wheel_name, is_error=True, error_message=f'Wheel processing error: {e}'))
    return results
    ...
    for result in results:
        if result.is_error:
            ...
            if result.docstrings_removed > 0:
                changes.append(f"{result.docstrings_removed} docstring{('s' if result.docstrings_removed != 1 else '')}")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description='Strip comments and docstrings from Python source files', formatter_class=argparse.RawDescriptionHelpFormatter, epilog='\nExamples:\n\n  python strip_comments.py\n\n\n  python strip_comments.py src/main.py\n\n\n  python strip_comments.py --remove-module-docstring\n\n\n  python strip_comments.py --dry-run\n        ')
    parser.add_argument('path', nargs='?', default='.', help='File or directory to process (default: current directory)')
    parser.add_argument('--remove-module-docstring', action='store_true', help='Also strip module-level docstrings (preserved by default)')
    ...
    for wheel_file in wheel_files:
        results = process_wheel_file(wheel_file, remove_module_docstring=args.remove_module_docstring, dry_run=args.dry_run)
        for result in results:
            _accumulate_result(stats, result)
    if python_files:
        tasks: list[tuple[Path, bool, bool]] = [(path, args.remove_module_docstring, args.dry_run) for path in python_files]
