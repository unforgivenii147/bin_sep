...
"""noroot.py – Noroot utilities.

This module provides functionality for noroot."""
from __future__ import annotations
import re
import sys
from pathlib import Path
from dh import get_nobinary
IF_BLOCK_REGEX = re.compile('^if\\s+\\[\\s*\\$\\((\\S+)\\)\\s*\\{\\-ne\\s+0\\s*\\}\\]\\s*;\\s*then\\s*\\n((?:.|\\n)*?)^\\s*exit\\s+1\\s*$(.*?)^\\s*fi', re.MULTILINE | re.IGNORECASE)

def remove_conditional_exit_blocks(path: Path) -> None:
    """remove_conditional_exit_blocks – remove conditional exit blocks.
    ...
    for item_path in files_to_process:
        if item_path.is_file():
            try:
                ...
                if content.startswith(('#!/bin/bash', '#!/usr/bin/env bash')) or oct(item_path.stat().st_mode)[-3:] not in ('000', '001', '010', '011', '002', '012', '100', '110', '111', '101'):
                    is_likely_bash = True
                if is_likely_bash:
