...
"""checkout_previous.py – Checkout Previous utilities.

This module provides functionality for checkout previous."""
...
Returns:
    ...
    result = subprocess.run(['git', 'rev-parse', '--is-inside-work-tree'], cwd=path, text=True, capture_output=True, check=False)
...
Args:
    ...
    try:
        ...
        if not latest_message.startswith(TARGET_PREFIX):
            print(f'[skip] {repo_name}: latest commit does not start with {TARGET_PREFIX}')
            return
        ...
        answer = input(f'Checkout {short_parent} in {repo_name}? [y/N]: ').strip().lower()
...
def main() -> None:
    ...
    candidates.extend((path for path in current_dir.iterdir() if path.is_dir() and (not path.is_symlink())))
