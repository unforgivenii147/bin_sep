...
"""exmd.py – Exmd utilities.

This module provides functionality for exmd."""
...
def extract_code_snippets_with_details(markdown_content: str) -> Any:
    """extract_code_snippets_with_details – extract code snippets with details.

Args:
    markdown_content: Description of markdown_content."""
    snippets_data = []
    ...
    for i, line in enumerate(lines):
        if line.strip().startswith('```'):
            if in_code_block:
                snippets_data.append({'language': language, 'start_line': start_line_num, 'end_line': i, 'content': '\n'.join(current_block_lines)})
                in_code_block = False
                current_block_lines = []
                language = ''
            else:
    ...
    if in_code_block:
        snippets_data.append({'language': language, 'start_line': start_line_num, 'end_line': len(lines), 'content': '\n'.join(current_block_lines)})
    return snippets_data

def get_extension_from_language(language: str) -> str:
    """get_extension_from_language – get extension from language.
...
Returns:
    ...
    extensions = {'sh': '.sh', 'bash': '.sh', 'zsh': '.sh', 'python': '.py', 'py': '.py', 'javascript': '.js', 'js': '.js', 'html': '.html', 'css': '.css', 'json': '.json', 'yaml': '.yaml', 'yml': '.yaml', 'sql': '.sql', 'md': '.md', 'text': '.txt', 'plain': '.txt', '': '.txt'}
    return extensions.get(language.lower(), '.txt')

def process_markdown_files(directory: str='.') -> None:
    """process_markdown_files – process markdown files.
