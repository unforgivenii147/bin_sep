...
"""chm2pdf_reportlab.py – Chm2Pdf Reportlab utilities.

This module provides functionality for chm2pdf reportlab."""
...
class CHMToPDF:
    ...
    def __init__(self, chm_file: Path | str, output_file: Path | str) -> None:
        """__init__ –   init  .
...
Args:
    ...
    def extract_html_content(self, path: Path | str) -> Any:
...
Args:
    ...
    def clean_html(self, html_content: str) -> Any:
...
Args:
            ...
            for path in ['#SYSTEM', '#TOPICS', '#STRINGS', '#URLTBL']:
                try:
                    ...
                    if data:
                        ...
                        entries = re.findall('[\\w\\s\\-\\.]+\\.html?', text, re.IGNORECASE)
                        if entries:
    ...
    def parse_toc(self, toc_obj: Any) -> Any:
        """parse_toc – parse toc.

Args:
    toc_obj: Description of toc_obj."""
        entries = []
    ...
    def convert_to_pdf(self) -> bool:
        ...
        try:
            doc = SimpleDocTemplate(str(self.output_file), pagesize=letter, rightMargin=72, leftMargin=72, topMargin=72, bottomMargin=72)
            styles = getSampleStyleSheet()
            story = []
            title_style = ParagraphStyle('CustomTitle', parent=styles['Heading1'], fontSize=24, textColor='darkblue', alignment=TA_CENTER, spaceAfter=30)
                        ...
                        for para in paragraphs:
                            if para.strip():
                                ...
                                try:
                                    ...
                                    story.append(Paragraph(para_text[:1000], styles['Normal']))
                        page_count += 1
