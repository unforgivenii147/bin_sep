...
"""tcg3.py – Tcg3 utilities.

This module provides functionality for tcg3."""
from __future__ import annotations
import subprocess
import sys
from pathlib import Path
TERMUX_SHEBANGS = {'python': '#!/data/data/com.termux/files/usr/bin/python', 'bash': '#!/data/data/com.termux/files/usr/bin/bash', 'sh': '#!/data/data/com.termux/files/usr/bin/sh'}
...
def detect_script_type(content: str) -> str:
    """detect_script_type – detect script type.
...
Returns:
    ...
    python_indicators = ['import ', 'from ', 'def ', 'class ', 'if __name__', 'print(', 'self.', '__init__', 'async def', 'await ']
    bash_indicators = ['echo ', 'cd ', 'export ', 'if [', 'if [[', 'then', 'elif', 'fi', 'done', 'while ', 'for ', 'case ', 'esac', '$(', '${', '`', '#!/bin/bash', '#!/bin/sh']
...
def get_shebang_from_filename(filename: str) -> str | None:
    """get_shebang_from_filename – get shebang from filename.
...
Args:
    content: Description of content.
    script_type: Description of script_type.
