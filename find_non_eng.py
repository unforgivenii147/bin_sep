...
"""find_non_eng.py – Find Non Eng utilities.

This module provides functionality for find non eng."""
...
Args:
    min_bytes: Description of min_bytes.
    max_bytes: Description of max_bytes."""
        self.min_bytes = min_bytes
        self.max_bytes = max_bytes
        self.stats = {'total_files': 0, 'skipped_binary': 0, 'skipped_small': 0, 'skipped_error': 0, 'non_english': [], 'languages': Counter()}
...
Returns:
    ...
    def detect_language(self, path: Path) -> Any:
...
Args:
    ...
    def scan_directory(self, directory: Path | str, show_progress: bool=True, only_report_non_english: bool=True) -> None:
        """scan_directory – scan directory.

Args:
    directory: Description of directory.
    show_progress: Description of show_progress.
    only_report_non_english: Description of only_report_non_english."""
        ...
        for root, dirs, files in os.walk(directory):
            ...
            for file in files:
                ...
                if show_progress:
                    print(f"\n{path} [Files: {self.stats['total_files']}]", end='', flush=True)
                ...
                if (lang_code != 'en' or not only_report_non_english) and (lang_code == 'en' and (not is_reliable) and only_report_non_english or lang_code != 'en'):
                    self.stats['non_english'].append({'file': path, 'language': lang_name, 'code': lang_code, 'reliable': is_reliable, 'confidence': percent})
        print('\n' + '=' * 40)
...
def main() -> None:
    ...
    parser.add_argument('--min-bytes', type=int, default=100, help='Minimum bytes to read for language detection (default: 100)')
    parser.add_argument('--max-bytes', type=int, default=10000, help='Maximum bytes to read from each file (default: 10000)')
