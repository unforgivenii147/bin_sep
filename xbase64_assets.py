...
BASE64_SIGNATURES: dict[str, tuple[bytes, str, str]] = {'image/png': (b'\x89PNG\r\n\x1a\n', '.png', 'images'), 'image/jpeg': (b'\xff\xd8\xff', '.jpg', 'images'), 'image/gif': (b'GIF87a', '.gif', 'images'), 'image/gif': (b'GIF89a', '.gif', 'images'), 'image/webp': (b'RIFF', '.webp', 'images'), 'image/svg+xml': (b'<?xml', '.svg', 'images'), 'image/svg+xml': (b'<svg', '.svg', 'images'), 'font/woff': (b'wOF2', '.woff2', 'fonts'), 'font/woff': (b'wOF2', '.woff2', 'fonts'), 'font/ttf': (b'\x00\x01\x00\x00', '.ttf', 'fonts'), 'application/x-font-ttf': (b'\x00\x01\x00\x00', '.ttf', 'fonts'), 'application/json': (b'{', '.json', 'data'), 'text/plain': (b'text', '.txt', 'data')}
...
def detect_base64_mime_type(data: bytes) -> tuple[str | None, str | None, str | None]:
    """
...
class Base64PatternDetector:
    ...
    PATTERNS: dict[str, re.Pattern] = {'url': re.compile(f"""\n            url\\s*\\(\\s*["']?\n            data:\n            (?P<mime>[^;,)\\s]+)\n            (?:;[^,)]*)?\n            ;base64\\s*,\\s*\n            (?P<data>{BASE64})\n            ["']?\\s*\\)\n            """, re.IGNORECASE | re.VERBOSE), 'src': re.compile(f"""\n            \\bsrc\\s*=\\s*["']\n            data:\n            (?P<mime>[^;,]+)\n            (?:;[^,]*)?\n            ;base64\\s*,\\s*\n            (?P<data>{BASE64})\n            ["']\n            """, re.IGNORECASE | re.VERBOSE), 'href': re.compile(f"""\n            \\bhref\\s*=\\s*["']\n            data:\n            (?P<mime>[^;,]+)\n            (?:;[^,]*)?\n            ;base64\\s*,\\s*\n            (?P<data>{BASE64})\n            ["']\n            """, re.IGNORECASE | re.VERBOSE), 'data_uri': re.compile(f'\n            data:\n            (?P<mime>[^;,]+)\n            (?:;[^,]*)?\n            ;base64\\s*,\\s*\n            (?P<data>{BASE64})\n            ', re.IGNORECASE | re.VERBOSE), 'background': re.compile(f"""\n            \\bbackground(?:-image)?\\s*:\\s*\n            url\\s*\\(\\s*["']?\n            data:\n            (?P<mime>[^;,)\\s]+)\n            (?:;[^,)]*)?\n            ;base64\\s*,\\s*\n            (?P<data>{BASE64})\n            ["']?\\s*\\)\n            """, re.IGNORECASE | re.VERBOSE)}
        ...
        for pattern_name, pattern in Base64PatternDetector.PATTERNS.items():
            for match in pattern.finditer(text):
                ...
                if len(base64_str) < 64:
                    continue
                matches.append(Base64Match(path=path, start_pos=match.start(), end_pos=match.end(), base64_str=base64_str, context=match.group(0), match_type=pattern_name))
        return matches
...
Args:
    assets_dir: Description of assets_dir."""
    ...
    def extract_asset(self, match: Base64Match) -> ExtractedAsset | None:
        ...
        try:
            ...
            if asset_path.exists():
                ...
                return ExtractedAsset(original_file=match.path, asset_path=asset_path, asset_url=asset_url, base64_match=match, extracted_bytes=decoded_bytes)
...
Args:
    asset_extractor: Description of asset_extractor."""
    ...
    def process_file(self, path: Path) -> ProcessingResult:
        ...
        try:
            if not path.exists():
                return ProcessingResult(path=path, success=False, extracted_count=0, replaced_count=0, error=f'File not found: {path}', duration=(datetime.now() - start_time).total_seconds())
            if path.stat().st_size > MAX_FILE_SIZE:
                return ProcessingResult(path=path, success=False, extracted_count=0, replaced_count=0, error=f'File too large: {path.stat().st_size} bytes', duration=(datetime.now() - start_time).total_seconds())
            try:
                with open(path, 'r', encoding='utf-8', errors='ignore') as f:
                    content = f.read()
            except Exception as e:
                return ProcessingResult(path=path, success=False, extracted_count=0, replaced_count=0, error=f'Failed to read file: {e}', duration=(datetime.now() - start_time).total_seconds())
            matches = Base64PatternDetector.find_all_base64(content, path)
            if not matches:
                logger.debug(f'No base64 found in {path}')
                return ProcessingResult(path=path, success=True, extracted_count=0, replaced_count=0, duration=(datetime.now() - start_time).total_seconds())
            ...
            print(f'Processed {path.name}: extracted={extracted_count}, replaced={len(replacements)}')
            return ProcessingResult(path=path, success=True, extracted_count=extracted_count, replaced_count=len(replacements), duration=(datetime.now() - start_time).total_seconds())
        except Exception as e:
            logger.error(f'Unexpected error processing {path}: {e}')
            return ProcessingResult(path=path, success=False, extracted_count=0, replaced_count=0, error=str(e), duration=(datetime.now() - start_time).total_seconds())
...
class FileDiscovery:
    ...
    SKIP_DIRS = {'.git', '.svn', '__pycache__', 'node_modules', '.venv', 'venv', '.env', '.egg-info', 'dist', 'build', '.idea', '.vscode', '.pytest_cache', '.tox', '.coverage', '.mypy_cache', 'target', 'out', 'bin', '.gradle', 'assets'}
...
Args:
    ...
    def _print_summary(self) -> None:
        ...
        if self.results:
            print('\nDetailed results:')
            for result in sorted(self.results, key=lambda r: r.extracted_count, reverse=True):
                if result.extracted_count > 0 or result.replaced_count > 0:
                    print(f'  {result.path.name:40} extracted={result.extracted_count:3} replaced={result.replaced_count:3} time={result.duration:.3f}s')
            ...
            if asset_count > 0:
                ...
                for category in ['images', 'fonts', 'videos', 'data']:
                    ...
                    if cat_dir.exists():
                        ...
                        if count > 0:
                            size = sum((_.stat().st_size for _ in cat_dir.iterdir() if _.is_file()))
                            print(f'  {category:8}: {count:4} files ({size / 1024:.1f} KB)')
...
def main() -> None:
    ...
    logger.add('extract_base64_assets.log', format='{time:YYYY-MM-DD HH:mm:ss} | {level: <8} | {name}:{function}:{line} - {message}', level='DEBUG', rotation='10 MB', retention='1 week')
    if len(sys.argv) > 1:
        paths = sys.argv[1:]
    else:
        paths = ['.']
