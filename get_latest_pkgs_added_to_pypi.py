...
"""get_latest_pkgs_added_to_pypi.py – Get Latest Pkgs Added To Pypi utilities.

This module provides functionality for get latest pkgs added to pypi."""
...
def fetch_pypi_updates() -> list[dict[str, str]]:
    """fetch_pypi_updates – fetch pypi updates.
...
Returns:
    ...
    try:
        ...
        for item in root.findall('.//item'):
            package_info = {'title': item.find('title').text if item.find('title') is not None else '', 'link': item.find('link').text if item.find('link') is not None else '', 'description': item.find('description').text if item.find('description') is not None else '', 'pub_date': item.find('pubDate').text if item.find('pubDate') is not None else '', 'guid': item.find('guid').text if item.find('guid') is not None else ''}
...
def main() -> None:
    ...
    parser.add_argument('-o', '--output', default='pypi_updates.json', help='Output filename (default: pypi_updates.json)')
    parser.add_argument('-f', '--format', choices=['json', 'csv', 'txt'], default='json', help='Output format (default: json)')
    parser.add_argument('-n', '--num-packages', type=int, default=None, help='Number of latest packages to save (default: all)')
