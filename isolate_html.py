...
"""isolate_html.py – Isolate Html utilities.

This module provides functionality for isolate html."""
...
class HTMLStandaloneMaker:
    """HTMLStandaloneMaker – HTMLStandaloneMaker."""
    MIME_MAP = {'.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.gif': 'image/gif', '.svg': 'image/svg+xml', '.webp': 'image/webp', '.ico': 'image/x-icon', '.woff': 'font/woff', '.woff2': 'font/woff2', '.ttf': 'font/ttf', '.otf': 'font/otf', '.eot': 'application/vnd.ms-fontobject', '.js': 'application/javascript', '.css': 'text/css', '.json': 'application/json', '.xml': 'application/xml', '.txt': 'text/plain'}
...
Returns:
    ...
    def encode_local_file_to_base64(self, path: Path) -> str | None:
        """encode_local_file_to_base64 – encode local file to base64.
...
Returns:
    ...
    def find_local_resource(self, resource_name: str | AttributeValueList, base_dir: Path) -> Path | None:
        """find_local_resource – find local resource.
...
Returns:
    ...
    def process_css_content(self, css_content: str, css_dir: Path) -> str:
        """process_css_content – process css content.

Args:
    css_content: Description of css_content.
    css_dir: Description of css_dir.
...
Returns:
        ...
        for font_url in font_url_matches:
            ...
            if local_font_path:
                ...
                if encoded_font:
                    ...
                    css_content = re.sub(escaped_url, f"url('data:{mime_type};base64,{encoded_font}')", css_content, flags=re.IGNORECASE)
    ...
    def process_image_tag(self, img_tag: Any, base_dir: Path) -> None:
        """process_image_tag – process image tag.

Args:
    img_tag: Description of img_tag.
    base_dir: Description of base_dir."""
        src = img_tag.get('src')
    ...
    def process_link_tag(self, link_tag: Any, base_dir: Path) -> None:
        """process_link_tag – process link tag.

Args:
    link_tag: Description of link_tag.
    base_dir: Description of base_dir."""
    ...
    def process_script_tag(self, script_tag: Any, base_dir: Path) -> None:
        """process_script_tag – process script tag.

Args:
    script_tag: Description of script_tag.
    base_dir: Description of base_dir."""
    ...
    def process_inline_styles(self, style_tag: Any, base_dir: Path) -> None:
        """process_inline_styles – process inline styles.

Args:
    style_tag: Description of style_tag.
    base_dir: Description of base_dir."""
    ...
    def make_html_standalone(self, html_path: Path) -> str | None:
        """make_html_standalone – make html standalone.
...
Returns:
    ...
    def save_standalone(self, html_path: Path, output_path: Path | None=None) -> bool:
        """save_standalone – save standalone.
