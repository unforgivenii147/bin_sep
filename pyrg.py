...
"""pyrg.py – Pyrg utilities.

This module provides functionality for pyrg."""
...
def get_files(paths: list[str], include_globs: list[str], exclude_globs: list[str], search_hidden: bool, max_size: int) -> Generator[Path, None, None]:
    """get_files – get files.

Args:
    paths: Description of paths.
    include_globs: Description of include_globs.
    exclude_globs: Description of exclude_globs.
    search_hidden: Description of search_hidden.
...
def matches_any_glob(path: Path, patterns: list[str]) -> bool:
    """matches_any_glob – matches any glob.
...
def search_file_text_mode(path: Path, cwd: Path, regex: re.Pattern | None, fixed: str, ignore_case: bool) -> tuple[str, list[tuple[int, str, list[tuple[int, int]]]]]:
    """search_file_text_mode – search file text mode.
...
Args:
    ...
    ignore_case: Description of ignore_case.

Returns:
    tuple[str, list[tuple[int, str, list[tuple[int, int]]]]]: Description of return value."""
    matches = []
    try:
...
def worker(args_tuple: Any) -> Any:
    """worker – worker.

Args:
    args_tuple: Description of args_tuple."""
    ...
    if is_binary(path):
        return (str(path), [])
    return search_file_text_mode(path=path, cwd=cwd, regex=compiled_regex, fixed=fixed, ignore_case=ignore_case)

def build_argparser() -> argparse.ArgumentParser:
    """build_argparser – build argparser.
...
Returns:
    ...
    p.add_argument('-w', '--workers', type=int, default=DEFAULT_WORKERS, help='Number of worker processes')
...
Returns:
    ...
    if not args.fixed_strings:
        ...
        try:
            ...
            return 2
    candidates = list(get_files(paths=args.paths, include_globs=args.glob or [], exclude_globs=args.exclude or [], search_hidden=args.hidden, max_size=args.max_filesize))
    color = not args.no_color and sys.stdout.isatty()
    any_match = False
    worker_args = [(path, cwd, pattern if not args.fixed_strings else None, pattern if args.fixed_strings else '', args.ignore_case) for path in candidates]
