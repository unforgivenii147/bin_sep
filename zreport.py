...
"""zreport.py – Zreport utilities.

This module provides functionality for zreport."""
...
def _stream_size(readable: Any, chunk: Any=1 << 20) -> int:
    """_stream_size –  stream size.

Args:
    readable: Description of readable.
...
def _tar_size(fileobj: Path | str, mode: str='r:*') -> int | None:
    """_tar_size –  tar size.

Args:
    fileobj: Description of fileobj.
...
Returns:
    ...
    try:
        ...
        return (None, str(e))
HANDLERS: Final[dict[str, tuple[str, Callable]]] = {'.zst': ('zstd', get_zst_size), '.xz': ('xz', get_xz_size), '.gz': ('gzip', get_gz_size), '.bz2': ('bzip2', get_bz2_size), '.7z': ('7zip', get_7z_size), '.zip': ('zip', get_zip_size), '.whl': ('wheel', get_zip_size), '.tar': ('tar', lambda p: (_tar_size(p.open('rb'), 'r:'), None))}
...
def main() -> None:
    ...
    for item in sorted(root.rglob('*')):
        ...
        print(col_fmt.format(name_trunc, label, fsz(comp_size), fsz(uncomp_size) if uncomp_size else 'Error', ratio_str))
    print('-' * 40)
