...
"""pip_get.py – Pip Get utilities.

This module provides functionality for pip get."""
...
from rich.progress import BarColumn, DownloadColumn, Progress, TextColumn, TimeElapsedColumn, TimeRemainingColumn, TransferSpeedColumn
console = Console()
MIRRORS = ['https://pypi.org/pypi', 'https://pypi.tuna.tsinghua.edu.cn/pypi', 'https://mirror-pypi.runflare.com/pypi']
RETRY_COUNT = 3
TIMEOUT = 30
...
def verify_file_hash(dest_path: Path, expected_digests: dict) -> bool:
    ...
    if 'sha256' in expected_digests:
        ...
        console.print('[yellow]No known hash provided in metadata. Skipping hash verification.[/yellow]')
        return True
    ...
    if calculated_hash == expected_hash.lower():
        console.print(f'[bold green]✓ Integrity check passed ({algo.upper()})[/bold green]')
        return True
    else:
        console.print(f'[bold red]✗ Hash verification failed! Expected: {expected_hash}, Got: {calculated_hash}[/bold red]')
        return False

def build_progress_bar() -> Progress:
    """Build a rich progress bar display."""
    return Progress(TextColumn('[bold blue]{task.fields[filename]}', justify='right'), BarColumn(bar_width=None), '[progress.percentage]{task.percentage:>3.1f}%', '•', DownloadColumn(), '•', TransferSpeedColumn(), '•', TextColumn('[cyan]Elapsed:[/cyan]'), TimeElapsedColumn(), '•', TextColumn('[cyan]ETA:[/cyan]'), TimeRemainingColumn(), console=console)

def download_with_pycurl(url: str, dest_path: Path, file_size: int) -> Any:
    ...
    with build_progress_bar() as progress:
        task_id = progress.add_task('download', filename=dest_path.name, total=file_size, completed=resume_byte)

        def write_callback(data: str) -> Any:
...
def download_with_requests(url: str, dest_path: Path, file_size: int, use_chunks: bool) -> None:
...
def download_with_aria2c(url: str, dest_path: Path) -> None:
    """Download using aria2c via subprocess (native support for resume & progress bar)."""
    cmd = ['aria2c', '--continue=true', f'--max-tries={RETRY_COUNT}', f'--timeout={TIMEOUT}', f'--dir={dest_path.parent.resolve()}', f'--out={dest_path.name}', url]
    subprocess.run(cmd, check=True)

def download_file(url: str, dest_path: Path, file_size: int, backend: str) -> None:
    """Route download requests to the designated backend tool."""
    is_chunked = file_size > CHUNK_THRESHOLD
    size_mb = file_size / (1024 * 1024)
    console.print(f'File size: [cyan]{size_mb:.2f} MB[/cyan] | Chunked mode: [cyan]{is_chunked}[/cyan]')
    if dest_path.exists() and dest_path.stat().st_size == file_size:
        console.print('[yellow]Local file matches full size. Skipping download...[/yellow]')
        return
    if backend == 'pycurl':
...
def main() -> None:
    ...
    if backend not in ('pycurl', 'requests', 'aria2c'):
        console.print(f"[red]Error: Unknown backend '{backend}'. Choose from: pycurl, requests, aria2c.[/red]")
        sys.exit(1)
    output_dir = Path.cwd()
    for pkg_spec in packages:
        ...
        try:
            ...
            if not verify_file_hash(target_path, digests):
                console.print('[bold red]Deleting corrupted/incomplete file...[/bold red]')
