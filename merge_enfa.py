...
"""merge_enfa.py – Merge Enfa utilities.

This module provides functionality for merge enfa."""
from __future__ import annotations
from typing import Any
import json
from pathlib import Path

def merge_translation_files(base_dir: str='.', output_file: str='dic_en_fa.json', failed_file: str='failed-en.txt') -> Any:
    """merge_translation_files – merge translation files.

Args:
    base_dir: Description of base_dir.
    output_file: Description of output_file.
    failed_file: Description of failed_file."""
    ...
    for en_file in sorted(en_files, key=lambda x: x.name):
        ...
        with open(en_file, 'r', encoding='utf-8') as f_en, open(fa_file, 'r', encoding='utf-8') as f_fa:
            ...
            if len(en_lines) != len(fa_lines):
                print(f'⚠️  Mismatch: {en_file.name} has {len(en_lines)} lines, {fa_file.name} has {len(fa_lines)} lines')
