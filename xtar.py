...
"""xtar.py – Xtar utilities.

This module provides functionality for xtar."""
...
def get_compression_type(path: Path | str) -> Any:
    """get_compression_type – get compression type.
...
def check_integrity(archive_path: Path | str) -> bool:
...
Args:
    ...
    try:
        ...
        with tarfile.open(archive_path, f'r:{compression}') as tar:
            tar.getmembers()
        return True
    except (tarfile.TarError, lzma.LZMAError, gzip.BadGzipFile, OSError, subprocess.SubprocessError) as e:
        print(f'Integrity check failed for {archive_path.name}: {e}')
        return False

def extract_archive(archive_path: Path | str) -> Any:
...
def find_archives(directory: str='.') -> Any:
