...
"""pdmp.py – Pdmp utilities.

This module provides functionality for pdmp."""
...
def delete_empty_dirs_iterative(root: Path, dry_run: bool=False, verbose: bool=False) -> tuple[int, list[Path]]:
    """delete_empty_dirs_iterative – delete empty dirs iterative.
...
Returns:
    tuple[int, list[Path]]: Description of return value."""
    removed_count: int = 0
...
def main() -> None:
    ...
    parser.add_argument('path', nargs='?', type=Path, default=Path.cwd(), help='The root directory to start scanning from (default: current working directory).')
    parser.add_argument('--dry-run', action='store_true', help='Perform a dry run: show what would be deleted without actually deleting.')
    parser.add_argument('-v', '--verbose', action='store_true', help='Enable verbose output, showing skipped and found directories.')
    ...
    removed_count, removed_dirs_list = delete_empty_dirs_iterative(root_path, dry_run=args.dry_run, verbose=args.verbose)
    if removed_count > 0:
        if args.dry_run:
