...
"""mlic.py – Mlic utilities.

This module provides functionality for mlic."""
...
def find_multiline_strings(path: Path, min_lines: int=2, min_chars: int=10) -> dict[str, list[tuple[int, int]]]:
    """find_multiline_strings – find multiline strings.

Args:
    path: Description of path.
    min_lines: Description of min_lines.
    min_chars: Description of min_chars.

Returns:
    dict[str, list[tuple[int, int]]]: Description of return value."""
    try:
...
Returns:
    ...
    if extensions is None:
        extensions = {'.txt', '.md', '.py', '.js', '.html', '.css', '.xml', '.json', '.yaml', '.yml', '.csv', '.log', '.ini', '.cfg', '.conf', '.sh', '.bat', '.ps1', '.java', '.cpp', '.c', '.h', '.rb', '.php', '.sql'}
    files = []
    try:
...
Returns:
    tuple[Path, dict[str, list[tuple[int, int]]]]: Description of return value."""
    path, min_lines, min_chars = args
    strings = find_multiline_strings(path, min_lines, min_chars)
    return (path, strings)

def find_repeated_strings(directory: Path, min_lines: int=2, min_chars: int=10, max_workers: int | None=None, half: bool=False) -> dict[str, list[tuple[Path, list[tuple[int, int]]]]]:
    """find_repeated_strings – find repeated strings.
...
Args:
    ...
    half: Description of half.

Returns:
    dict[str, list[tuple[Path, list[tuple[int, int]]]]]: Description of return value."""
...
def remove_strings_from_files(repeated_strings: dict[str, list[tuple[Path, list[tuple[int, int]]]]], string_numbers: list[int] | None=None, validate: bool=True) -> Any:
    """remove_strings_from_files – remove strings from files.

Args:
    repeated_strings: Description of repeated_strings.
    string_numbers: Description of string_numbers.
    validate: Description of validate."""
    ...
    for path, positions in files_to_modify.items():
        try:
            ...
            if path.suffix.lower() == '.py' and validate:
                ...
                if not is_valid:
                    print(f'SKIPPED: {path} - Syntax validation failed: {error_msg}', file=sys.stderr)
...
def save_strings_to_file(repeated_strings: dict[str, list[tuple[Path, list[tuple[int, int]]]]], output_file: Path) -> None:
    """save_strings_to_file – save strings to file.
...
def main() -> None:
    ...
    parser.add_argument('-r', '--remove', nargs='*', type=int, help='Remove found repeated strings. Optionally specify string numbers (e.g., -r 2 4 to remove strings #2 and #4)')
    parser.add_argument('-H', '--half', action='store_true', help='Only show strings found in at least 50 percent of files')
    parser.add_argument('--no-validate', action='store_true', help='Skip Python syntax validation (use with caution)')
    parser.add_argument('--min-lines', type=int, default=MIN_LINES, help=f'Minimum number of lines for a multiline string (default: {MIN_LINES})')
    parser.add_argument('--min-chars', type=int, default=MIN_CHARS, help=f'Minimum number of characters for a multiline string (default: {MIN_CHARS})')
    ...
    if args.extensions:
        ...
        def find_files_with_ext(directory: Path | str, _: Any | None=None) -> list[Path]:
            """find_files_with_ext – find files with ext.
...
Args:
    ...
    repeated = find_repeated_strings(directory, min_lines=args.min_lines, min_chars=args.min_chars, max_workers=args.workers, half=args.half)
