...
def clean() -> None:
    ...
    proc = subprocess.run([AGE_BIN, '-r', pubkey, '-a'], input=sys.stdin.buffer.read(), capture_output=True)
...
def smudge() -> None:
    ...
    if not os.path.exists(AGE_KEY_FILE):
        ...
        sys.stdout.buffer.write(data)
        return
    proc = subprocess.run([AGE_BIN, '-d', '-i', AGE_KEY_FILE], input=data, capture_output=True)
    if proc.returncode != 0:
