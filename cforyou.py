...
"""cforyou.py – Cforyou utilities.

This module provides functionality for cforyou."""
...
Returns:
    ...
    try:
        ...
        return None
    wheel_pattern = re.compile(f'{re.escape(pkg_name)}-([0-9][A-Za-z0-9\\.\\-_]*)\\.(?:whl|tar\\.gz|zip)', re.IGNORECASE)
    versions = []
...
def load_previous_results() -> dict[str, dict]:
    """load_previous_results – load previous results.

Returns:
    dict[str, dict]: Description of return value."""
...
if __name__ == '__main__':
    ...
    for i, (pkg_name, installed_version) in enumerate(packages_to_check):
        ...
        current_results[pkg_name] = {'installed_version': installed_version, 'latest_version': latest_version_str, 'checked_at': time.strftime('%Y-%m-%d %H:%M:%S')}
        if latest_version_str:
            try:
                ...
                if installed_ver < latest_ver:
                    ...
                    cprint(f'[{i + 1}/{len(packages_to_check)}] {pkg_name}: {installed_version} -> {latest_version_str} (Updatable!)', 'green')
                else:
                    cprint(f'[{i + 1}/{len(packages_to_check)}] {pkg_name}: {installed_version} (Latest: {latest_version_str})', 'white')
            except Exception as ver_err:
                cprint(f"[{i + 1}/{len(packages_to_check)}] {pkg_name}: Could not parse versions '{installed_version}' or '{latest_version_str}': {ver_err}", 'yellow')
        else:
    ...
    if updatable_pkgs_info:
        ...
        cprint(f"\nTo update these packages, you can use: pip install --upgrade {' '.join([p[0] for p in updatable_pkgs_info])}", 'yellow')
    else:
