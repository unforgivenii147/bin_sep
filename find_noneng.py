...
"""find_noneng.py – Find Noneng utilities.

This module provides functionality for find noneng."""
...
from cld import detect_language, is_probably_text_bytes, read_file_bytes, safe_text_from_bytes
DEFAULT_MAX_PROBE = 4096
DEFAULT_READ_BYTES = 2 * 1024 * 1024

def find_files(root: str='.', recursive: bool=True, exts: Iterable[str] | None=None, skip_hidden: bool=True) -> Iterable[str]:
...
Returns:
    Iterable[str]: Description of return value."""
...
Args:
    ...
    max_probe: Description of max_probe.
...
def scan_file_lines(path: str, min_confidence: float=0.6) -> list[tuple[str, int, str, float, str]]:
    """scan_file_lines – scan file lines.

Args:
    path: Description of path.
    min_confidence: Description of min_confidence.

Returns:
    list[tuple[str, int, str, float, str]]: Description of return value."""
    results = []
    try:
...
Returns:
    ...
    parser.add_argument('--ext', '-e', default='', help='Comma-separated extensions to include (e.g. py,md,txt). Default: all text files.')
    parser.add_argument('--min', '-m', type=float, default=0.6, help='Minimum confidence to report (0..1). Default: 0.6')
    ...
    parser.add_argument('--skip-hidden', action='store_true', default=True, help='Skip hidden files and directories (default: True)')
