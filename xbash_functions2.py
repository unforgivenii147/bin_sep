...
"""xbash_functions2.py – Xbash Functions2 utilities.

This module provides functionality for xbash functions2."""
...
EXCLUDED = {'.py', '.h', '.c', '.js', '.ts', '.hpp', '.cpp', '.pyx', '.jsx', '.lua', '.tsx', '.pl', '.am', '.pm', '.so', '.rmeta', '.syntax'}

class ShellScriptFinder:
    """ShellScriptFinder – ShellScriptFinder."""

    def __init__(self, include_extensionless: bool=True, skip_hidden: bool=False) -> None:
        """__init__ –   init  .

Args:
    include_extensionless: Description of include_extensionless.
...
Args:
    sh_file: Description of sh_file.

Returns:
    Generator[tuple[str, str], None, None]: Description of return value."""
        try:
        ...
        while i < len(lines):
            ...
            if match:
                ...
                if brace_count == 0:
                    ...
                    print(f"Warning: Could not find matching closing brace for function '{func_name}' in {sh_file}", file=sys.stderr)
                i = j
            else:
                i += 1
...
Args:
    ...
    use_extension: Description of use_extension."""
        self.output_dir = output_dir
        self.use_extension = use_extension
        self.written_count = 0

    def write_function(self, func_name: str, func_content: str, source_file: Path) -> Path | None:
        """write_function – write function.

Args:
    func_name: Description of func_name.
    func_content: Description of func_content.
    source_file: Description of source_file.
...
def process_paths(input_paths: list[Path], output_dir: Path, include_extensionless: bool=True, use_extension: bool=True, skip_hidden: bool=False, verbose: bool=False) -> tuple[int, int]:
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Extract functions from shell scripts using generator-style filesystem walker', formatter_class=argparse.RawDescriptionHelpFormatter, epilog="\nExamples:\n  # Process all shell scripts in current directory recursively\n  %(prog)s\n  \n  # Process specific files and directories\n  %(prog)s script1.sh myscript dir1/ dir2/\n  \n  # Specify output directory\n  %(prog)s -o extracted_functions dir1/ dir2/\n  \n  # Only process .sh files (ignore extensionless scripts)\n  %(prog)s --sh-only\n  \n  # Skip hidden files and directories\n  %(prog)s --skip-hidden\n  \n  # Verbose output\n  %(prog)s --verbose\n\nGenerator Benefits:\n  - Memory efficient: only processes one file at a time\n  - Lazy evaluation: doesn't load all files into memory\n  - Ideal for large directory trees and Termux environments\n  - Uses os.scandir() for efficient filesystem walking\n        ")
    parser.add_argument('inputs', nargs='*', type=Path, help='Files and/or directories to process. If none provided, processes all shell scripts in current directory recursively.')
    parser.add_argument('-o', '--output', type=Path, default=Path('extracted_functions'), help='Output directory for extracted functions (default: extracted_functions)')
    parser.add_argument('--sh-only', action='store_true', help='Only process files with .sh extension (ignore extensionless scripts)')
    ...
    parser.add_argument('--dry-run', action='store_true', help='Only find and list scripts without extracting functions')
    ...
    if args.dry_run:
        ...
        return 0
    print('Searching for shell scripts...')
    files_processed, functions_extracted = process_paths(input_paths=input_paths, output_dir=args.output, include_extensionless=include_extensionless, use_extension=not args.no_extension, skip_hidden=args.skip_hidden, verbose=args.verbose)
    print('\nDone!')
