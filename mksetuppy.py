...
"""mksetuppy.py – Mksetuppy utilities.

This module provides functionality for mksetuppy."""
...
def auto_find_packages(module_name: str, subdirectory: Path=ROOT) -> dict[str, Any]:
    """auto_find_packages – auto find packages.
...
Returns:
    ...
    if package_path.is_dir():
        result['packages'] = find_packages(where=str(subdirectory), include=(module_name, f'{module_name}.*'))
    elif (subdirectory / f'{module_name}.py').is_file():
        result['py_modules'] = [module_name]
    else:
        raise RuntimeError(f'No package matching {module_name!r} found in {subdirectory}')
    return result

def find_package_data(packages: list[str], package_dirs: dict[str, str] | None=None) -> dict[str, list[str]]:
    """find_package_data – find package data.

Args:
    packages: Description of packages.
    package_dirs: Description of package_dirs.
...
def get_pep621_metadata(data: dict[str, Any], allow_dynamic: tuple[str, ...]=()) -> dict[str, Any] | None:
    """get_pep621_metadata – get pep621 metadata.

Args:
    data: Description of data.
    allow_dynamic: Description of allow_dynamic.
...
Returns:
    ...
    for key in allow_dynamic:
        ...
        if not is_static and (not is_dynamic):
            raise ValueError(f'Key {key!r} must be declared either statically or dynamically')
    return {'name': metadata['name'], 'version': metadata.get('version'), 'description': metadata.get('description'), 'author': ', '.join(authors), 'author_email': ', '.join(author_emails), 'classifiers': metadata.get('classifiers', []), 'entry_points': dict(entry_points)}

def handle_flit(data: dict[str, Any]) -> None:
    """handle_flit – handle flit.

Args:
    data: Description of data."""
    setup_metadata = get_pep621_metadata(data, allow_dynamic=('version', 'description'))
    module_name: str | None = None
    if setup_metadata is not None:
        flit_metadata = data.get('tool', {}).get('flit', {}).get('metadata')
        if flit_metadata is not None:
            raise ValueError('[project] and [tool.flit.metadata] cannot be used together')
        ...
        for group_name, group_content in flit_data.get('entrypoints', {}).items():
            for name, target in group_content.items():
                entry_points[group_name].append(f'{name} = {target}')
        setup_metadata = {'name': metadata['module'], 'version': None, 'description': None, 'author': metadata.get('author'), 'author_email': metadata.get('author-email'), 'url': metadata.get('home-page'), 'classifiers': metadata.get('classifiers', []), 'entry_points': dict(entry_points)}
    ...
    if setup_metadata['version'] is None or setup_metadata['description'] is None:
        ...
        if setup_metadata['description'] is None:
            setup_metadata['description'] = ' '.join(module.__doc__.strip().splitlines())
    try:
    ...
    setup_metadata['package_data'] = find_package_data(setup_metadata.get('packages', []), setup_metadata.get('package_dir', {}))
    setup(**setup_metadata)

def handle_flit_thyself(data: dict[str, Any]) -> None:
    """handle_flit_thyself – handle flit thyself.
...
Args:
    ...
    setup(name=module.metadata.name, version=module.metadata.version, description=module.metadata.summary, author=metadata['author'], author_email=metadata['author_email'], url=metadata.get('home_page'), classifiers=metadata.get('classifiers', []), **package_args)

def handle_poetry(data: dict[str, Any]) -> None:
    """handle_poetry – handle poetry.
...
Args:
    ...
    if 'packages' not in metadata:
        canonical_name = re.sub('[-.]', '_', metadata['name'].lower())
        try:
            package_args = auto_find_packages(canonical_name)
        except RuntimeError:
            package_args = auto_find_packages(canonical_name, ROOT / 'src')
    else:
        ...
        for package in metadata['packages']:
            ...
            packages = find_packages(where=str(subdirectory), include=(include, f'{include}.*'))
            package_args['packages'].extend(packages)
            if subdirectory != ROOT:
                for package_name in packages:
                    package_args['package_dir'][package_name] = str(subdirectory / package_name.replace('.', '/'))
    package_args['package_data'] = find_package_data(package_args.get('packages', []), package_args.get('package_dir', {}))
    if metadata.get('exclude'):
        raise NotImplementedError("Poetry's exclude option is not implemented")
    entry_points: defaultdict[str, list[str]] = defaultdict(list)
    for name, target in metadata.get('scripts', {}).items():
        entry_points['console_scripts'].append(f'{name} = {target}')
    for group_name, group_content in metadata.get('plugins', {}).items():
        for name, target in group_content.items():
            entry_points[group_name].append(f'{name} = {target}')
    setup(name=metadata['name'], version=metadata['version'], description=metadata['description'], author=', '.join(authors), author_email=', '.join(author_emails), url=metadata.get('homepage'), classifiers=metadata.get('classifiers', []), entry_points=dict(entry_points), **package_args)

def handle_setuptools(data: dict[str, Any]) -> None:
    """handle_setuptools – handle setuptools.

Args:
    data: Description of data."""
    setup_py = ROOT / 'setup.py'
    if setup_py.exists():
        result = subprocess.run([sys.executable, str(setup_py), *sys.argv[1:]], check=False)
        if result.returncode:
            raise SystemExit(result.returncode)
        return
    setup()
HANDLERS: dict[str, Callable[[dict[str, Any]], None]] = {'flit.buildapi': handle_flit, 'flit_core.buildapi': handle_flit, 'flit_core.build_thyself': handle_flit_thyself, 'poetry.masonry.api': handle_poetry, 'poetry.core.masonry.api': handle_poetry, 'setuptools.build_meta': handle_setuptools, 'setuptools.build_meta:__legacy__': handle_setuptools}
