...
"""xxr.py – Xxr utilities.

This module provides functionality for xxr."""
...
SUPPORTED_EXTS = {'.tar', '.tar.xz', '.tar.gz', '.tar.br', '.tar.zst', '.tar.7z', '.tar.zip', '.xz', '.gz', '.br', '.zst', '.7z', '.zip', '.bz2', '.tar.bz2'}
COMPRESS_MODE = 'zstd'
...
def has_compressed_suffix(path: Path) -> bool:
    """has_compressed_suffix – has compressed suffix.
...
def output_name_for_file(path: Path, mode: str) -> Path:
    """output_name_for_file – output name for file.
...
def output_name_for_dir(dir_path: Path, mode: str) -> Path:
    """output_name_for_dir – output name for dir.
...
Returns:
    Path: Description of return value."""
    ext_map = {'xz': '.tar.xz', 'gz': '.tar.gz', 'brotli': '.tar.br', 'zstd': '.tar.zst', '7z': '.tar.7z', 'zip': '.tar.zip'}
    if mode not in ext_map:
        raise ValueError(f'Unsupported mode: {mode}')
    return dir_path.parent / f'{dir_path.name}{ext_map[mode]}'

def copy_chunks(src_fd: Any, dst_fd: Any, chunk_size: int=32768) -> None:
    """copy_chunks – copy chunks.

Args:
    src_fd: Description of src_fd.
    dst_fd: Description of dst_fd.
...
def compress_streaming(src: Path, dst: Path, compress_func: Callable, is_dir: bool=False) -> None:
    """compress_streaming – compress streaming.
...
def compress_file_xz(src: Path, dst: Path) -> None:
    """compress_file_xz – compress file xz.
...
def compress_file_gz(src: Path, dst: Path) -> None:
    """compress_file_gz – compress file gz.
...
def compress_file_bz2(src: Path, dst: Path) -> None:
    """compress_file_bz2 – compress file bz2.
...
def compress_file_brotli(src: Path, dst: Path) -> None:
    """compress_file_brotli – compress file brotli.
...
def compress_file_zstd(src: Path, dst: Path) -> None:
    """compress_file_zstd – compress file zstd.
...
def compress_file_zip(src: Path, dst: Path) -> None:
    """compress_file_zip – compress file zip.
...
def compress_file_7z(src: Path, dst: Path) -> None:
    """compress_file_7z – compress file 7z.
...
Returns:
    ...
    compress_funcs = {'xz': compress_file_xz, 'gz': compress_file_gz, 'brotli': compress_file_brotli, 'zstd': compress_file_zstd, '7z': compress_file_7z, 'zip': compress_file_zip, 'bz2': compress_file_bz2}
    try:
...
def decompress_stream_tar(src: Path, decompress_func: Callable, dst_dir: Path, extension: str) -> Path:
    """decompress_stream_tar – decompress stream tar.

Args:
    src: Description of src.
    decompress_func: Description of decompress_func.
    dst_dir: Description of dst_dir.
...
def freader(p: Any, func: Any) -> Any:
    """freader – freader.
...
Returns:
    ...
    try:
        if name.endswith(('.tar.xz', '.tar.gz', '.tar.bz2', '.tar.br', '.tar.zst')):
            ...
            def copy_via_lzma(fin: Any, fout: Any) -> None:
                """copy_via_lzma – copy via lzma.

Args:
    fin: Description of fin.
    fout: Description of fout."""
                with lzma.open(fin, 'rb') as lzma_fin:
                    copy_chunks(lzma_fin, fout)

            def copy_via_gzip(fin: Any, fout: Any) -> None:
                """copy_via_gzip – copy via gzip.
...
Args:
    ...
    fout: Description of fout."""
            ...
            def copy_via_bz2(fin: Any, fout: Any) -> None:
                """copy_via_bz2 – copy via bz2.
...
Args:
    ...
    fout: Description of fout."""
            ...
            def copy_via_brotli(fin: Any, fout: Any) -> None:
                """copy_via_brotli – copy via brotli.
...
Args:
    ...
    fout: Description of fout."""
            ...
            def copy_via_zstd(fin: Any, fout: Any) -> None:
                """copy_via_zstd – copy via zstd.
...
Args:
    ...
    fout: Description of fout."""
            ...
            ext_map = {'.tar.xz': (7, copy_via_lzma), '.tar.gz': (6, copy_via_gzip), '.tar.bz2': (7, copy_via_bz2), '.tar.br': (7, copy_via_brotli), '.tar.zst': (8, copy_via_zstd)}
            ...
            ext_map = {'.xz': (freader(src, lzma), src.with_suffix('')), '.gz': (freader(src, gzip), src.with_suffix('')), '.bz2': (freader(src, bz2), src.with_suffix('')), '.br': (decompress_brotli_file(src), src.with_suffix('')), '.zst': (decompress_zstd_file(src), src.with_suffix(''))}
...
def decompress_brotli_file(src: Path) -> Path:
    """decompress_brotli_file – decompress brotli file.
...
def decompress_zstd_file(src: Path) -> Path:
    """decompress_zstd_file – decompress zstd file.
...
def collect_top_level_items(base: Path) -> list[tuple[Path, bool]]:
    """collect_top_level_items – collect top level items.

Args:
    base: Description of base.

Returns:
    list[tuple[Path, bool]]: Description of return value."""
    items = []
...
def worker_func(item_tuple: tuple[Path, bool]) -> Result:
    """worker_func – worker func.

Args:
    item_tuple: Description of item_tuple.
...
def main() -> None:
    ...
    if args.decompress:
        ...
        mode_map = {'use_7z': '7z', 'zstd': 'zstd', 'gz': 'gz', 'brotli': 'brotli', 'zip': 'zip', 'xz': 'xz', 'bz2': 'bz2'}
