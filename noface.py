...
"""noface.py – Noface utilities.

This module provides functionality for noface."""
...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', handlers=[logging.FileHandler(Path.home() / 'face_detection.log'), logging.StreamHandler()])
...
def create_face_detector(cascade_path: Path | str) -> Any:
    """create_face_detector – create face detector.

Args:
    cascade_path: Description of cascade_path."""
...
def process_image_batch(args: str) -> Any:
    """process_image_batch – process image batch.
...
Args:
    ...
    exclude_dir: Description of exclude_dir.
...
def process_images(num_workers: int | None=None) -> bool:
...
Args:
    ...
    if num_workers > 1:
        with Pool(processes=num_workers) as pool:
            for result in tqdm(pool.imap_unordered(process_image_batch, args_list), total=len(images), desc='Processing images', unit='img'):
