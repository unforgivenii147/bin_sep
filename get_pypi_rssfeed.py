...
"""get_pypi_rssfeed.py – Get Pypi Rssfeed utilities.

This module provides functionality for get pypi rssfeed."""
...
def parse_rss_feed(xml_content: str) -> list[dict[str, str]]:
    """parse_rss_feed – parse rss feed.

Args:
    xml_content: Description of xml_content.

Returns:
    list[dict[str, str]]: Description of return value."""
    packages = []
    try:
        ...
        for item in items:
            package_info = {'title': item.findtext('title', 'Unknown'), 'link': item.findtext('link', 'Unknown'), 'description': item.findtext('description', 'No description'), 'pub_date': item.findtext('pubDate', 'Unknown'), 'guid': item.findtext('guid', 'Unknown')}
...
def display_packages(packages: list[dict[str, str]], limit: int | None=None) -> None:
    """display_packages – display packages.
...
Args:
    ...
    for i, pkg in enumerate(display_packages, 1):
        ...
        print(f"  Description: {pkg['description'][:100]}..." if len(pkg['description']) > 100 else f"  Description: {pkg['description']}")
        print(f"  GUID:        {pkg['guid']}")
        print('-' * 40)

def save_to_file(packages: list[dict[str, str]], filename: str='pypi_packages.txt') -> None:
