...
"""wpack.py – Wpack utilities.

This module provides functionality for wpack."""
...
def find_dist_info_dir(pkg_dir: Path) -> Path | None:
    """find_dist_info_dir – find dist info dir.

Args:
    pkg_dir: Description of pkg_dir.
...
Returns:
    ...
    if not candidates:
        return None
    if len(candidates) > 1:
        print(f'Warning: Multiple .dist-info dirs found in {pkg_dir}, using the first: {candidates[0].name}', file=sys.stderr)
    return candidates[0]

def create_wheel_for_dir_sync(pkg_dir: Path, dest_dir: Path | None=None) -> tuple[str, bool]:
    """create_wheel_for_dir_sync – create wheel for dir sync.
...
async def process_package_async(pkg_dir: Path, dest_dir: Path | None, task_queue: asyncio.Queue) -> None:
    """process_package_async – process package async.

Args:
    pkg_dir: Description of pkg_dir.
    dest_dir: Description of dest_dir.
    task_queue: Description of task_queue."""
...
def main_multiprocessing() -> None:
    """main_multiprocessing – main multiprocessing."""
    if WHEELS_OUTPUT_DIR:
        WHEELS_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
        print(f'Output directory for wheels: {WHEELS_OUTPUT_DIR}')
    dirs_to_process = []
