...
"""tlic.py – Tlic utilities.

This module provides functionality for tlic."""
...
def extract_blocks_from_file(path: Path, min_lines: int=2) -> list[tuple[str, int, list[str]]]:
    """extract_blocks_from_file – extract blocks from file.
...
def collect_blocks_parallel(root: Path, min_lines: int=2, n_jobs: int=8) -> dict[str, list[tuple[Path, int, list[str]]]]:
    """collect_blocks_parallel – collect blocks parallel.
...
Returns:
    ...
    for batch_start in range(0, total_files, batch_size):
        ...
        results = Parallel(n_jobs=n_jobs, prefer='threads', verbose=0)((delayed(extract_blocks_from_file)(path, min_lines) for path in batch_files))
...
def process_file_removal(path: Path, removals: list[tuple[int, list[str]]], root: Path) -> tuple[Path, int, bool]:
    """process_file_removal – process file removal.

Args:
    path: Description of path.
    removals: Description of removals.
    root: Description of root.
...
Args:
    ...
    results = Parallel(n_jobs=n_jobs, prefer='threads', verbose=0)((delayed(process_file_removal)(path, removals, root) for path, removals in file_removals.items()))
    removed_total = 0
    files_changed = 0
...
def main() -> None:
    ...
    parser.add_argument('-r', '--remove', action='store_true', help='Remove found repeated multi-line blocks from files')
    parser.add_argument('--min-lines', type=int, default=2, help='Minimum consecutive lines to consider a block (default: 2)')
