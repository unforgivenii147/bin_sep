...
"""tsext.py – Tsext utilities.

This module provides functionality for tsext."""
...
def extract_python_code_elements(path: Path) -> Any:
    """extract_python_code_elements – extract python code elements.
    ...
    while nodes_to_visit:
        ...
        for child in node.children:
            if child.type == 'function_definition':
                ...
                if (target and target.text.decode('utf-8').isupper() and (len(target.text.decode('utf-8')) > 1)) and child.named_child_count == 2:
                    constants.append(target.text.decode('utf-8'))
            elif child.type == 'import_statement':
                imports.extend((import_node.text.decode('utf-8') for import_node in child.children if import_node.type == 'dotted_name'))
