...
"""strict_filter_noneng.py – Strict Filter Noneng utilities.

This module provides functionality for strict filter noneng."""
...
def is_english_strict(line: str, detector: Any, english_vocab: set, min_ratio: float=0.5) -> tuple[bool, str]:
    """is_english_strict – is english strict.

Args:
    line: Description of line.
    detector: Description of detector.
    english_vocab: Description of english_vocab.
    min_ratio: Description of min_ratio.
...
Returns:
    ...
    tokens = re.findall('\\b[a-zA-Z]+\\b', clean_line.lower())
    if not tokens:
...
def process_file_lines(input_path: Path, move_mode: bool, strict_ratio: float) -> None:
    """process_file_lines – process file lines.

Args:
    input_path: Description of input_path.
    move_mode: Description of move_mode.
    strict_ratio: Description of strict_ratio."""
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Filter non-English lines strictly using a hybrid GCLD3 and NLTK dictionary method.')
    parser.add_argument('file', type=str, help='The text file to scan.')
    parser.add_argument('-m', '--move', action='store_true', help='Isolate non-English lines into noneng.txt and update source in-place.')
    parser.add_argument('-t', '--threshold', type=float, default=0.5, help='Minimum ratio (0.0 to 1.0) of tokens that must be valid NLTK English words (default: 0.50).')
