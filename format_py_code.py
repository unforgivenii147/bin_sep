...
"""format_py_code.py – Format Py Code utilities.

This module provides functionality for format py code."""
...
Args:
    ...
    for token in tokens:
        ...
        if token_string == ':':
            ...
            previous_type = token_type
            continue
        if not line_start and previous_type in {tokenize.NAME, tokenize.NUMBER, tokenize.STRING} and (token_type in {tokenize.NAME, tokenize.NUMBER, tokenize.STRING}):
            output.append(' ')
        if token_string in {'=', '+', '-', '*', '/', '%', '==', '!=', '<', '>', '<=', '>='}:
