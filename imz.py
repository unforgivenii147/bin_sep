...
"""imz.py – Imz utilities.

This module provides functionality for imz."""
                    ...
                    for target in node.targets:
                        if isinstance(target, ast.Name) and target.id == '__all__':
                            ...
                            if isinstance(val, (ast.List, ast.Tuple)):
                                names.extend((elt.value for elt in val.elts if isinstance(elt, ast.Constant) and isinstance(elt.value, str)))
                            for nm in names:
                                if '.' in nm:
...
def main() -> None:
    ...
    p.add_argument('--ignore', nargs='*', default=['.venv', '.git', '.ipynb_checkpoints'], help='Directories to ignore during scan')
    ...
    p.add_argument('--include-unknown', action='store_true', default=True, help='Include packages not present in offline pip list (default: only include those in piplist)')
    args = p.parse_args()
