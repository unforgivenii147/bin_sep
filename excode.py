...
"""excode.py – Excode utilities.

This module provides functionality for excode."""
from __future__ import annotations
from typing import Any
import re
from pathlib import Path
LANG_TO_EXT = {'python': '.py', 'py': '.py', 'javascript': '.js', 'js': '.js', 'typescript': '.ts', 'ts': '.ts', 'c': '.c', 'cpp': '.cpp', 'c++': '.cpp', 'h': '.h', 'java': '.java', 'csharp': '.cs', 'c#': '.cs', 'go': '.go', 'golang': '.go', 'rust': '.rs', 'ruby': '.rb', 'rails': '.rb', 'php': '.php', 'swift': '.swift', 'kotlin': '.kt', 'scala': '.scala', 'sql': '.sql', 'bash': '.sh', 'sh': '.sh', 'zsh': '.sh', 'shell': '.sh', 'powershell': '.ps1', 'ps1': '.ps1', 'yaml': '.yaml', 'yml': '.yaml', 'json': '.json', 'html': '.html', 'htm': '.html', 'css': '.css', 'make': 'Makefile', 'makefile': 'Makefile', 'dockerfile': 'Dockerfile', 'text': '.txt', 'txt': '.txt', 'plain': '.txt'}
CODE_BLOCK_RE = re.compile('```(?P<lang>[A-Za-z0-9_+\\-.]*)[ \\t]*\\n(?P<code>.*?)(?<=\\n)```', re.DOTALL | re.IGNORECASE)
...
def extract_code_blocks(md_file: Path, out_dir: Path) -> Any:
    """extract_code_blocks – extract code blocks.

Args:
    md_file: Description of md_file.
