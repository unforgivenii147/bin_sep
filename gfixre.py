...
"""gfixre.py – Gfixre utilities.

This module provides functionality for gfixre."""
...
class RegexRawConverter(ast.NodeTransformer):
    """RegexRawConverter – RegexRawConverter."""

    def __init__(self, source_lines: Any, source_text: str) -> None:
        """__init__ –   init  .

Args:
    source_lines: Description of source_lines.
    source_text: Description of source_text."""
...
Args:
    node: Description of node."""
        if isinstance(node.func, ast.Attribute) and node.func.attr in ('compile', 'match', 'search', 'sub', 'findall', 'split', 'fullmatch') and node.args and isinstance(node.args[0], ast.Constant):
            const = node.args[0]
            if (isinstance(const.value, str) and '\\' in const.value and (not const.value.startswith('\\x'))) and const.lineno <= len(self.source_lines):
...
def convert_to_raw_string(source_text: str) -> str:
    """convert_to_raw_string – convert to raw string.
    ...
    try:
        ...
        if original != converted:
            if autofix:
                ...
                diff = list(unified_diff(orig_lines, conv_lines, fromfile=str(path), tofile=str(path)))
                return {'status': 'diff', 'path': path, 'diff': diff}
        return {'status': 'unchanged', 'path': path}
    except (SyntaxError, UnicodeDecodeError) as e:
        return {'status': 'error', 'path': path, 'error': str(e)}

def process_file_wrapper(args: str) -> Any:
