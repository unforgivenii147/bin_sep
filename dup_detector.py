...
"""dup_detector.py – Dup Detector utilities.

This module provides functionality for dup detector."""
...
def get_py_files(directory: Path | str) -> list[Path]:
...
def extract_objects(path: Path | str) -> Any:
...
Args:
    ...
    for node in tree.body:
        ...
        if obj_type and name:
            ...
            objects.append({'object_type': obj_type, 'object_name': name, 'source_code': src_code, 'reference_file': path, 'content_hash': content_hash, 'start_line': node.lineno, 'end_line': node.end_lineno, 'ssdeep_hash': ssdeep.hash(src_code)})
    return objects
...
def save_exact_duplicates(all_objects: Any, output_file: str='exact_duplicates.json') -> Any:
    """save_exact_duplicates – save exact duplicates.
...
Args:
    ...
    for content_hash, group in hash_groups.items():
        ...
        if occurrence_count <= 1:
            continue
        for obj in group:
            duplicate_objects.append({'object_type': obj['object_type'], 'object_name': obj['object_name'], 'source_code': obj['source_code'], 'reference_file': obj['reference_file'], 'content_hash': content_hash, 'occurrence_count': occurrence_count, 'start_line': obj['start_line'], 'end_line': obj['end_line']})
    duplicate_objects.sort(key=lambda obj: (-obj['occurrence_count'], obj['object_type'], obj['object_name'], obj['reference_file']))
...
Args:
    ...
    output_files = {'function': 'function_duplicates.json', 'class': 'class_duplicates.json', 'constant': 'constant_duplicates.json'}
...
def refactor_duplicates(hash_groups: Any) -> None:
    """refactor_duplicates – refactor duplicates.

Args:
    hash_groups: Description of hash_groups."""
    utils_path = 'utils.py'
    utils_content = ''
    ...
    for path, names_to_import in file_imports.items():
        with open(path, 'r', encoding='utf-8') as f:
            lines = f.readlines()
        objs_to_remove = [obj for group in hash_groups.values() if len(group) > 5 for obj in group if obj['reference_file'] == path]
...
def generate_fuzzy_report(all_objects: Any, output_file: str='fuzzy_duplicates.json') -> None:
    """generate_fuzzy_report – generate fuzzy report.
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if ssdeep_sim > 0:
                ...
                if ratio > 50.0:
                    report.append({'object_1': {'type': obj1['object_type'], 'name': obj1['object_name'], 'file': obj1['reference_file'], 'source_code': obj1['source_code']}, 'object_2': {'type': obj2['object_type'], 'name': obj2['object_name'], 'file': obj2['reference_file'], 'source_code': obj2['source_code']}, 'similarity_percentage': round(ratio, 2), 'ssdeep_score': ssdeep_sim})
...
def main() -> None:
    ...
    parser.add_argument('-r', '--refactor', action='store_true', help='Move duplicates with >5 appearances to utils.py and update imports.')
    parser.add_argument('-f', '--fuzzy', action='store_true', help='Generate a report for objects with >50% similarity using ssdeep and rapidfuzz.')
