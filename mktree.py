...
"""mktree.py – Mktree utilities.

This module provides functionality for mktree."""
...
Returns:
    str: Description of return value."""
        has_tree_symbols = any((any((sym in line for sym in self.TREE_SYMBOLS)) for line in lines))
    ...
    def _parse_tree_format(self, lines: list[str]) -> None:
        """_parse_tree_format –  parse tree format.
    ...
    lines: Description of lines."""
        ...
        for line in lines:
            ...
            if not name:
                continue
            if name in ('.', './', '.\\'):
                self.entries.append({'depth': 0, 'name': '.', 'is_dir': True, 'explicit_dir': True, 'raw': name})
            ...
            explicit_dir = name.endswith(('/', '\\'))
            self.entries.append({'depth': depth, 'name': name, 'path': current_path, 'is_dir': explicit_dir, 'explicit_dir': explicit_dir, 'raw': name})
            if not explicit_dir:
                stack.append((depth, current_path))

    def _parse_indented_format(self, lines: list[str]) -> None:
        """_parse_indented_format –  parse indented format.
    ...
    lines: Description of lines."""
        ...
        for line in lines:
            ...
            if stack:
                ...
                current_path = self.base_path / name
            explicit_dir = name.endswith(('/', '\\'))
            self.entries.append({'depth': indent, 'name': name, 'path': current_path, 'is_dir': explicit_dir, 'explicit_dir': explicit_dir, 'raw': name})
            if not explicit_dir:
                stack.append((indent, current_path))

    def _parse_simple_format(self, lines: list[str]) -> None:
        """_parse_simple_format –  parse simple format.
...
Args:
    lines: Description of lines."""
        for line in lines:
            ...
            self.entries.append({'depth': 0, 'name': clean_name, 'path': self.base_path / clean_name, 'is_dir': explicit_dir, 'explicit_dir': explicit_dir, 'raw': name})

    def _clean_name(self, name: str) -> str:
        """_clean_name –  clean name.
...
Returns:
    ...
    def _determine_file_types(self) -> None:
        """_determine_file_types –  determine file types."""
        ...
        for i, entry in enumerate(self.entries):
            ...
            if i + 1 < len(self.entries):
                ...
                if next_entry['depth'] > entry['depth']:
                    entry['is_dir'] = True
        for entry in self.entries:
            if not entry['is_dir'] and '.' in entry['name'].split('/')[-1].split('\\')[-1]:
                entry['is_dir'] = False
...
Returns:
    Image.Image | None: Description of return value."""
        try:
...
Returns:
    str: Description of return value."""
        if not HAS_OCR:
            raise ImportError('OCR libraries not installed. Install with: pip install pytesseract opencv-python pillow')
        ...
        try:
            custom_config = '--oem 3 --psm 6 -c tessedit_char_whitelist=./\\abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-\\|\x1b'
...
Returns:
    ...
    if ImageProcessor.is_image_file(path):
        if not HAS_OCR:
            raise ImportError('Image input requires OCR libraries. Install with: pip install pytesseract opencv-python pillow numpy')
        text = ImageProcessor.extract_text_from_image(path)
        return text
    return path.read_text(encoding='utf-8', errors='replace')

def create_tree(entries: list[dict[str, Any]], base_path: Path, dry_run: bool=False, verbose: bool=False) -> tuple[int, int]:
...
def validate_entries(entries: list[dict[str, Any]]) -> bool:
    """validate_entries – validate entries.
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Create folder tree from text file or image', formatter_class=argparse.RawDescriptionHelpFormatter, epilog='\nExamples:\n  python create_tree.py tree.txt\n  python create_tree.py folder_tree.png\n  python create_tree.py tree.txt --output ./my_project\n  python create_tree.py tree.txt --dry-run\n  python create_tree.py tree.txt -v\n        ')
    parser.add_argument('input', nargs='?', default='-', help='Input file (text or image) or "-" for stdin')
    ...
    parser.add_argument('--format', choices=['auto', 'tree', 'simple', 'indented'], default='auto', help='Force a specific input format (default: auto-detect)')
    ...
    if args.verbose:
        ...
        print()
    created_dirs, created_files = create_tree(entries, base_path, dry_run=args.dry_run, verbose=args.verbose)
    if args.dry_run:
