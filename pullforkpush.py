...
"""pullforkpush.py – Pullforkpush utilities.

This module provides functionality for pullforkpush."""
...
def setup_github_client() -> Any:
    """setup_github_client – setup github client."""
...
def get_repo_info(repo: git.Repo) -> Any:
...
def main() -> None:
    ...
    try:
        ...
        if not active_branch.tracking_branch():
            print(f"[!] Active branch '{active_branch.name}' has no upstream tracking branch. Setting to origin/{active_branch.name}")
