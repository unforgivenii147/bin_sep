...
"""cleanmd.py – Cleanmd utilities.

This module provides functionality for cleanmd."""
...
class MarkdownPatterns:
    ...
    INLINE_IMAGE = re.compile('!\\[([^\\[\\]]*)\\]\\(([^\\)]+)\\)', re.MULTILINE)
    HTML_IMG_TAG = re.compile('<img\\s+[^>]*src=[\'\\"]?([^\'\\">\\s]+)[\'\\"]?[^>]*/?>\\s*', re.IGNORECASE | re.MULTILINE)
    REFERENCE_IMAGE = re.compile('!\\[([^\\[\\]]*)\\]\\[([^\\[\\]]+)\\]', re.MULTILINE)
    IMAGE_DEF = re.compile('^\\s*\\[([^\\[\\]]+)\\]:\\s*(.+?(?:\\.(?:png|jpg|jpeg|gif|webp|svg|bmp))?)\\s*(?:\\"[^\\"]*\\")?\\s*$', re.MULTILINE | re.IGNORECASE)
    PICTURE_TAG = re.compile('<picture\\s*>.*?</picture>', re.DOTALL | re.IGNORECASE)
    FIGURE_TAG = re.compile('<figure\\s*>.*?</figure>', re.DOTALL | re.IGNORECASE)

class MarkdownImageRemover:
    """MarkdownImageRemover – MarkdownImageRemover."""
...
Returns:
    ...
    def _remove_pattern(self, content: str, pattern: re.Pattern) -> tuple[str, int]:
        """_remove_pattern –  remove pattern.
...
def get_markdown_files(path: Path) -> list[Path]:
    """get_markdown_files – get markdown files.
...
Returns:
    ImageStats: Description of return value."""
    try:
        ...
        return ImageStats(path=path, rel_path=str(path.relative_to(Path.cwd())), images_removed=images_removed, references_removed=references_removed, original_size=original_size, final_size=final_size, error=None)
    except Exception as e:
        return ImageStats(path=path, rel_path=str(path.relative_to(Path.cwd())), images_removed=0, references_removed=0, original_size=0, final_size=0, error=str(e))

def worker_process_file(args: tuple[Path, ProcessingConfig]) -> ImageStats:
    """worker_process_file – worker process file.
...
class Reporter:
    ...
    @staticmethod
    def print_file_result(stats: ImageStats) -> None:
        """print_file_result – print file result.
...
Args:
    stats: Description of stats."""
        ...
        reduction_pct = reduction / stats.original_size * 40 if stats.original_size > 0 else 0
        status = Styling.success('✓') if stats.images_removed > 0 else Styling.dim('∘')
        print(f"{status} {Styling.info(stats.rel_path)}\n  Images removed: {Styling.style(str(stats.images_removed), Color.YELLOW, bold=True)} | Size: {Styling.dim(Reporter._format_size(stats.original_size))} → {Styling.dim(Reporter._format_size(stats.final_size))} {Styling.dim(f'(-{reduction_pct:.1f}%)')}")

    @staticmethod
    def print_summary(results: list[ImageStats]) -> None:
...
Args:
    results: Description of results."""
        ...
        reduction_pct = total_reduction / total_original * 40 if total_original > 0 else 0
        ...
        print(f"  Total size reduction: {Styling.style(Reporter._format_size(total_reduction), Color.BRIGHT_GREEN)} {Styling.dim(f'(-{reduction_pct:.1f}%)')}")
        print()
