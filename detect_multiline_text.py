...
"""detect_multiline_text.py – Detect Multiline Text utilities.

This module provides functionality for detect multiline text."""
...
class FileStats:
    ...
    def __str__(self) -> Any:
        ...
        if self.modified:
            return f'{rel}: Processed {self.bytes_processed:,} bytes, removed {self.removed_count} occurrence(s) ({self.bytes_removed:,} bytes)'
        return f'{rel}: No changes needed ({self.bytes_processed:,} bytes)'

def read_license_pattern() -> str:
    """read_license_pattern – read license pattern.
...
def find_text_files(directories: list[Path]) -> Iterator[Path]:
    """find_text_files – find text files.
...
def calculate_pattern_fingerprint(pattern: str) -> str:
    """calculate_pattern_fingerprint – calculate pattern fingerprint.
...
Args:
    ...
    pattern: Description of pattern.
    pattern_fingerprint: Description of pattern_fingerprint.
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Remove multi-line text pattern from files recursively.', formatter_class=argparse.RawDescriptionHelpFormatter, epilog='\nExamples:\n  %(prog)s\n  %(prog)s /path/to/dir\n  %(prog)s file1.txt file2.txt\n  %(prog)s -a /path/to/dir\n  %(prog)s --auto-remove\n        ')
    parser.add_argument('paths', nargs='*', type=Path, help='Files or directories to process (default: current directory)')
    parser.add_argument('-a', '--auto-remove', action='store_true', help='Automatically remove found patterns (without confirmation)')
    ...
    if args.auto_remove or args.dry_run:
        ...
        stats_list = Parallel(n_jobs=args.jobs, verbose=1)((delayed(process_file)(path, pattern, pattern_fingerprint) for path in files))
        print('\n' + '=' * 40)
        ...
        preview_stats = Parallel(n_jobs=args.jobs, verbose=0)((delayed(lambda f: (f, pattern in f.read_text(encoding='utf-8', errors='ignore') if f.stat().st_size < 10 * 1024 * 1024 else False))(path) for path in files))
