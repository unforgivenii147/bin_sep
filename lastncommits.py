...
"""lastncommits.py – Lastncommits utilities.

This module provides functionality for lastncommits."""
...
def get_created_files(n_commits: int) -> list:
    """get_created_files – get created files.

Args:
    n_commits: Description of n_commits.

Returns:
    list: Description of return value."""
    try:
        cmd = ['git', 'log', '-n', str(n_commits), '--pretty=format:', '--name-status', '--diff-filter=A']
...
def main() -> None:
    ...
    try:
        ...
        if created_files:
            ...
            print(f'No files created in the last {n_commits} commit(s)', file=sys.stderr)
    except ValueError:
