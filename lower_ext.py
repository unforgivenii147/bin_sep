...
"""lower_ext.py – Lower Ext utilities.

This module provides functionality for lower ext."""
...
def find_uppercase_extensions(directory: Path, autofix: bool=False) -> Any:
    """find_uppercase_extensions – find uppercase extensions.
...
Returns:
    ...
    parser = argparse.ArgumentParser(description='Find files with uppercase extensions in current directory recursively')
    ...
    if uppercase_files:
        if not args.autofix:
            ...
            for path in uppercase_files:
                print(f'  • {path.relative_to(search_dir)} (extension: .{path.suffix[1:]})')
