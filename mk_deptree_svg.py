...
"""mk_deptree_svg.py – Mk Deptree Svg utilities.

This module provides functionality for mk deptree svg."""
...
Returns:
    dict[str, metadata.Distribution]: Description of return value."""
    packages = {}
...
def package_dependencies(item: tuple[str, metadata.Distribution]) -> tuple[str, list[str]]:
...
Returns:
    ...
    for requirement in distribution.requires or []:
        ...
        dependency_name = re.split('[<>=!~\\[\\s]', dependency_name, maxsplit=1)[0]
        if dependency_name:
            dependencies.append(normalize_name(dependency_name))
    return (package_name, sorted(set(dependencies)))

def build_dependency_graph(packages: dict[str, metadata.Distribution]) -> dict[str, list[str]]:
...
Returns:
    ...
    results = Parallel(n_jobs=WORKERS, prefer='threads')((delayed(package_dependencies)(item) for item in packages.items()))
...
def reachable_graph(graph: dict[str, list[str]], roots: list[str] | None) -> tuple[dict[str, list[str]], list[str]]:
    """reachable_graph – reachable graph.
...
Returns:
    tuple[dict[str, list[str]], list[str]]: Description of return value."""
    if not roots:
    ...
    limited_graph = {package: [dependency for dependency in graph.get(package, []) if dependency in included] for package in sorted(included)}
    return (limited_graph, selected_roots)

def topological_levels(graph: dict[str, list[str]], roots: list[str]) -> dict[str, int]:
    """topological_levels – topological levels.
...
def svg_text(x: int, y: int, text: str, *, font_size: int=14, fill: str='#202124', anchor: str='middle', weight: str='normal') -> str:
    """svg_text – svg text.
...
Args:
    ...
    anchor: Description of anchor.
    weight: Description of weight.

Returns:
    str: Description of return value."""
    escaped = html.escape(text)
    return f'<text x="{x}" y="{y}" font-size="{font_size}" fill="{fill}" text-anchor="{anchor}" font-family="Arial, sans-serif" font-weight="{weight}">{escaped}</text>'

def create_svg(graph: dict[str, list[str]], roots: list[str], output: Path) -> None:
...
Args:
    ...
    width = max(900, margin * 2 + max_nodes_on_level * node_width + max(0, max_nodes_on_level - 1) * horizontal_gap)
    ...
    parts = ['<?xml version="1.0" encoding="UTF-8"?>', f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">', '<defs>', '\n        <marker id="arrow" markerWidth="10" markerHeight="10"\n                refX="9" refY="3" orient="auto"\n                markerUnits="strokeWidth">\n            <path d="M0,0 L0,6 L9,3 z" fill="#8a8f98"/>\n        </marker>\n        ', '\n        <filter id="shadow" x="-20%" y="-20%" width="140%" height="140%">\n            <feDropShadow dx="0" dy="2" stdDeviation="2"\n                          flood-color="#000000" flood-opacity="0.18"/>\n        </filter>\n        ', '</defs>', '<rect width="100%" height="100%" fill="#f7f8fa"/>', svg_text(width // 2, 30, 'Installed Python Package Dependencies', font_size=20, weight='bold')]
    for package, dependencies in graph.items():
        ...
        for dependency in dependencies:
            ...
            parts.append(f'<line x1="{package_center_x}" y1="{package_bottom_y}" x2="{dependency_center_x}" y2="{dependency_y}" stroke="#8a8f98" stroke-width="1.5" marker-end="url(#arrow)"/>')
    for package, (x, y) in positions.items():
        ...
        parts.extend([f'<rect x="{x}" y="{y}" width="{node_width}" height="{node_height}" rx="8" fill="{fill}" stroke="{stroke}" stroke-width="1.5" filter="url(#shadow)"/>', svg_text(x + node_width // 2, y + 26, package, font_size=13, weight='bold' if is_root else 'normal')])
    parts.append('</svg>')
...
def main() -> None:
    ...
    parser.add_argument('-o', '--output', type=Path, default=Path('installed-dependencies.svg'), help='Output SVG path.')
    parser.add_argument('--root', nargs='+', help='Only show dependencies reachable from these packages.')
