...
"""auto_archive.py – Auto Archive utilities.

This module provides functionality for auto archive."""
...
Args:
    data: Description of data.
    level: Description of level.
    src_name: Description of src_name.
...
def _make_algorithms() -> Any:
    """_make_algorithms –  make algorithms."""
    algos = []
    if zstd:
...
def best_for_algo(data: bytes, name: str, ext: str, fn: Any, min_l: int, max_l: int, src_name: str='data') -> tuple[str, str, int, bytes] | None:
    """best_for_algo – best for algo.
...
Args:
    ...
    min_l: Description of min_l.
    max_l: Description of max_l.
    src_name: Description of src_name.

Returns:
    tuple[str, str, int, bytes] | None: Description of return value."""
    best_size = None
    best_level = min_l
...
Returns:
    ...
    for name, ext, fn, min_l, max_l in ALGORITHMS:
        ...
        if result:
            ...
            log.info('  %-8s best level=%2d  %s → %s  (%.1f%%)', r_name, r_level, fsz(original_size), fsz(len(r_bytes)), 100 * len(r_bytes) / original_size)
...
def _worker(args: str) -> Path | None:
    """_worker –  worker.
