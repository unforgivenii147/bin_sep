...
"""cleanerts.py – Cleanerts utilities.

This module provides functionality for cleanerts."""
...
class ProcessResult:
    ...
    file_size: int = 0
    file_type: str = ''
EXTENSION_TO_LANGUAGE = {'.js': 'tree_sitter_javascript', '.jsx': 'tree_sitter_javascript', '.ts': 'tree_sitter_typescript', '.tsx': 'tree_sitter_tsx', '.rb': 'tree_sitter_ruby', '.php': 'tree_sitter_php', '.java': 'tree_sitter_java', '.c': 'tree_sitter_c', '.h': 'tree_sitter_c', '.cpp': 'tree_sitter_cpp', '.hpp': 'tree_sitter_cpp', '.cc': 'tree_sitter_cpp', '.cs': 'tree_sitter_c_sharp', '.go': 'tree_sitter_go', '.rs': 'tree_sitter_rust', '.swift': 'tree_sitter_swift', '.kt': 'tree_sitter_kotlin', '.scala': 'tree_sitter_scala', '.lua': 'tree_sitter_lua', '.r': 'tree_sitter_r', '.jl': 'tree_sitter_julia', '.dart': 'tree_sitter_dart', '.ex': 'tree_sitter_elixir', '.exs': 'tree_sitter_elixir', '.erl': 'tree_sitter_erlang', '.hs': 'tree_sitter_haskell', '.clj': 'tree_sitter_clojure', '.cljs': 'tree_sitter_clojure', '.fs': 'tree_sitter_fsharp', '.fsx': 'tree_sitter_fsharp', '.nim': 'tree_sitter_nim', '.zig': 'tree_sitter_zig', '.v': 'tree_sitter_v', '.sol': 'tree_sitter_solidity', '.html': 'tree_sitter_html', '.htm': 'tree_sitter_html', '.xml': 'tree_sitter_xml', '.css': 'tree_sitter_css', '.scss': 'tree_sitter_scss', '.sass': 'tree_sitter_sass', '.less': 'tree_sitter_less', '.vue': 'tree_sitter_vue', '.svelte': 'tree_sitter_svelte', '.sh': 'tree_sitter_bash', '.bash': 'tree_sitter_bash', '.zsh': 'tree_sitter_bash', '.fish': 'tree_sitter_fish', '.ps1': 'tree_sitter_powershell', '.vim': 'tree_sitter_vim', '.json': 'tree_sitter_json', '.jsonc': 'tree_sitter_json', '.yaml': 'tree_sitter_yaml', '.yml': 'tree_sitter_yaml', '.toml': 'tree_sitter_toml', '.ini': 'tree_sitter_ini', '.cfg': 'tree_sitter_ini', '.conf': 'tree_sitter_ini', '.sql': 'tree_sitter_sql', '.graphql': 'tree_sitter_graphql', '.gql': 'tree_sitter_graphql', '.md': 'tree_sitter_markdown', '.markdown': 'tree_sitter_markdown', '.rst': 'tree_sitter_rst', '.tex': 'tree_sitter_latex', '.org': 'tree_sitter_org', '.proto': 'tree_sitter_proto', '.cmake': 'tree_sitter_cmake', '.dockerfile': 'tree_sitter_dockerfile', '.make': 'tree_sitter_make', '.nix': 'tree_sitter_nix', '.elm': 'tree_sitter_elm', '.pug': 'tree_sitter_pug', '.wasm': 'tree_sitter_wasm', '.wgsl': 'tree_sitter_wgsl'}

class UniversalCommentRemover:
    """UniversalCommentRemover – UniversalCommentRemover."""
    ...
    def _load_language(self, module_name: str) -> Language | None:
        """_load_language –  load language.

Args:
    module_name: Description of module_name.

Returns:
    Language | None: Description of return value."""
    ...
    def get_parser_for_file(self, path: Path) -> tuple[Parser, str] | None:
        """get_parser_for_file – get parser for file.

Args:
    path: Description of path.

Returns:
    tuple[Parser, str] | None: Description of return value."""
...
Args:
    node: Description of node."""
            if node.type in ('comment', 'line_comment', 'block_comment', 'comment_block', 'comment_line', 'doc_comment'):
...
Args:
    content: Description of content.
    file_extension: Description of file_extension.
...
Returns:
    ...
    def _remove_json_comments(self, content: bytes) -> tuple[bytes, int]:
        """_remove_json_comments –  remove json comments.
...
Returns:
    tuple[bytes, int]: Description of return value."""
        import re
        pattern_line = re.compile(b'//.*?$', re.MULTILINE)
        pattern_block = re.compile(b'/\\*.*?\\*/', re.DOTALL)
        comments_count = 0
...
def collect_supported_files(inputs: list[str]) -> list[Path]:
    """collect_supported_files – collect supported files.
...
Returns:
    ...
    for input_path in inputs:
        ...
        if path.is_file():
            if path.suffix.lower() in supported_extensions or path.name.lower() in ('dockerfile', 'makefile'):
...
Returns:
    ...
    try:
        ...
        if file_size == 0:
            return ProcessResult(path=path, success=True, comments_removed=0, processing_time=time.perf_counter() - start_time, file_size=0, file_type=path.suffix)
        ...
        return ProcessResult(path=path, success=True, comments_removed=comments_removed, processing_time=processing_time, file_size=file_size, file_type=path.suffix)
    except Exception as e:
        processing_time = time.perf_counter() - start_time
        return ProcessResult(path=path, success=False, error_message=str(e), processing_time=processing_time, file_type=path.suffix if path.suffix else 'unknown')
            ...
            try:
                ...
                if result.success:
                    if result.comments_removed > 0:
                        ...
                        print(f'✓ [{result.file_type:6}] {result.path}: removed {result.comments_removed} comments ({size_kb:.1f} KB, {result.processing_time:.3f}s)')
                    else:
