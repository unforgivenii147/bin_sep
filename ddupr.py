...
"""ddupr.py – Ddupr utilities.

This module provides functionality for ddupr."""
...
def _collect_imports(tree: ast.Module, node: ast.AST) -> list[str]:
    """_collect_imports –  collect imports.
...
def _is_constant_node(node: ast.AST) -> tuple[bool, str]:
    """_is_constant_node –  is constant node.
...
Returns:
    list[PyObject]: Description of return value."""
    ...
    for node in tree.body:
        try:
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                objects.append(PyObject(kind='func', name=node.name, source=src, content_hash=_content_hash(src), origin_file=origin, node_lineno=node.lineno, node_end_lineno=node.end_lineno, imports=_collect_imports(tree, node)))
            elif isinstance(node, ast.ClassDef):
                src = _node_source(source, node)
                objects.append(PyObject(kind='class', name=node.name, source=src, content_hash=_content_hash(src), origin_file=origin, node_lineno=node.lineno, node_end_lineno=node.end_lineno, imports=_collect_imports(tree, node)))
            else:
                is_const, name = _is_constant_node(node)
                if is_const:
                    src = _node_source(source, node)
                    objects.append(PyObject(kind='const', name=name, source=src, content_hash=_content_hash(src), origin_file=origin, node_lineno=node.lineno, node_end_lineno=node.end_lineno, imports=_collect_imports(tree, node)))
...
def read_file_sources(path: Path) -> list[tuple[str, str]]:
    """read_file_sources – read file sources.
...
def _build_utils_source(objects: list[PyObject]) -> str:
    """_build_utils_source –  build utils source.

Args:
    objects: Description of objects.
...
def _validate_source(source: str, dest: Path) -> bool:
    """_validate_source –  validate source.
...
def write_utils(grouped: dict[str, list[PyObject]], utils_dir: Path, *, dry_run: bool=False) -> dict[str, Path]:
    """write_utils – write utils.

Args:
    grouped: Description of grouped.
    utils_dir: Description of utils_dir.
    dry_run: Description of dry_run.

Returns:
    dict[str, Path]: Description of return value."""
...
def _build_import_line(utils_dir: Path, cwd: Path, kind: str) -> str:
    """_build_import_line –  build import line.
...
def remove_and_patch(objects_to_remove: list[PyObject], utils_dir: Path, cwd: Path) -> None:
    """remove_and_patch – remove and patch.

Args:
    objects_to_remove: Description of objects_to_remove.
    utils_dir: Description of utils_dir.
    cwd: Description of cwd."""
    ...
    for path, objs in by_file.items():
        ...
        try:
            ...
            logger.success('Patched {}: removed {} definition(s), added imports', path, len(objs))
        except Exception as exc:
            logger.error('Cannot write patched {}: {}', path, exc)

def collect_all_paths(root: Path) -> list[Path]:
    """collect_all_paths – collect all paths.
...
Args:
    all_objects: Description of all_objects.

Returns:
    tuple[dict[str, list[PyObject]], dict[str, list[PyObject]]]: Description of return value."""
...
Args:
    ...
    print('Found {} duplicate group(s): {} func, {} class, {} const', len(duplicates), len(grouped.get('func', [])), len(grouped.get('class', [])), len(grouped.get('const', [])))
    if not duplicates:
        print('Nothing to do.')
        return
    if mode is None:
        for kind, objs in grouped.items():
            for obj in objs:
                print("[{}] '{}' duplicated in {} file(s)", kind, obj.name, len(duplicates[obj.content_hash]))
        return
...
Returns:
    ...
    parser = argparse.ArgumentParser(description='Extract duplicate Python definitions into utils/.', formatter_class=argparse.RawDescriptionHelpFormatter)
    group = parser.add_mutually_exclusive_group()
    group.add_argument('-c', '--copy', action='store_true', help='Copy duplicate definitions to utils/ (originals untouched)')
    group.add_argument('-m', '--move', action='store_true', help='Move duplicate definitions to utils/ and patch original files')
    parser.add_argument('--dir', type=Path, default=Path('.'), help='Root directory to scan (default: current directory)')
    parser.add_argument('--workers', type=int, default=max(1, cpu_count() - 1), help='Number of worker processes (default: cpu_count - 1)')
    parser.add_argument('--log-level', default='INFO', choices=['TRACE', 'DEBUG', 'INFO', 'SUCCESS', 'WARNING', 'ERROR'], help='Loguru log level (default: INFO)')
...
def main() -> None:
    ...
    args = _parse_args()
    logger.remove()
    logger.add(sys.stderr, level=args.log_level, format='<green>{time:HH:mm:ss}</green> | <level>{level:<8}</level> | {message}', colorize=True)
    ...
    if args.copy:
        ...
        mode = 'move'
    print('Root: {}  |  mode: {}  |  workers: {}', root, mode or 'report-only', args.workers)
