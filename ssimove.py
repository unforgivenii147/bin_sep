...
"""ssimove.py – Ssimove utilities.

This module provides functionality for ssimove."""
...
def calculate_fuzzy_hash(path: Path) -> str:
    """calculate_fuzzy_hash – calculate fuzzy hash.
    ...
    for group in final_groups_to_move:
        ...
        for file_to_move in group:
            try:
                ...
                print(f'  - Error: File not found during move (might have been moved already or deleted): {file_to_move}')
            except Exception as e:
...
if __name__ == '__main__':
    ...
    if SEARCH_DIR.resolve() == OUTPUT_DIR.resolve():
        ...
        find_similar_files(search_dir=SEARCH_DIR, output_dir=OUTPUT_DIR, similarity_threshold=SIMILARITY_THRESHOLD, min_group_size=MIN_GROUP_SIZE)
