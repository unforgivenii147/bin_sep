...
"""jsonvalidator.py – Jsonvalidator utilities.

This module provides functionality for jsonvalidator."""
...
def assert_has_typed_keys(path: str, data: dict, keys: dict[str, T.Any]) -> dict:
    """assert_has_typed_keys – assert has typed keys.
...
def validate_base_obj(path: str, name: str, obj: dict) -> None:
    """validate_base_obj – validate base obj.

Args:
    path: Description of path.
    name: Description of name.
    obj: Description of obj."""
    expected: dict[str, T.Any] = {'name': str, 'description': str, 'since': (str, T_None), 'deprecated': (str, T_None), 'notes': list, 'warnings': list}
...
Args:
    ...
    expected: dict[str, T.Any] = {'type': list, 'type_str': str, 'required': bool, 'default': (str, T_None), 'min_varargs': (int, T_None), 'max_varargs': (int, T_None)}
...
Args:
    ...
    expected: dict[str, T.Any] = {'returns': list, 'returns_str': str, 'example': (str, T_None), 'posargs': dict, 'optargs': dict, 'kwargs': dict, 'varargs': (dict, T_None), 'arg_flattening': bool}
...
def validate_object(path: str, name: str, obj: dict) -> None:
    """validate_object – validate object.
...
Args:
    ...
    expected: dict[str, T.Any] = {'example': (str, T_None), 'object_type': str, 'methods': dict, 'is_container': bool, 'extends': (str, T_None), 'returned_by': list, 'extended_by': list, 'defined_by_module': (str, T_None)}
    cur = assert_has_typed_keys(f'{path}.{name}', obj, expected)
...
Returns:
    ...
    expected: dict[str, T.Any] = {'version_major': int, 'version_minor': int, 'meson_version': str, 'functions': dict, 'objects': dict, 'objects_by_type': dict}
