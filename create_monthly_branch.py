...
"""create_monthly_branch.py – Create Monthly Branch utilities.

This module provides functionality for create monthly branch."""
...
def get_branch_name() -> str:
    """get_branch_name – get branch name.
...
Returns:
    ...
    try:
        ...
        if branch_name in result.stdout:
            return True
        result = subprocess.run(['git', 'ls-remote', '--heads', 'origin', branch_name], cwd=REPO_PATH, capture_output=True, text=True)
        return f'refs/heads/{branch_name}' in result.stdout
    except Exception:
        return False

def create_monthly_branch() -> bool:
    """create_monthly_branch – create monthly branch.
