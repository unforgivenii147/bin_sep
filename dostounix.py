...
"""dostounix.py – Dostounix utilities.

This module provides functionality for dostounix."""
...
def _iter_files(paths: Path | str) -> Iterator[Any]:
    """_iter_files –  iter files.
...
def _run_parallel(files: list[Path]) -> tuple[int, int, list[str]]:
    """_run_parallel –  run parallel.

Args:
    files: Description of files.

Returns:
    tuple[int, int, list[str]]: Description of return value."""
    changed = 0
    errors: list[str] = []
...
Returns:
    ...
    import argparse
    parser = argparse.ArgumentParser(description='Convert DOS/Windows (CRLF) files to Unix (LF) format in-place.', epilog='If no paths are given, processes all files in the current directory recursively.')
    parser.add_argument('paths', nargs='*', metavar='FILE_OR_DIR', help='Files or directories to process (recursive for directories).')
    parser.add_argument('-q', '--quiet', action='store_true', help='Suppress per-file output; only show summary.')
    parser.add_argument('-n', '--dry-run', action='store_true', help='Show what would be done without modifying files.')
