...
"""usrpack.py – Usrpack utilities.

This module provides functionality for usrpack."""
...
logging.basicConfig(level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s', handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler('wheel_repack.log', mode='w', encoding='utf-8')])

def verify_and_get_files(dist_info_dir: Path | str) -> Any:
    """verify_and_get_files – verify and get files.
...
def build_wheel_worker(args: str) -> bool:
    """build_wheel_worker – build wheel worker.
