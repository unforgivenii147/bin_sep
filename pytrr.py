...
"""pytrr.py – Pytrr utilities.

This module provides functionality for pytrr."""
...
def create_archive_streaming_optimized() -> None:
    """create_archive_streaming_optimized – create archive streaming optimized."""
    ...
    try:
        ...
        print('Compressing...')
        with open(archive_path, 'wb') as f_out, compressor.stream_writer(f_out) as zstd_writer, tarfile.open(fileobj=zstd_writer, mode='w|') as tar:
...
def verify_archive(archive_path: Path | str) -> None:
...
Args:
    ...
    try:
        ...
        with open(archive_path, 'rb') as f, decompressor.stream_reader(f) as zstd_reader, tarfile.open(fileobj=zstd_reader, mode='r|') as tar:
            for member in tar:
                print(f"{member.name} ({('dir' if member.isdir() else 'file')}, size: {member.size}, mode: {oct(member.mode)})")
    except Exception as e:
