...
"""ssdip.py – Ssdip utilities.

This module provides functionality for ssdip."""
...
def find_fuzzy_duplicates(threshold: int=70) -> None:
    """find_fuzzy_duplicates – find fuzzy duplicates.
    ...
    for i, f1_path in enumerate(processed_files):
        ...
        for j in range(i + 1, len(processed_files)):
            ...
            if comparison_score >= threshold:
                print(f'Potential match: {f1_path.relative_to(start_dir)} and {f2_path.relative_to(start_dir)} (Score: {comparison_score})')
