...
"""srtshift.py – Srtshift utilities.

This module provides functionality for srtshift."""
from __future__ import annotations
import multiprocessing as mp
import re
import sys
from pathlib import Path
TIMESTAMP_RE = re.compile('(\\d{2,3}:\\d{2}:\\d{2},\\d{3})\\s*-->\\s*(\\d{2,3}:\\d{2}:\\d{2},\\d{3})')
TIME_PART_RE = re.compile('(\\d{2,3}):(\\d{2}):(\\d{2}),(\\d{3})')
ENCODINGS = ['utf-8-sig', 'utf-8', 'cp1252', 'latin1']

def ms_to_time(total_ms: int) -> str:
    """ms_to_time – ms to time.

Args:
    total_ms: Description of total_ms.
...
def process_timestamp_line(line: str, shift_ms: int) -> str:
    """process_timestamp_line – process timestamp line.
...
Args:
    ...
    try:
        with open(path, 'r', encoding=enc, errors='replace', newline='') as fin, open(temp_path, 'w', encoding=enc, newline='') as fout:
            for line in fin:
                if '-->' in line:
...
def discover_srt_files(paths: list[Path]) -> list[Path]:
    """discover_srt_files – discover srt files.
...
Returns:
    tuple[list[Path], int]: Description of return value."""
    if len(sys.argv) < 2:
