...
"""neg.py – Neg utilities.

This module provides functionality for neg."""
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Convert images to negative (invert colors) recursively in place', formatter_class=argparse.RawDescriptionHelpFormatter, epilog='\nExamples:\n  %(prog)s ./photos                 # Invert all images recursively in ./photos\n  %(prog)s ./images -n 4            # Use 4 parallel processes\n  %(prog)s ./images --dry-run       # Preview what would be processed\n  %(prog)s ./images --no-recursive  # Only process current directory\n        ')
    parser.add_argument('directory', type=str, help='Directory containing images to invert')
    parser.add_argument('-n', '--processes', type=int, default=None, help='Number of parallel processes (default: CPU count)')
    parser.add_argument('--no-recursive', action='store_true', help='Do not process subdirectories recursively')
    parser.add_argument('--dry-run', action='store_true', help='Show what would be processed without actually modifying files')
    parser.add_argument('--extensions', type=str, nargs='+', help='Additional file extensions to process (e.g., .jpg .png)')
    ...
    print(f"Scanning {('recursively' if recursive else 'non-recursively')} in: {root_dir}")
