...
Returns:
    ...
    parser = argparse.ArgumentParser(description='Convert JSONL/NDJSON files to JSON arrays, streaming one record at a time. With no paths, recursively process JSONL files below the current directory.')
    parser.add_argument('inputs', nargs='*', type=Path, help='Input JSONL/NDJSON files and/or directories.')
    parser.add_argument('-o', '--output-dir', type=Path, help='Write converted files below this directory. Directory inputs preserve their relative structure. Defaults to writing beside each source file.')
    parser.add_argument('--indent', type=int, default=None, help='Pretty-print JSON using this indentation level.')
    parser.add_argument('--ensure-ascii', action='store_true', help='Escape non-ASCII characters in the generated JSON.')
    parser.add_argument('--skip-invalid', action='store_true', help='Skip malformed non-empty JSONL lines instead of failing the entire file.')
    parser.add_argument('--overwrite', action='store_true', help='Replace an existing output .json file.')
    return parser.parse_args()

def is_jsonl_file(path: Path) -> bool:
    """is_jsonl_file – is jsonl file.
...
def iter_jsonl_files(inputs: Iterable[Path]) -> Iterator[Path]:
    ...
    for input_path in inputs:
        ...
        if path.is_dir():
            ...
            continue
        print(f'warning: skipping unsupported input (expected file or directory): {path}', file=sys.stderr)

def destination_for(source: Path, *, output_dir: Path | None, input_roots: tuple[Path, ...]) -> Path:
    """
...
def encode_record(value: object, *, indent: int | None, ensure_ascii: bool) -> str:
    """encode_record – encode record.

Args:
    value: Description of value.
    indent: Description of indent.
    ensure_ascii: Description of ensure_ascii.

Returns:
    str: Description of return value."""
    return json.dumps(value, ensure_ascii=ensure_ascii, indent=indent, separators=None if indent is not None else (',', ':'))
...
def convert_one(job: Job) -> Result:
    ...
    if destination.exists() and (not job.overwrite):
        return Result(source=source, destination=destination, records_written=0, blank_lines=0, invalid_lines=0, error=f'destination exists (use --overwrite): {destination}')
    ...
    try:
        ...
        file_descriptor, temp_name = tempfile.mkstemp(prefix=f'.{destination.name}.', suffix='.tmp', dir=destination.parent, text=True)
        temp_path = Path(temp_name)
        with os.fdopen(file_descriptor, 'w', encoding='utf-8', newline='\n') as output, source.open('r', encoding='utf-8-sig', newline=None) as input_file:
            output.write('[')
            for line_number, line in enumerate(input_file, start=1):
                ...
                if job.indent is not None:
                    output.write('\n')
                output.write(encode_record(record, indent=job.indent, ensure_ascii=job.ensure_ascii))
                records_written += 1
        ...
        temp_path = None
        return Result(source=source, destination=destination, records_written=records_written, blank_lines=blank_lines, invalid_lines=invalid_lines)
    except (OSError, UnicodeError, ValueError) as exc:
        return Result(source=source, destination=destination, records_written=records_written, blank_lines=blank_lines, invalid_lines=invalid_lines, error=str(exc))
    finally:
...
Returns:
    Iterator[Job]: Description of return value."""
        destinations: set[Path] = set()
        for source in sources:
            destination = destination_for(source, output_dir=args.output_dir, input_roots=input_roots).resolve()
            if destination in destinations:
                print(f'warning: skipping {source}; output collision at {destination}', file=sys.stderr)
                continue
            destinations.add(destination)
            yield Job(source=source, destination=destination, indent=args.indent, ensure_ascii=args.ensure_ascii, skip_invalid=args.skip_invalid, overwrite=args.overwrite)
    completed = 0
    failed = 0
    records = 0
    skipped_invalid = 0
    with mp.Pool(processes=WORKERS) as pool:
        for result in pool.imap_unordered(convert_one, jobs(), chunksize=1):
            ...
            if result.error:
                failed += 1
                print(f'FAILED  {result.source} -> {result.destination}\n        {result.error}', file=sys.stderr)
            else:
    ...
    print(f'\nFinished: {completed} file(s), {records} record(s), {skipped_invalid} invalid line(s) skipped, {failed} failure(s).', file=sys.stderr)
