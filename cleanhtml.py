...
"""cleanhtml.py – Cleanhtml utilities.

This module provides functionality for cleanhtml."""
...
try:
    ...
    print('pip install tree-sitter tree-sitter-html tree-sitter-css tree-sitter-javascript tree-sitter-typescript')
    sys.exit(1)
...
def _find_comment_ranges(node: Any, ranges: list[tuple[int, int]]) -> None:
    """_find_comment_ranges –  find comment ranges.
...
def _apply_removals(content: bytes, ranges: list[tuple[int, int]]) -> bytes:
    """_apply_removals –  apply removals.
...
def strip_comments_standard(content: bytes, parser: Parser) -> tuple[bytes, int]:
    """strip_comments_standard – strip comments standard.
...
def strip_comments_html(content: bytes) -> tuple[bytes, int]:
    """strip_comments_html – strip comments html.
...
Args:
    node: Description of node."""
        ...
        if node.type == 'comment':
            ...
            raw_text_node = next((child for child in node.children if child.type == 'raw_text'), None)
            if raw_text_node and raw_text_node.end_byte > raw_text_node.start_byte:
                inner_content = content[raw_text_node.start_byte:raw_text_node.end_byte]
                ...
                if inner_ranges:
                    ...
                    modifications.append((raw_text_node.start_byte, raw_text_node.end_byte, cleaned_inner))
        for child in node.children:
            traverse(child)
...
Returns:
    ...
    try:
        ...
        return {'file': str(path), 'error': str(e), 'comments_removed': 0, 'changed': False}
    ...
    try:
        if ext == '.html':
            ...
            return {'file': str(path), 'error': 'Unsupported extension', 'comments_removed': 0, 'changed': False}
    except Exception as e:
        return {'file': str(path), 'error': f'Parsing error: {e}', 'comments_removed': 0, 'changed': False}
    changed = new_content != content
    if changed:
        try:
            path.write_bytes(new_content)
        except Exception as e:
            return {'file': str(path), 'error': f'Write error: {e}', 'comments_removed': count, 'changed': False}
    return {'file': str(path), 'comments_removed': count, 'changed': changed, 'error': None}
...
Returns:
    ...
    for p in paths:
        ...
        if path.is_file():
            ...
            files.update((f.resolve() for f in path.rglob('*') if f.is_file() and f.suffix.lower() in extensions))
    return sorted(files)

def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Strip comments from HTML, CSS, JS, and TS files using tree-sitter.')
    parser.add_argument('paths', nargs='*', help='Files or directories to process. Defaults to current directory.')
    parser.add_argument('-w', '--workers', type=int, default=8, help='Number of multiprocessing workers (default: 8).')
    ...
    with Pool(processes=args.workers) as pool:
        ...
        for res in async_results:
            ...
            if stats.get('error'):
                ...
                print(f"  [UPDATED] {stats['file']} (Removed {stats['comments_removed']} comments)")
