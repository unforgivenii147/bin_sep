...
"""pymhtml.py – Pymhtml utilities.

This module provides functionality for pymhtml."""
...
Args:
    ...
    html_text = re.sub('(src|href)=[\\\\\\"\']cid:([^\\\\\\"\']+)[\\\\\\"\']', lambda m: f'{m.group(1)}="{out_dir.name}/{cid_to_file.get(m.group(2), m.group(2))}"' if m.group(2) in cid_to_file else m.group(0), html_text, flags=re.IGNORECASE)
