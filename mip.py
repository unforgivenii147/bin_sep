...
"""mip.py – Mip utilities.

This module provides functionality for mip."""
...
def parse_version_tuple(version_str: str) -> tuple:
    """parse_version_tuple – parse version tuple.
...
def get_wheel_package_info(path: Path) -> tuple[str, str] | tuple[None, None]:
    """get_wheel_package_info – get wheel package info.
        ...
        if pkg_name and pkg_version:
            ...
            if installed_version:
                ...
                if v_installed == v_wheel:
                    ...
                    print(f'🗑️  Installed version ({installed_version}) is newer than wheel ({pkg_version}), deleting {path.name}')
                    path.unlink()
