...
"""rpack.py – Rpack utilities.

This module provides functionality for rpack."""
...
def list_installed_packages(site: Path) -> Any:
...
Args:
    ...
    candidates = [site / pkg, site / f'{pkg}.py', site / f"{pkg.replace('-', '_')}", site / f"{pkg.replace('-', '_')}.py"]
    for c in candidates:
        if c.exists():
            if c.is_dir():
...
Args:
    pkg: Description of pkg.
    site: Description of site.
    out_repack: Description of out_repack.
    out_whl: Description of out_whl."""
    pkg_lower = pkg.lower()
