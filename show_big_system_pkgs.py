...
"""show_big_system_pkgs.py – Show Big System Pkgs utilities.

This module provides functionality for show big system pkgs."""
...
Args:
    size_str: Description of size_str.
...
def get_package_info(package: Any) -> Any:
...
def process_packages_parallel(packages: Any, threshold_bytes: int, num_processes: int | None=None) -> Any:
    """process_packages_parallel – process packages parallel.

Args:
    packages: Description of packages.
    threshold_bytes: Description of threshold_bytes.
...
def save_json_results(data: str, filename: str, threshold_mb: float, include_all: bool=False) -> bool:
    """save_json_results – save json results.

Args:
    data: Description of data.
    filename: Description of filename.
    threshold_mb: Description of threshold_mb.
    include_all: Description of include_all.

Returns:
    bool: Description of return value."""
    output = {'metadata': {'threshold_mb': threshold_mb, 'threshold_bytes': threshold_mb * 1024 * 1024, 'scan_date': datetime.now().isoformat(), 'total_packages': len(data.get('all_packages', {})), 'packages_above_threshold': len(data.get('large_packages', {}))}, 'packages': data.get('large_packages', {})}
    if include_all:
