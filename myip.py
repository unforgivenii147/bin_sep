...
"""myip.py – Myip utilities.

This module provides functionality for myip."""
...
class IPAddressManager:
    ...
    def parse_ip_addr_output(self, output: str) -> dict[str, str]:
        """parse_ip_addr_output – parse ip addr output.
...
Returns:
    dict[str, str]: Description of return value."""
        ...
        for line in output.split('\n'):
            iface_match = re.match('^\\d+:\\s+([^:@]+)(?:@[^:]+)?:', line)
            if iface_match:
                current_iface = iface_match.group(1)
                current_is_up = 'UP' in line
                continue
            if current_iface and current_is_up:
                ip_match = re.search('inet\\s+(\\d+\\.\\d+\\.\\d+\\.\\d+)/\\d+', line)
                if ip_match:
...
Args:
    specific_iface: Description of specific_iface.
...
Returns:
    LocationInfo: Description of return value."""
        ...
        try:
            ...
            if data.get('status') == 'success':
                return LocationInfo(country=data.get('country', ''), region=data.get('regionName', ''), city=data.get('city', ''), isp=data.get('isp', ''), lat=data.get('lat', 0.0), lon=data.get('lon', 0.0))
...
Returns:
    ...
    def display_ip_info(self, ip: str, show_location: bool, interface: str='') -> None:
        """display_ip_info – display ip info.

Args:
    ip: Description of ip.
    show_location: Description of show_location.
