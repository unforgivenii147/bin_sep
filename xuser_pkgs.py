...
"""xuser_pkgs.py – Xuser Pkgs utilities.

This module provides functionality for xuser pkgs."""
...
def get_user_site_path() -> Path:
    """get_user_site_path – get user site path.
...
def get_packages_with_entry_points() -> list[str]:
    """get_packages_with_entry_points – get packages with entry points.
...
def get_matching_packages(pattern: str) -> list[str]:
    """get_matching_packages – get matching packages.
...
def resolve_package_list(patterns: list[str], entry_points_only: bool=False) -> tuple[list[str], list[str]]:
    """resolve_package_list – resolve package list.

Args:
    patterns: Description of patterns.
    entry_points_only: Description of entry_points_only.
...
Returns:
    ...
    if entry_points_only:
        ...
        for pattern in patterns:
            ...
            elif pattern in packages_with_eps:
...
def copy_single_file(record_row: list[str], dist_location: Path, target_dir: Path) -> bool:
    """copy_single_file – copy single file.

Args:
    record_row: Description of record_row.
...
Args:
    ...
    user_site: Description of user_site.
    base_target_dir: Description of base_target_dir.
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Extract installed user-site Python packages that have entry points to ~/tmp/pkgs/<pkgname>', epilog='Examples:\n  python script.py requests              # Extract \'requests\' if it has entry points\n  python script.py "req*"                 # Extract packages starting with "req" that have entry points\n  python script.py -e                     # Extract ALL packages with entry points\n  python script.py -e "django*"           # Extract django packages with entry points\n  python script.py -e --list-only         # List all packages with entry points', formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument('patterns', nargs='*', help='Package names or wildcard patterns (e.g., "a*" for all packages starting with "a")')
    parser.add_argument('-e', '--entry-points', action='store_true', help='Extract only packages that have entry points (console scripts, plugins, etc.)')
    parser.add_argument('--list-only', action='store_true', help='Only list matching packages without extracting')
    parser.add_argument('--show-entry-points', action='store_true', help='Show detailed entry point information when listing')
    ...
    matched_packages, unmatched_patterns = resolve_package_list(args.patterns if args.patterns else [], args.entry_points)
    seen = set()
    ...
    with concurrent.futures.ThreadPoolExecutor() as pkg_executor:
        future_to_pkg = {pkg_executor.submit(process_package, pkg, user_site, base_target_dir): pkg for pkg in packages_to_extract}
