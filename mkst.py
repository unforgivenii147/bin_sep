...
IMAGE_EXTENSIONS: set[str] = {'.png', '.jpg', '.jpeg', '.gif', '.svg', '.webp', '.ico', '.avif', '.bmp', '.tiff'}
CSS_URL_PATTERN: re.Pattern[str] = re.compile('url\\((["\\\']?)([^)"\\\']+)\\1\\)')
TIMEOUT: int = 10
POOL_SIZE: int = 8
Stats = dict[str, Any]
...
def process_html_file(path: Path) -> Stats:
    ...
    stats: Stats = {'path': str(path), 'local': 0, 'remote': 0, 'time': 0.0, 'status': 'success'}
...
def process_file(path: Path) -> Stats:
    ...
    return {'path': str(path), 'local': 0, 'remote': 0, 'time': 0.0, 'status': 'skipped'}
...
def main() -> None:
    ...
    with Pool(processes=POOL_SIZE) as pool:
        async_results: list[tuple[AsyncResult[Stats], Path]] = [(pool.apply_async(process_file, (p,)), p) for p in targets]
        for ar, p in async_results:
            ...
            status = s['status']
            if status == 'success':
                print(f"[SUCCESS] {display_path} ({s['time']:.2f}s) - Embedded: {s['local']} local, {s['remote']} remote")
            elif status == 'skipped':
                pass
            else:
