...
"""type_hinter.py – Type Hinter utilities.

This module provides functionality for type hinter."""
...
Args:
    node: Description of node."""
        ...
        if node.params.star_arg and node.params.star_arg.annotation:
            param_types.append(cst.Module([]).code_for_node(node.params.star_arg.annotation))
        if node.params.star_kwarg and node.params.star_kwarg.annotation:
            param_types.append(cst.Module([]).code_for_node(node.params.star_kwarg.annotation))
...
Args:
    type_map: Description of type_map."""
...
Returns:
    cst.FunctionDef: Description of return value."""
        ...
        if updated_node.returns is None:
            updated_node = updated_node.with_changes(returns=cst.Annotation(annotation=cst.parse_expression(ret_type)))
        params = updated_node.params
        new_params_list = []
        for i, param in enumerate(params.params):
            if i < len(param_types) and param.annotation is None:
                new_param = param.with_changes(annotation=cst.Annotation(annotation=cst.parse_expression(param_types[i])))
                new_params_list.append(new_param)
            else:
                new_params_list.append(param)
        updated_node = updated_node.with_changes(params=params.with_changes(params=new_params_list))
        return updated_node

def run_stubgen(path: str) -> str:
    """run_stubgen – run stubgen.
