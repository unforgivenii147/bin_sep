...
"""find_dup_func_class_const.py – Find Dup Func Class Const utilities.

This module provides functionality for find dup func class const."""
...
def parse_file_definitions(path: Path) -> dict:
    """parse_file_definitions – parse file definitions.
...
def modify_affected_file(path_str: str, obj_name: str, obj_type: str, raw_obj_code: str) -> str:
    """modify_affected_file – modify affected file.

Args:
    path_str: Description of path_str.
    obj_name: Description of obj_name.
    obj_type: Description of obj_type.
    raw_obj_code: Description of raw_obj_code.
...
Returns:
    ...
    removed = False
    for node in tree.body:
        if obj_type in ('function', 'class') and isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Recursively find and consolidate duplicate functions, classes, and constants.')
    parser.add_argument('-m', '--move', action='store_true', help='Consolidate duplicate code blocks into dh.py and update files with required imports.')
