...
"""fztrans.py – Fztrans utilities.

This module provides functionality for fztrans."""
...
Returns:
    tuple[dict[str, str], dict[str, str]]: Description of return value."""
    if not path.exists():
...
Args:
    ...
    fa_en: Description of fa_en.
    en_fa: Description of en_fa.
...
Args:
    word: Description of word.
    all_words: Description of all_words.
    limit: Description of limit.
    cutoff: Description of cutoff.
...
Returns:
    ...
    if not shutil.which('fzf'):
        ...
        return None
    try:
        proc = subprocess.run(['fzf', '--prompt=Select word: ', '--height=40%', '--layout=reverse', '--border'], input='\n'.join(sorted(all_words)), text=True, capture_output=True, check=False)
