...
"""ex_tar.py – Ex Tar utilities.

This module provides functionality for ex tar."""
...
def extract_zst_file(archive_path: Path | str, extract_path: Path | str) -> list[Path]:
    """extract_zst_file – extract zst file.

Args:
    archive_path: Description of archive_path.
    extract_path: Description of extract_path."""
...
def extract_tar_zst(archive_path: Path | str, extract_path: Path | str) -> None:
    """extract_tar_zst – extract tar zst.
...
def extract_tar_xz(archive_path: Path | str, extract_path: Path | str) -> None:
    """extract_tar_xz – extract tar xz.
...
def process_archive(archive_path: Path, dry_run: bool=False, keep_original: bool=False, quiet: bool=False) -> Any:
    """process_archive – process archive.

Args:
    archive_path: Description of archive_path.
    dry_run: Description of dry_run.
    keep_original: Description of keep_original.
...
Returns:
    int: Description of return value."""
    parser = argparse.ArgumentParser(description='Extract .zst, .tar.zst, and .tar.xz archives.\nIf a filename is provided, process only that file.\nIf no argument, recursively search current directory.', formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument('target', nargs='?', default=None, help='File to extract or directory to search (default: current directory)')
    parser.add_argument('--dry-run', '-n', action='store_true', help='Show what would be done without actually extracting')
    parser.add_argument('--keep-original', '-k', action='store_true', help='Keep original archive files after extraction')
