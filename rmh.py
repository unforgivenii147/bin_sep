...
"""rmh.py – Rmh utilities.

This module provides functionality for rmh."""
...
class CommentRemover:
    ...
    def _protect_strings(self, text: str) -> tuple[str, dict]:
        """_protect_strings –  protect strings.

Args:
    text: Description of text.

Returns:
    tuple[str, dict]: Description of return value."""
        protected_strings = {}
        placeholder_counter = [0]

        def replace_string(match: Any) -> Any:
...
Args:
    ...
    def _restore_strings(self, text: str, protected_strings: dict) -> str:
        """_restore_strings –  restore strings.

Args:
    text: Description of text.
    protected_strings: Description of protected_strings.
...
Returns:
    ProcessResult: Description of return value."""
        try:
            ...
            return ProcessResult(path=path, success=True, original_lines=original_lines, final_lines=final_lines, comments_removed=comments_removed, original_size=original_size, final_size=final_size, backup_size=backup_size)
        except UnicodeDecodeError as e:
            return ProcessResult(path=path, success=False, original_lines=0, final_lines=0, comments_removed=0, original_size=0, final_size=0, backup_size=0, error=f'Encoding error: {e}')
        except Exception as e:
            return ProcessResult(path=path, success=False, original_lines=0, final_lines=0, comments_removed=0, original_size=0, final_size=0, backup_size=0, error=f'Processing error: {e}')
...
def collect_source_files(targets: list[str]) -> list[Path]:
    """collect_source_files – collect source files.
...
Returns:
    ...
    with Pool(num_workers) as pool:
        results = list(tqdm(pool.imap_unordered(remover.process_file, paths), total=len(paths), desc='Processing files', unit='file'))
    return results

def _format_bytes(bytes_val: int) -> str:
    """_format_bytes –  format bytes.
...
Args:
    ...
    if successful:
        ...
        for result in successful:
            print(f'  {result.path.name}: {result.comments_removed} comments, {result.original_lines - result.final_lines} lines removed, freed {_format_bytes(result.space_freed)}')
    if failed:
        logger.warning('Failed files:')
        for result in failed:
            logger.warning(f'  {result.path.name}: {result.error}')

def main(targets: list[str] | None=None, num_workers: int | None=None, keep_backups: bool=True, dry_run: bool=False) -> int:
    """main – main.

Args:
    targets: Description of targets.
    num_workers: Description of num_workers.
    keep_backups: Description of keep_backups.
