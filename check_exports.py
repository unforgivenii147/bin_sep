...
Returns:
    ...
    parser.add_argument('-a', '--autofix', action='store_true', help='Automatically add missing definitions to __init__.py')
    parser.add_argument('--dry-run', action='store_true', help='With -a, show what would be changed without writing')
...
def main() -> None:
    ...
    args = parse_args()
    logger.remove()
    logger.add(lambda msg: print(msg, end=''), format='<level>{level: <8}</level> | <level>{message}</level>', level='DEBUG' if args.verbose else 'INFO', colorize=True)
