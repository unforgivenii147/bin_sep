...
"""compare_dirs.py – Compare Dirs utilities.

This module provides functionality for compare dirs."""
...
def write_shell_copy(script_path: Path, src_root: Path, dst_root: Path, only_dirs: Path | str, only_files: Path | str) -> None:
    """write_shell_copy – write shell copy.

Args:
    script_path: Description of script_path.
    src_root: Description of src_root.
    dst_root: Description of dst_root.
    only_dirs: Description of only_dirs.
    only_files: Description of only_files."""
    with script_path.open('w', encoding='utf-8') as sh:
        ...
        for f in sorted(only_files):
            ...
            sh.write(f'mkdir -p {shlex.quote(str(parent))} && cp -a {shlex.quote(str(src_file))} {shlex.quote(str(dst_file))}\n')
    st = script_path.stat()
