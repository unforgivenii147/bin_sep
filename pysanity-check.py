...
"""pysanity-check.py – Pysanity Check utilities.

This module provides functionality for pysanity check."""
...
def _normalize_name(name: str) -> str:
    """_normalize_name –  normalize name.
...
Returns:
    ...
    return re_sub('[-_.]+', '-', name).lower()

def get_installed_python_packages() -> list[tuple[str, str]]:
    """get_installed_python_packages – get installed python packages.
...
def check_package_importable(package_name: str) -> tuple[bool, str]:
    """check_package_importable – check package importable.
...
Returns:
    str: Description of return value."""
    try:
        result = subprocess.run([sys.executable, '-m', 'pip', 'install', f'{package_name}==', '--dry-run'], capture_output=True, text=True, check=True)
