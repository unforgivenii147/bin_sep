...
"""ocrgrid2.py – Ocrgrid2 utilities.

This module provides functionality for ocrgrid2."""
...
def prepare_image_for_ocr(img_path: Path) -> Any:
    """prepare_image_for_ocr – prepare image for ocr.
...
def run_tesseract_on_image(img: Any, oem: int, psm: int) -> tuple[bytes | dict[str, bytes | str] | str, str, float, str] | tuple[str, str, float, str]:
    """run_tesseract_on_image – run tesseract on image.

Args:
    img: Description of img.
    oem: Description of oem.
    psm: Description of psm.

Returns:
    tuple[bytes | dict[str, bytes | str] | str, str, float, str] | tuple[str, str, float, str]: Description of return value."""
...
def main() -> None:
    ...
    for img_path in image_files:
        ...
        for oem, psm in itertools.product(OEM_OPTIONS, PSM_OPTIONS):
            ...
            result = {'image': img_path.name, 'config': config, 'oem': oem, 'psm': psm, 'duration_sec': duration, 'error': error, 'text': text}
