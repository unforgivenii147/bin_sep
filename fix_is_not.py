...
"""fix_is_not.py – Fix Is Not utilities.

This module provides functionality for fix is not."""
...
Returns:
    ...
    result = {'path': path, 'found_count': 0, 'fixed': False, 'lines': [], 'error': None}
    try:
    ...
    while i < n:
        tok = tokens[i]
        if i + 1 < n and tok.type == tokenize.NAME and (tok.string == 'is') and (tokens[i + 1].type == tokenize.NAME) and (tokens[i + 1].string == 'not'):
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description="Recursively find and optionally replace 'is not' with '!=' in Python files.")
    parser.add_argument('-a', '--auto-fix', action='store_true', help="Automatically replace 'is not' with '!=' across files.")
