...
"""checkforupdate.py – Checkforupdate utilities.

This module provides functionality for checkforupdate."""
...
Args:
    ...
    def save_state(self) -> None:
        ...
        state = {'last_updated': datetime.now().isoformat(), 'processed_packages': self.processed_packages}
        try:
...
Returns:
        ...
        for name in names_to_try:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    return {'latest_version': latest_version, 'download_url': download_url, 'pypi_name': name}
    ...
    def write_updates_to_file(self, updates: dict[str, dict]) -> None:
        """write_updates_to_file – write updates to file.
...
Args:
    ...
    def check_updates(self) -> bool:
        ...
        packages_to_check = {name: ver for name, ver in installed_packages.items() if name not in self.processed_packages}
            ...
            for i, (pkg_name, current_version) in enumerate(sorted(packages_to_check.items()), 1):
                ...
                try:
                    ...
                    if latest_info:
                        ...
                        self.processed_packages[pkg_name] = {'current_version': current_version, 'latest_version': latest_version, 'download_url': latest_info.get('download_url'), 'needs_update': needs_update, 'checked_at': datetime.now().isoformat()}
                        if needs_update:
                            print(f'    ⬆️  Update available: {current_version} → {latest_version}')
                        else:
                            print(f'    ✓ Up to date ({current_version})')
                    else:
                        self.processed_packages[pkg_name] = {'current_version': current_version, 'latest_version': None, 'download_url': None, 'needs_update': False, 'checked_at': datetime.now().isoformat(), 'error': 'Package not found on PyPI'}
                        print(f'    ⚠️  Not found on PyPI')
                except Exception as e:
                    print(f'    ❌ Error checking {pkg_name}: {e}')
                    self.processed_packages[pkg_name] = {'current_version': current_version, 'latest_version': None, 'download_url': None, 'needs_update': False, 'checked_at': datetime.now().isoformat(), 'error': str(e)}
                if i % 20 == 0:
                    self.save_state()
