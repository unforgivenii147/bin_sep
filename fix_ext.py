...
"""fix_ext.py – Fix Ext utilities.

This module provides functionality for fix ext."""
...
def get_extension_from_mime(mime_type: str) -> str:
    """get_extension_from_mime – get extension from mime.
...
Returns:
    ...
    mime_map = {'image/jpeg': '.jpg', 'image/png': '.png', 'image/gif': '.gif', 'image/webp': '.webp', 'image/tiff': '.tiff', 'image/bmp': '.bmp', 'application/pdf': '.pdf', 'application/zip': '.zip', 'application/x-tar': '.tar', 'application/gzip': '.gz', 'application/x-7z-compressed': '.7z', 'application/x-rar': '.rar', 'video/mp4': '.mp4', 'video/x-msvideo': '.avi', 'video/quicktime': '.mov', 'audio/mpeg': '.mp3', 'audio/wav': '.wav', 'text/plain': '.txt', 'application/octet-stream': ''}
    return mime_map.get(mime_type, '')

def detect_true_extension(path: Path) -> str:
    """detect_true_extension – detect true extension.
...
Returns:
    tuple[Path, str, str] | None: Description of return value."""
...
def autofix_filename(path: Path, current_ext: str, true_ext: str) -> Path:
    """autofix_filename – autofix filename.

Args:
    path: Description of path.
    current_ext: Description of current_ext.
    true_ext: Description of true_ext.
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Recursively detect and optionally fix file extension mismatches based on file headers (magic numbers).')
    parser.add_argument('directory', type=str, help='The directory to recursively scan.')
    parser.add_argument('-a', '--autofix', action='store_true', help='Automatically rename files to fix detected mismatches in place.')
    ...
    for path, current_ext, true_ext in sorted(mismatches):
        print(f"[MISMATCH] '{path}' | Current: '{current_ext}' | Detected: '{true_ext}'")
        if args.autofix:
            try:
