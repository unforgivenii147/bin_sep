...
try:
    ...
    HAS_ZSTD = False
SKIP_DIRS = frozenset({'.git', '__pycache__', '.venv', 'node_modules', '.env', '.pytest_cache'})
COMMON_IMPORTS: dict[str, set[str]] = {'typing': {'List', 'Dict', 'Set', 'Tuple', 'Optional', 'Union', 'Any', 'Callable', 'Type', 'Generic', 'TypeVar', 'cast', 'overload', 'Protocol', 'Iterator', 'Iterable', 'Sequence', 'Mapping'}, 'dataclasses': {'dataclass', 'field', 'InitVar', 'FrozenInstanceError', 'MISSING', 'fields', 'asdict', 'astuple', 'make_dataclass', 'replace'}, 'functools': {'lru_cache', 'wraps', 'partial', 'total_ordering', 'reduce', 'cmp_to_key', 'singledispatch'}, 'itertools': {'combinations', 'permutations', 'product', 'chain', 'groupby', 'repeat', 'cycle', 'islice', 'takewhile', 'dropwhile'}, 'pathlib': {'Path', 'PurePath', 'PureWindowsPath', 'PurePosixPath', 'WindowsPath', 'PosixPath'}, 'datetime': {'datetime', 'date', 'time', 'timedelta', 'timezone', 'tzinfo', 'strptime', 'now'}, 'json': {'dumps', 'loads', 'dump', 'load', 'JSONEncoder', 'JSONDecoder'}, 're': {'compile', 'match', 'search', 'findall', 'finditer', 'sub', 'split', 'escape', 'IGNORECASE', 'MULTILINE', 'DOTALL'}, 'collections': {'defaultdict', 'OrderedDict', 'Counter', 'deque', 'namedtuple', 'ChainMap'}, 'enum': {'Enum', 'IntEnum', 'Flag', 'IntFlag', 'auto', 'unique'}, 'abc': {'ABC', 'abstractmethod', 'abstractproperty', 'ABCMeta', 'abstractclassmethod'}, 'contextlib': {'contextmanager', 'closing', 'suppress', 'redirect_stdout', 'redirect_stderr'}, 'copy': {'copy', 'deepcopy'}, 'pickle': {'dumps', 'loads', 'dump', 'load'}, 'logging': {'getLogger', 'debug', 'info', 'warning', 'error', 'critical', 'basicConfig'}, 'os': {'path', 'environ', 'getcwd', 'chdir', 'listdir', 'mkdir', 'makedirs', 'remove', 'rmdir'}, 'sys': {'argv', 'exit', 'stdout', 'stderr', 'stdin', 'path', 'modules'}, 'subprocess': {'run', 'Popen', 'PIPE', 'STDOUT', 'CalledProcessError'}, 'threading': {'Thread', 'Lock', 'RLock', 'Condition', 'Semaphore', 'Event'}, 'asyncio': {'run', 'gather', 'create_task', 'sleep', 'Queue', 'Event', 'Lock'}, 'urllib': {'request', 'parse', 'error'}, 'requests': {'get', 'post', 'put', 'delete', 'Session', 'Response'}, 'numpy': {'array', 'zeros', 'ones', 'arange', 'linspace', 'ndarray'}, 'pandas': {'DataFrame', 'Series', 'read_csv', 'read_excel', 'concat', 'merge'}}
    ...
    def validate_python_code(source: str) -> tuple[bool, str | None]:
        ...
        try:
            ...
            if undefined_names:
                ...
                common_names = {'self', 'cls', 'List', 'Dict', 'Set', 'Tuple', 'Optional', 'Union', 'Any', 'Callable', 'Type', 'Generic', 'TypeVar', 'Path', 'datetime', 'date', 'time', 'timedelta', 'timezone', 'json', 're', 'os', 'sys', 'logging', 'logger', 'loguru'}
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    ...
                    node.decorator_list = [d for d in node.decorator_list if not MethodConverter._is_class_decorator(d)]
...
class ImportAnalyzer:
    ...
    def consolidate_imports(existing: set[str], needed: set[str]) -> list[str]:
        ...
        for imp in sorted(all_imports):
            ...
            elif any((imp.startswith((f'from {mod}', f'import {mod}')) for mod in ['os', 'sys', 'json', 're', 'pathlib', 'datetime', 'asyncio', 'subprocess', 'threading', 'logging'])):
...
class EntityVisitor(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> None:
        ...
        self.entities.append(Entity(name=node.name, type='class', source=source, full_name=node.name, source_file=self.path, line_number=node.lineno, decorators=[self._get_decorator_name(d) for d in node.decorator_list]))
        old_class = self.current_class
        self.current_class = node.name
        for item in node.body:
            if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                self._process_function(item, is_async=isinstance(item, ast.AsyncFunctionDef), in_class=True)
            self.visit(item)
        ...
        if self.current_class is None:
            for target in node.targets:
                if isinstance(target, ast.Name) and self._is_constant_name(target.id):
                    source = self._get_source_slice(node)
                    self.entities.append(Entity(name=target.id, type='constant', source=source, full_name=target.id, source_file=self.path, line_number=node.lineno))
        self.generic_visit(node)

    def _process_function(self, node: ast.FunctionDef | ast.AsyncFunctionDef, is_async: bool=False, in_class: bool=False) -> None:
        ...
        if in_class:
            ...
            entity_type = 'function'
            full_name = node.name
        self.entities.append(Entity(name=node.name, type=entity_type, source=source, full_name=full_name, source_file=self.path, line_number=node.lineno, decorators=[self._get_decorator_name(d) for d in node.decorator_list]))
...
def main() -> int:
    ...
    import argparse
    parser = argparse.ArgumentParser(description='Extract Python code entities from files and archives', formatter_class=argparse.RawDescriptionHelpFormatter, epilog='\nExamples:\n\n  python extract_entities.py\n\n\n  python extract_entities.py -t\n\n\n  python extract_entities.py --workers 16\n        ')
    parser.add_argument('-t', '--temp', action='store_true', help='Save to ~/tmp/output/ instead of ./output/')
    ...
    print(f'Found {len(python_files):,} Python files and {len(archive_members):,} archive members\n')
    all_entities: list[Entity] = []
    all_imports: set[str] = set()
    entity_count: dict[str, int] = {'function': 0, 'class': 0, 'constant': 0}
    error_count = 0
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        futures: dict = {executor.submit(process_file_worker, fpath): ('file', str(fpath)) for fpath in python_files}
        for virtual_path, source in archive_members:
            futures[executor.submit(process_archive_member_worker, (virtual_path, source))] = ('archive', virtual_path)
        processed = 0
