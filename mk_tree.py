...
"""mk_tree.py – Mk Tree utilities.

This module provides functionality for mk tree."""
...
NODE_MARKERS = ('├──', '└──', '|--', '`--')
FOOTER_RE = re.compile('^\\d+\\s+(directories|files|dirs|items)(,\\s*\\d+\\s+(directories|files|dirs|items))?$')
JUNK_LINE_CHARS = set(' │├└─|+-`')
JUNK_TOKEN_RE = re.compile('^[\\u2502\\u251c\\u2514\\u2500|+\\-]+$')
LEAD_JUNK_RE = re.compile('^([\\u2502\\u251c\\u2514\\u2500|+`\\-]{2,}|[\\u2502\\u251c\\u2514\\u2500|+`\\-]+\\s+)')

def strip_leading_junk(left: Any, text: str) -> Any:
    """strip_leading_junk – strip leading junk.
...
def find_node_marker(line: str) -> Any:
    """find_node_marker – find node marker.
...
def clean_name(raw: str, keep_suffix: bool=False) -> Any:
    """clean_name – clean name.

Args:
    raw: Description of raw.
    keep_suffix: Description of keep_suffix."""
    raw = raw.strip()
...
def cluster_means(values: list[Any], min_gap: Any | None=None) -> Any:
    """cluster_means – cluster means.

Args:
    values: Description of values.
    min_gap: Description of min_gap."""
...
def parse_tree_text(text: str, keep_suffix: bool=False, warn: Any=print) -> Any:
    """parse_tree_text – parse tree text.
...
Args:
    ...
    for line_no, idx, raw in candidates:
        ...
        if idx is None:
            ...
            depth = 1
        entries.append({'depth': depth, 'name': name, 'is_dir': explicit, 'explicit_dir': explicit, 'line_no': line_no})
    for w in warnings:
        warn('warning: ' + w)
    return entries

def load_ocr(choice: Any) -> Any:
    """load_ocr – load ocr.
...
Args:
    img: Description of img."""
                ...
                for i, t in enumerate(data['text']):
                    ...
                    if conf < 40:
                        continue
                    tokens.append((int(data['left'][i]), int(data['top'][i]), int(data['width'][i]), t, conf))
                return tokens
            return ('pytesseract', run)
        except ImportError:
            if choice == 'pytesseract':
                sys.exit('error: pytesseract is not installed (pip install pytesseract)')
    sys.exit('error: no OCR engine available — install one with:\n    pip install rapidocr-onnxruntime     (recommended, self-contained)\n  or: pip install pytesseract            (requires the tesseract binary too)')

def detect_marker_xs(img: Any, rows: Any, pitch: Any, char_width: Any) -> Any:
    """detect_marker_xs – detect marker xs.

Args:
    img: Description of img.
    rows: Description of rows.
    pitch: Description of pitch.
    char_width: Description of char_width."""
...
def image_to_entries(path: Path | str, keep_suffix: bool=False, engine: str='auto') -> Any:
    """image_to_entries – image to entries.
...
Args:
    ...
    for top, parts in rows:
        ...
        row_names.append(re.sub('\\s*\\.\\s*', '.', ''.join((p[1] for p in parts))))
    if not cell_widths:
...
Args:
    ...
    for i, (top, left, raw) in enumerate(zip((r[0] for r in rows), row_lefts, row_names, strict=False)):
        ...
        if name is None:
            continue
        entries.append({'depth': depth_of(i), 'name': name, 'is_dir': explicit, 'explicit_dir': explicit, 'line_no': round(top)})
    if not entries:
        sys.exit(f'error: OCR found no usable entries in {path}')
    return (engine_name, entries, {'tokens': raw_count, 'rows': len(rows)})

def finalize(entries: Any, assume_dir: bool=False) -> None:
    """finalize – finalize.

Args:
    entries: Description of entries.
    assume_dir: Description of assume_dir."""
...
def ambiguous_notes(entries: Any, assume_dir: bool=False) -> Any:
    """ambiguous_notes – ambiguous notes.

Args:
    entries: Description of entries.
    assume_dir: Description of assume_dir."""
    notes = []
    ambiguous = [e['name'] for i, e in enumerate(entries) if e['name'] != '.' and (not e['explicit_dir']) and (not (i + 1 < len(entries) and entries[i + 1]['depth'] > e['depth']))]
    if ambiguous:
        n = len(ambiguous)
        noun = 'entry' if n == 1 else 'entries'
        if assume_dir:
            notes.append(f"{n} {noun} had no visible children -> created as director{('y' if n == 1 else 'ies')} (--assume-dir)")
        else:
            notes.append(f"{n} {noun} had no visible children -> created as file{('s' if n != 1 else '')}; re-run with --assume-dir to make them directories")
    return notes

def _unsafe_name(name: str) -> Any:
    """_unsafe_name –  unsafe name.
...
def create_tree(entries: Any, base_dir: Path, dry_run: bool=False) -> Any:
...
Args:
    ...
    try:
        ...
        magics = (b'\x89PNG\r\n\x1a\n', b'\xff\xd8\xff', b'GIF8', b'RIFF', b'BM', b'II*\x00', b'MM\x00*')
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(prog='tree2fs', description='Recreate a directory tree from a `tree` listing (text) or a screenshot of one (OCR).', epilog='examples:\n  tree2fs tree.txt                     # build from a text listing\n  tree2fs screenshot.png               # build from a screenshot (OCR)\n  tree2fs tree.txt --base /tmp/out     # build elsewhere\n  tree2fs tree.txt -d -n               # ambiguous entries as dirs, dry run', formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument('input', type=Path, help='text listing or screenshot image')
    parser.add_argument('--base', type=Path, default=Path.cwd(), help='directory to build the tree in (default: current directory)')
    parser.add_argument('-d', '--assume-dir', action='store_true', help='entries without visible children are created as directories (default: files)')
    parser.add_argument('-n', '--dry-run', action='store_true', help='print the plan without creating anything')
    parser.add_argument('--engine', choices=('auto', 'rapidocr', 'pytesseract'), default='auto', help='OCR engine (default: auto)')
    parser.add_argument('--keep-suffix', action='store_true', help='keep tree -F suffixes (* @ = |) in names')
    ...
    print(f"{what} {counts['dirs']} dir(s), {counts['files']} file(s) ({counts['existing']} already existed, {counts['skipped']} skipped).")
