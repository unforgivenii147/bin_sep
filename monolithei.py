...
"""monolithei.py – Monolithei utilities.

This module provides functionality for monolithei."""
...
class Monolith:
    """Monolith – Monolith."""
    MIME_TYPES = {'.css': 'text/css', '.js': 'application/javascript', '.svg': 'image/svg+xml', '.woff': 'font/woff', '.woff2': 'font/woff2', '.ttf': 'font/ttf', '.eot': 'application/vnd.ms-fontobject', '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.gif': 'image/gif', '.webp': 'image/webp'}

    def __init__(self, ignore_errors: bool=False, no_images: bool=False, timeout: int=10, encoding: str='utf-8') -> None:
        """__init__ –   init  .

Args:
    ignore_errors: Description of ignore_errors.
    no_images: Description of no_images.
    ...
    encoding: Description of encoding."""
        ...
        self.session.headers.update({'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) Monolith/1.0'})
...
Args:
    ...
    def resource_to_data_uri(self, url: str, mime_type: Any | None=None) -> Any:
        """resource_to_data_uri – resource to data uri.
...
Args:
    ...
    def inline_css(self, soup: Any) -> None:
...
Args:
    ...
    def _rebase_css_urls(self, css: Any, base_url: Any) -> Any:
        """_rebase_css_urls –  rebase css urls.

Args:
    css: Description of css.
    base_url: Description of base_url."""

        def replace_url(match: Any) -> Any:
    ...
    match: Description of match."""
            ...
            try:
                ...
                if not self.ignore_errors:
                    raise
                return match.group(0)
        return re.sub('url\\([\\\'"]?([^\\)]+)[\\\'"]?\\)', replace_url, css)

    def inline_scripts(self, soup: Any) -> None:
...
Args:
    ...
    def inline_images(self, soup: Any) -> None:
...
Args:
    ...
    def inline_fonts(self, soup: Any) -> None:
        """inline_fonts – inline fonts.
...
Args:
    ...
    def process_html(self, html_content: str, source_url: Any | None=None) -> Any:
        """process_html – process html.

Args:
    html_content: Description of html_content.
    source_url: Description of source_url."""
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Save webpages as single HTML files with embedded resources', prog='monolith')
    parser.add_argument('source', help='URL or local file path')
    parser.add_argument('-e', '--ignore-errors', action='store_true', help='Ignore unreachable resources (continue on errors)')
    parser.add_argument('-i', '--no-images', action='store_true', help='Strip all images from output')
