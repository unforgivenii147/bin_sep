...
"""exsrt3.py – Exsrt3 utilities.

This module provides functionality for exsrt3."""
...
def get_subtitle_streams_info(input_path: str) -> list[dict]:
    """get_subtitle_streams_info – get subtitle streams info.
    ...
    try:
        ...
        for stream in probe_data.get('streams', []):
            ...
            if stream.get('codec_type') == 'subtitle':
                stream_info = {'index': stream.get('index'), 'language': stream.get('tags', {}).get('language', 'und'), 'title': stream.get('tags', {}).get('title'), 'forced': stream.get('disposition', {}).get('forced', 0) == 1, 'codec_name': stream.get('codec_name')}
                print(stream_info)
