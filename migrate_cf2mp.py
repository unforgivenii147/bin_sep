...
class ExecutorCollector(ast.NodeVisitor):
    ...
    def __init__(self) -> None:
        """__init__ –   init  ."""
        self.executor_vars = set()
        self.future_vars = set()

    def _is_executor_constructor(self, node: Any) -> Any:
...
Args:
    node: Description of node."""
        if not isinstance(node, ast.Call):
            return False
        func = node.func
        if isinstance(func, ast.Name) and func.id in ('ProcessPoolExecutor', 'ThreadPoolExecutor'):
            return True
        return bool(isinstance(func, ast.Attribute) and func.attr in ('ProcessPoolExecutor', 'ThreadPoolExecutor'))
...
Args:
    node: Description of node."""
        ...
        if isinstance(node.value, ast.Call) and isinstance(node.value.func, ast.Attribute) and (node.value.func.attr == 'submit'):
            for target in node.targets:
                if isinstance(target, ast.Name):
...
class FuturesToPoolMigrator(ast.NodeTransformer):
    ...
    def __init__(self, executor_vars: Any, future_vars: Any) -> None:
        """__init__ –   init  .

Args:
    executor_vars: Description of executor_vars.
    future_vars: Description of future_vars."""
        ...
        self.executor_to_pool = {}
        self.pool_counter = 0

    def _get_pool_name(self, executor_var: Any) -> Any:
...
Args:
    ...
    def visit_Import(self, node: Any) -> Any | None:
        ...
        if self._is_executor_constructor(node.value):
            ...
            for target in node.targets:
                if isinstance(target, ast.Name):
                    ...
                    new_assign = ast.Assign(targets=[ast.Name(id=pool_var, ctx=ast.Store())], value=ast.Call(func=ast.Attribute(value=ast.Name(id='mp', ctx=ast.Load()), attr='Pool', ctx=ast.Load()), args=[], keywords=[]))
                    ast.copy_location(new_assign, node)
                    ast.fix_missing_locations(new_assign)
                    return new_assign
        return node

    def visit_With(self, node: Any) -> Any:
        """visit_With – visit With.
...
Args:
    node: Description of node."""
        ...
        if len(executor_items) > 1:
            print('Warning: Multiple executor context managers in one with statement; only first will be migrated.')
        ...
        pool_var = self._get_pool_name(executor_var)
        self.executor_to_pool[executor_var] = pool_var
        pool_assign = ast.Assign(targets=[ast.Name(id=pool_var, ctx=ast.Store())], value=ast.Call(func=ast.Attribute(value=ast.Name(id='mp', ctx=ast.Load()), attr='Pool', ctx=ast.Load()), args=[], keywords=[]))
        ...
        close_call = ast.Expr(value=ast.Call(func=ast.Attribute(value=ast.Name(id=pool_var, ctx=ast.Load()), attr='close', ctx=ast.Load()), args=[], keywords=[]))
        join_call = ast.Expr(value=ast.Call(func=ast.Attribute(value=ast.Name(id=pool_var, ctx=ast.Load()), attr='join', ctx=ast.Load()), args=[], keywords=[]))
        ast.copy_location(close_call, node)
        ast.copy_location(join_call, node)
        try_finally = ast.Try(body=transformed_body, handlers=[], orelse=[], finalbody=[close_call, join_call])
            ...
            if isinstance(base, ast.Name) and base.id in self.executor_vars:
                ...
                for kw in node.keywords:
                    if kw.arg is not None:
                        ...
                        return node
                if kwds_entries:
                    kwds_dict = ast.Dict(keys=[ast.Constant(value=k.arg) for k in kwds_entries], values=[k.value for k in kwds_entries])
                    new_call = ast.Call(func=ast.Attribute(value=ast.Name(id=pool_var, ctx=ast.Load()), attr='apply_async', ctx=ast.Load()), args=[func_arg, args_tuple], keywords=[ast.keyword(arg='kwds', value=kwds_dict)])
                else:
                    new_call = ast.Call(func=ast.Attribute(value=ast.Name(id=pool_var, ctx=ast.Load()), attr='apply_async', ctx=ast.Load()), args=[func_arg, args_tuple], keywords=[])
...
def _find_import_insert_index(module_node: Any) -> Any:
    """Return index to insert import after docstring and __future__ imports."""
    index = 0
    if module_node.body and isinstance(module_node.body[0], ast.Expr) and isinstance(module_node.body[0].value, ast.Constant) and isinstance(module_node.body[0].value.value, str):
        index = 1
...
def migrate_file(path: Path) -> bool:
    ...
    try:
        ...
        transformer = FuturesToPoolMigrator(collector.executor_vars, collector.future_vars)
        new_tree = transformer.visit(tree)
        ast.fix_missing_locations(new_tree)
        if not transformer.needs_migration:
            return False
        if not transformer.has_mp_alias:
            import_node = ast.Import(names=[ast.alias(name='multiprocessing', asname='mp')])
