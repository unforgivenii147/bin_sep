...
"""mo2po.py – Mo2Po utilities.

This module provides functionality for mo2po."""
...
def mo_to_po(mo_path: Path | str, remove_orig: bool=True, verbose: bool=False) -> bool:
    """mo_to_po – mo to po.

Args:
    mo_path: Description of mo_path.
...
Returns:
    ...
    try:
        with open(po_path, 'w', encoding='utf-8') as po_file:
            subprocess.run(['msgunfmt', str(mo_path)], stdout=po_file, stderr=subprocess.PIPE, text=True, check=True)
...
def mo_to_po_python_only(mo_path: Path | str, remove_orig: bool=True, verbose: bool=False) -> bool:
    """mo_to_po_python_only – mo to po python only.
...
def process_directory(directory: Path, recursive: bool=False, remove_orig: bool=True, verbose: bool=False, fallback: bool=False) -> None:
...
Args:
    ...
    remove_orig: Description of remove_orig.
    verbose: Description of verbose.
    fallback: Description of fallback."""
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Convert .mo files to .po files in-place', formatter_class=argparse.RawDescriptionHelpFormatter, epilog='\nExamples:\n  %(prog)s file.mo\n  %(prog)s -k file.mo\n  %(prog)s /path/to/locale/\n  %(prog)s -r /usr/share/locale/\n  %(prog)s -v -r ./locale/\n  %(prog)s --fallback file.mo\n        ')
    ...
    parser.add_argument('--fallback', action='store_true', help="Use pure Python fallback method (doesn't require gettext utilities)")
