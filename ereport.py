...
"""ereport.py – Ereport utilities.

This module provides functionality for ereport."""
from __future__ import annotations
import json
import shutil
import subprocess
from pathlib import Path
TOOLS = {'mypy': ['mypy', '--ignore-missing-imports'], 'ruff': ['ruff', 'check', '--fix', '--unsafe-fixes'], 'ty': ['ty', 'check'], 'pyrefly': ['pyrefly', 'check'], 'pylint': ['pylint', '--errors-only']}
...
def execute_tool(cmd_base: list[str], target_file: Path) -> dict:
    ...
    if shutil.which(tool_name) is None:
        return {'status': 'skipped', 'error': f"Executable '{tool_name}' not found in PATH."}
    cmd = cmd_base + [str(target_file)]
    try:
        proc = subprocess.run(cmd, capture_output=True, text=True, check=False)
        combined_output = (proc.stdout + proc.stderr).strip()
        return {'exit_code': proc.returncode, 'has_errors': proc.returncode != 0, 'output': combined_output if combined_output else 'No output.'}
...
def analyze_file(py_file: Path, report_dir: Path) -> None:
    ...
    report_data = {'filename': py_file.name, 'filepath': str(py_file.resolve()), 'tools': {}}
