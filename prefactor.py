...
"""prefactor.py – Prefactor utilities.

This module provides functionality for prefactor."""
...
def module_fullname_for_path(root: Path, path: Path, package_mode: bool, package_name: str | None) -> str:
    """module_fullname_for_path – module fullname for path.

Args:
    root: Description of root.
    path: Description of path.
    package_mode: Description of package_mode.
...
def resolve_relative_import(curr_fullname: str, module: str | None, level: int) -> str | None:
    """resolve_relative_import – resolve relative import.

Args:
    curr_fullname: Description of curr_fullname.
...
def analyze_file(args: str) -> ModuleInfo:
...
Returns:
    ModuleInfo: Description of return value."""
...
Returns:
    tuple[list[str], list[set[str]]]: Description of return value."""
...
def build_merged_source(modules: dict[str, ModuleInfo], ordered: list[str], out_module_name: str) -> str:
    """build_merged_source – build merged source.

Args:
    modules: Description of modules.
    ordered: Description of ordered.
    out_module_name: Description of out_module_name.
...
def main() -> None:
    ...
    parser.add_argument('input', nargs='?', type=Path, default=Path.cwd(), help='Input directory containing .py files or a package. Defaults to current directory.')
    parser.add_argument('output_dir', nargs='?', type=Path, default=Path.cwd() / 'out', help='Directory to write merged file to. Defaults to ./out.')
    ...
    parser.add_argument('--package-name', type=str, default=None, help='Force package name (defaults to input folder name if package).')
    parser.add_argument('--force', action='store_true', help='Force merge even if file count > max-files.')
    parser.add_argument('-j', '--jobs', type=int, default=0, help='Number of worker processes to use (default: auto cpu count).')
