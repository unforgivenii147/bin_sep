...
FLOAT_RE = re.compile('^[+-]?(?:\\d+\\.\\d*|\\.\\d+|\\d+)(?:[eE][+-]?\\d+)?$')
...
Returns:
    ...
    parser = argparse.ArgumentParser(description='Convert SQL INSERT dump files to streaming newline-delimited JSON using eight worker processes.')
    parser.add_argument('inputs', nargs='*', type=Path, help='SQL files or directories. Defaults to the current directory recursively.')
    parser.add_argument('-o', '--output-dir', type=Path, help='Directory for generated .jsonl files. By default, output is written beside each source SQL file.')
    parser.add_argument('--overwrite', action='store_true', help='Replace an existing output file instead of skipping it.')
    parser.add_argument('--encoding', default=DEFAULT_ENCODING, help=f'Input encoding, default: {DEFAULT_ENCODING}.')
    parser.add_argument('--strict', action='store_true', help='Fail a file when malformed INSERT data is found.')
...
Returns:
    ...
    suffix = f'{source.stem}_{abs(hash(str(source.parent))) & 4294967295:08x}.jsonl'
...
def parse_columns(column_group: str | None) -> list[str] | None:
    """parse_columns – parse columns.

Args:
    column_group: Description of column_group.

Returns:
    list[str] | None: Description of return value."""
...
def iter_insert_statements(path: Path, encoding: str, chunk_bytes: int=READ_CHUNK_BYTES) -> Iterator[str]:
    """
...
def rows_from_statement(statement: str) -> Iterator[dict[str, Any]]:
    """rows_from_statement – rows from statement.

Args:
    statement: Description of statement.

Returns:
    Iterator[dict[str, Any]]: Description of return value."""
    ...
    for tuple_text in extract_tuples(values_text):
        ...
        if columns is None:
            row: dict[str, Any] = {'_table': table, '_values': values}
        else:
            row = {'_table': table}
...
def convert_file(job: Job, encoding: str, strict: bool) -> Result:
    ...
    try:
        ...
        fd, temp_name = tempfile.mkstemp(prefix=f'.{destination.name}.', suffix='.tmp', dir=destination.parent, text=True)
                ...
                for statement in iter_insert_statements(source, encoding):
                    try:
                        for row in rows_from_statement(statement):
                            json.dump(row, output, ensure_ascii=False, separators=(',', ':'), allow_nan=False)
                            output.write('\n')
                            rows_written += 1
...
Returns:
    ...
    jobs = [Job(source=source, destination=output_path_for(source, output_dir), overwrite=args.overwrite) for source in files]
    ...
    with Pool(processes=WORKERS) as pool:
        for result in pool.imap_unordered(worker, payloads, chunksize=chunksize):
            if result.error is not None:
                ...
                print('%s -> %s (%d rows)', result.source, result.destination, result.rows)
    print('Completed: %d file(s), %d row(s), %d failure(s).', len(files), total_rows, failures)
