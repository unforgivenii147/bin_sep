...
"""fix_regex_escape.py – Fix Regex Escape utilities.

This module provides functionality for fix regex escape."""
...
Returns:
    Call: Description of return value."""
        self.generic_visit(node)
        if isinstance(node.func, ast.Attribute) and (isinstance(node.func.value, ast.Name) and node.func.value.id == 're' and (node.func.attr in TARGET_FUNCS) and node.args):
