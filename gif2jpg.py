...
"""gif2jpg.py – Gif2Jpg utilities.

This module provides functionality for gif2jpg."""
...
def frames_are_similar(arr_a: np.ndarray, arr_b: np.ndarray) -> bool:
    """frames_are_similar – frames are similar.

Args:
    arr_a: Description of arr_a.
    arr_b: Description of arr_b.
...
def extract_unique_frames(gif_path: Path) -> list[np.ndarray]:
    """extract_unique_frames – extract unique frames.

Args:
    gif_path: Description of gif_path.

Returns:
    list[np.ndarray]: Description of return value."""
    frames: list[np.ndarray] = []
    try:
        with Image.open(gif_path) as img:
            if not hasattr(img, 'n_frames'):
                canvas = Image.new('RGB', img.size, (255, 255, 255))
                canvas.paste(img.convert('RGBA'), mask=img.convert('RGBA').split()[3] if img.mode in ('RGBA', 'P') else None)
...
def convert_gif(gif_path: Path) -> tuple[Path, int, int]:
    """convert_gif – convert gif.

Args:
    gif_path: Description of gif_path.

Returns:
    tuple[Path, int, int]: Description of return value."""
...
def main() -> None:
    ...
    if not unique_gifs:
        ...
        return
    log.info('Found %d GIF file(s). Starting conversion with %s workers…', len(unique_gifs), 'all CPUs' if N_JOBS == -1 else str(N_JOBS))
