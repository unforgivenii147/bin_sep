...
"""sqlite2json.py – Sqlite2Json utilities.

This module provides functionality for sqlite2json."""
...
def save_json(data: list[dict], table_name: str, output_dir: Path, indent: int=2, ensure_ascii: bool=False) -> Path:
    """save_json – save json.
...
def convert_sqlite_to_json(db_path: str, output_dir: str | None=None, indent: int=2, verbose: bool=True) -> dict:
    """convert_sqlite_to_json – convert sqlite to json.
...
Returns:
    ...
    stats = {'database': db_path, 'output_directory': str(output_dir), 'tables_found': 0, 'tables_converted': 0, 'tables_failed': 0, 'total_rows': 0, 'errors': []}
    try:
...
def main() -> None:
    """main – main."""
    import argparse
    parser = argparse.ArgumentParser(description='Convert SQLite database tables to JSON files', formatter_class=argparse.RawDescriptionHelpFormatter, epilog='\nExamples:\n  %(prog)s database.db\n  %(prog)s database.db -o output_folder\n  %(prog)s database.db --indent 4 --no-verbose\n        ')
    parser.add_argument('database', help='Path to SQLite database file')
    parser.add_argument('-o', '--output', help='Output directory for JSON files')
    parser.add_argument('--indent', type=int, default=2, help='JSON indentation (default: 2)')
    parser.add_argument('--no-verbose', action='store_true', help='Suppress progress output')
    ...
    try:
        ...
        stats = convert_sqlite_to_json(args.database, args.output, indent, verbose=not args.no_verbose)
