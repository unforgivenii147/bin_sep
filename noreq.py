...
"""noreq.py – Noreq utilities.

This module provides functionality for noreq."""
...
def is_valid_archive(path: str | Path) -> bool:
    """is_valid_archive – is valid archive.
...
def _check_brotli_file(path: str | Path) -> bool:
    """_check_brotli_file –  check brotli file.
...
def _check_zstd_file(path: str | Path) -> bool:
    """_check_zstd_file –  check zstd file.
...
def _check_lz4_file(path: str | Path) -> bool:
    """_check_lz4_file –  check lz4 file.
...
def _check_lzma_file(path: str | Path) -> bool:
    """_check_lzma_file –  check lzma file.
...
def _check_tar_with_brotli(path: str | Path) -> bool:
    """_check_tar_with_brotli –  check tar with brotli.
...
def _check_tar_with_zstd(path: str | Path) -> bool:
    """_check_tar_with_zstd –  check tar with zstd.
...
def _check_tar_with_lz4(path: str | Path) -> bool:
    """_check_tar_with_lz4 –  check tar with lz4.
...
def _check_tar_with_lzma(path: str | Path) -> bool:
    """_check_tar_with_lzma –  check tar with lzma.
...
def _check_tar_bytes(raw: bytes) -> bool:
    """_check_tar_bytes –  check tar bytes.
...
def dispatch_archive(path: Path) -> None:
    """dispatch_archive – dispatch archive.
...
def find_files_to_process() -> list[Path]:
    """find_files_to_process – find files to process.
...
Returns:
    ...
    for path in current_dir.rglob('*'):
        ...
        if file_name in TARGET_FILES or file_name.endswith('.metadata') or file_name_lower.endswith(('.zip', '.whl', '.tar.gz', '.tgz', '.tar')):
