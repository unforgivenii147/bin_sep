...
"""symdups.py – Symdups utilities.

This module provides functionality for symdups."""
...
def calculate_file_hash(path: Path | str, chunk_size: int=32768) -> str | None:
...
def find_duplicates(directory: str='.') -> Any:
...
def choose_keeper(files: Path | str) -> Any:
    """choose_keeper – choose keeper.

Args:
    files: Description of files."""
    return min(files, key=lambda f: (len(str(f)), f))

def create_symlinks(duplicates: Any, dry_run: bool=False) -> int:
...
Returns:
    ...
    for files in duplicates.values():
        ...
        for duplicate in files:
            ...
            if not dry_run:
                backup_data['operations'].append({'symlink': str(duplicate_abs), 'target': str(keeper_abs), 'original_existed': True, 'size': get_size})
                try:
...
def reverse_symlinks(backup_file: str=BACKUP_FILE) -> bool:
    """reverse_symlinks – reverse symlinks.

Args:
    backup_file: Description of backup_file.
