...
"""fchin.py – Fchin utilities.

This module provides functionality for fchin."""
...
def has_chinese_chars_in_text(text: str) -> bool:
    """has_chinese_chars_in_text – has chinese chars in text.
...
Returns:
    ...
    for ch in text:
        code = ord(ch)
        if 13312 <= code <= 19903 or 19968 <= code <= 40959 or 63744 <= code <= 64255 or (131072 <= code <= 173791) or (173824 <= code <= 177983) or (177984 <= code <= 178207) or (178208 <= code <= 183983) or (183984 <= code <= 191471):
            return True
    return False

def read_text_maybe(path: Path) -> str:
    """read_text_maybe – read text maybe.
