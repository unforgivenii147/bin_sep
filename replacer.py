...
"""replacer.py – Replacer utilities.

This module provides functionality for replacer."""
...
def process_file(path: Path, search_text: str, replace_text: str='', remove_mode: bool=False, dry_run: bool=False) -> bool:
    """process_file – process file.

Args:
    path: Description of path.
    search_text: Description of search_text.
    replace_text: Description of replace_text.
    remove_mode: Description of remove_mode.
...
def replace_in_files(search_text: str, replace_text: str='', remove_mode: bool=False, target_file: str | None=None, dry_run: bool=False) -> tuple[int, int]:
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Recursively replace or remove text in files.', formatter_class=argparse.RawDescriptionHelpFormatter, epilog="Examples:\n  %(prog)s 'old_text' 'new_text'    # Replace text\n  %(prog)s -r 'text_to_remove'      # Remove text\n  %(prog)s 'text' -r                # Remove text (alternative syntax)\n  %(prog)s 'text' --dry-run         # Preview changes")
    parser.add_argument('search', help='Text to search for')
    parser.add_argument('replace', nargs='?', default='', help='Replacement text (optional, defaults to empty string)')
    ...
    print(f'--- {action} ---')
    files_processed, files_changed = replace_in_files(search_text, replace_text, remove_mode=args.remove, target_file=args.file, dry_run=args.dry_run)
