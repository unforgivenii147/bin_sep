...
"""sparse_clone.py – Sparse Clone utilities.

This module provides functionality for sparse clone."""
...
Args:
    repo_url: Description of repo_url.
...
Returns:
    ...
    try:
        ...
        subprocess.run(['git', 'clone', '--filter=blob:none', '--sparse', repo_url, str(repo_path)], check=True, capture_output=True, timeout=300)
        subprocess.run(['git', '-C', str(repo_path), 'sparse-checkout', 'init', '--no-cone'], check=True, capture_output=True)
        patterns = [f"**/*{(ext if ext.startswith('.') else f'.{ext}')}" for ext in extensions]
        subprocess.run(['git', '-C', str(repo_path), 'sparse-checkout', 'set'] + patterns, check=True, capture_output=True)
