...
"""plotter.py – Plotter utilities.

This module provides functionality for plotter."""
...
Args:
    x: Description of x."""
    return eval(function_expr, {'__builtins__': {}}, {'x': x, 'np': np, 'sin': np.sin, 'cos': np.cos, 'tan': np.tan, 'arctan': np.arctan, 'arcsin': np.arcsin, 'arccos': np.arccos, 'sqrt': np.sqrt, 'exp': np.exp, 'log': np.log, 'pi': np.pi, 'e': np.e})
