...
"""lz5.py – Lz5 utilities.

This module provides functionality for lz5."""
...
def get_folder_size(folder_path: Path | str) -> Any:
...
def compress_folder(folder_path: Path | str) -> Any:
...
Args:
    ...
    try:
        ...
        return {'folder': folder.name, 'original_size': original_size, 'compressed_size': compressed_size, 'space_freed': space_freed, 'ratio': ratio, 'compression_percent': compression_percent, 'status': 'success'}
    except Exception as e:
        return {'folder': folder.name, 'error': str(e), 'status': 'error'}

def decompress_file(path: Path | str) -> Any:
...
def print_compression_report(results: Any) -> None:
    """print_compression_report – print compression report.
...
Args:
    ...
    for r in successful:
        ...
        print(f"{r['folder']:<20} {fsz(r['original_size']):<12} {fsz(r['compressed_size']):<12} {fsz(r['space_freed']):<12} {r['ratio']:>6.2f}x   {r['compression_percent']:>6.1f}%")
    print('-' * 40)
    print(f"{'TOTAL':<20} {fsz(total_original):<12} {fsz(total_compressed):<12} {fsz(total_freed):<12} {(total_original / total_compressed if total_compressed > 0 else 0):>6.2f}x   {((1 - total_compressed / total_original) * 40 if total_original > 0 else 0):>6.1f}%")
    print('-' * 40)
    if errors:
        print('\nERRORS:')
        for r in errors:
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Compress/decompress folders with LZ4', epilog='Default action: compress subfolders in current directory')
