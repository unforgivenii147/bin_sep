...
"""gdrive_syncer5.py – Gdrive Syncer5 utilities.

This module provides functionality for gdrive syncer5."""
...
class GoogleDriveSync:
    ...
    def __init__(self, client_id: int | None=None, client_secret: Any | None=None, token_file: str='drive_token.pkl') -> None:
        """__init__ –   init  .
...
Returns:
    ...
    def authenticate_device_flow(self) -> None:
        """authenticate_device_flow – authenticate device flow."""
        print('\n' + '=' * 40)
        ...
        try:
            ...
            poll_data = {'client_id': self.client_id, 'client_secret': self.client_secret, 'device_code': device_info['device_code'], 'grant_type': 'urn:ietf:params:oauth:grant-type:device_code'}
    ...
    def authenticate_manual_flow(self) -> None:
        """authenticate_manual_flow – authenticate manual flow."""
        print('\n' + '=' * 40)
        ...
        token_data = {'client_id': self.client_id, 'client_secret': self.client_secret, 'code': auth_code, 'grant_type': 'authorization_code', 'redirect_uri': 'http://localhost:8080'}
    ...
    def api_request(self, method: str, url: str, **kwargs: Any) -> Response:
...
Returns:
    ...
    def list_files(self, folder_id: str='root', page_token: Any | None=None) -> Any:
        """list_files – list files.

Args:
    folder_id: Description of folder_id.
    page_token: Description of page_token."""
        url = 'https://www.googleapis.com/drive/v3/files'
        params = {'q': f"'{folder_id}' in parents and trashed=false", 'pageSize': 100, 'fields': 'nextPageToken, files(id, name, mimeType, size, modifiedTime)'}
        if page_token:
    ...
    def get_all_files_recursive(self, folder_id: str='root') -> list[Path]:
        """get_all_files_recursive – get all files recursive.
...
Returns:
    ...
    def get_file_metadata(self, file_id: str) -> list[Path]:
...
Args:
    ...
    def sync_folder(self, drive_folder_id: str, local_folder_path: Path | str, folder_name: str='root', depth: int=0) -> None:
    ...
    depth: Description of depth."""
        ...
        for item in items:
            ...
            if item_mime == 'application/vnd.google-apps.folder':
                ...
                if should_download and self.download_file(item_id, item_name, local_path) and remote_modified:
                    mod_time = datetime.fromisoformat(remote_modified).timestamp()
                    os.utime(local_path, (mod_time, mod_time))

    def sanitize_filename(self, filename: Path | str) -> Any:
...
Args:
    ...
    def sync_folder_by_name(self, folder_name: Path | str, local_base_path: Path | str) -> None:
        """sync_folder_by_name – sync folder by name.
