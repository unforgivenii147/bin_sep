...
"""dirperm.py – Dirperm utilities.

This module provides functionality for dirperm."""
...
def walk_all(root_path: str='.') -> Iterator[Any]:
    """walk_all – walk all.
...
def walk_dirs(root_path: str='.') -> Iterator[Any]:
    """walk_dirs – walk dirs.
...
def determine_dir_target_mode() -> int:
    """determine_dir_target_mode – determine dir target mode."""
    return 509

def determine_file_target_mode(path: Path | str) -> int:
    """determine_file_target_mode – determine file target mode.
...
def analyze_item(item_type: Any, path: Path | str) -> Any:
...
def process_item(path: Path | str, target_mode: Any, dry_run: bool=False) -> bool:
...
Args:
    ...
    total_changes = len(stats['dirs_to_change']) + len(stats['files_make_executable']) + len(stats['files_set_standard'])
    print(f"\n{'=' * 40}")
    print('Scan Summary:')
...
def main() -> None:
    ...
    parser.add_argument('--show-examples', action='store_true', help='Show example files/directories that would be changed')
    ...
    stats = {'total_dirs': 0, 'total_files': 0, 'dirs_to_change': [], 'dirs_correct': [], 'files_skip_executable': [], 'files_skip_correct': [], 'files_make_executable': [], 'files_set_standard': [], 'errors': []}
