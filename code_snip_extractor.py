...
"""code_snip_extractor.py – Code Snip Extractor utilities.

This module provides functionality for code snip extractor."""
...
def extract_snippets(path: Path) -> Generator[tuple[int, str], None, None]:
    """extract_snippets – extract snippets.

Args:
    path: Description of path.

Returns:
    Generator[tuple[int, str], None, None]: Description of return value."""
    try:
    ...
    while i < len(lines):
        if lines[i].strip().startswith('```'):
            fence_match = re.match('^```+(\\w*)', lines[i].strip())
            if fence_match:
                start_line = i + 1
                i += 1
                code_lines = []
        ...
        if lines[i].strip().startswith('>>>'):
            ...
            while i < len(lines):
                ...
                if stripped.startswith('>>>'):
                    ...
                    if code_lines and (not any((c in stripped for c in ['=', 'True', 'False', '[']))):
                        break
                    i += 1
            code_text = '\n'.join(code_lines).strip()
            if code_text:
...
Args:
    ...
    for target in targets:
        if target.is_file():
            ...
            all_files.extend((p for p in target.rglob('*') if p.is_file() and '.git' not in p.parts and (not p.is_symlink()) and (p.suffix in {'.md', '.rst', '.txt', '.METADATA', '.PKG-INFO', '.cfg', '.ini'})))
    if not all_files:
