...
def pil_to_array(image: Image) -> ndarray:
    return np.array(image.convert("RGBA"), dtype=np.uint8)


def array_to_pil(array: ndarray) -> Image:
    if array.ndim == 2:
...
def flatten_alpha(image: Image, background: tuple[int, int, int, int]) -> Image:
    if image.mode != "RGBA":
...
def load_image(path: Path, page: int = 0) -> Image:
...
def rotate_image(
    image: Image, angle: float, background: tuple[int, int, int, int]
) -> Image:
    return image.rotate(
...
def opencv_blur(image: Image, radius: float, sigma: float) -> Image:
...
def sharpen_image(
    image: Image, radius: float, sigma: float, amount: float, threshold: int
) -> Image:
    array = pil_to_array(image)
    kernel = max(3, round(radius * 2 + 1) | 1)
    sigma = sigma if sigma > 0 else radius
...
def median_image(image: Image, size: int) -> Image:
    array = pil_to_array(image)
    kernel = max(3, int(size) | 1)
    return array_to_pil(cv2.medianBlur(array, kernel))


def despeckle_image(image: Image) -> Image:
    array = pil_to_array(image)
    return array_to_pil(cv2.medianBlur(array, 3))


def edge_image(image: Image, radius: float) -> Image:
...
def emboss_image(image: Image) -> Image:
    array = pil_to_array(image)
...
def normalize_image(image: Image, equalize: bool = False) -> Image:
    array = pil_to_array(image)
    rgb = array[:, :, :3]
    if equalize:
...
def contrast_stretch(image: Image, low: float, high: float) -> Image:
    array = pil_to_array(image)
...
def gamma_image(image: Image, gamma: float) -> Image:
    array = pil_to_array(image)
...
def modulate_image(
    image: Image, brightness: float, saturation: float, hue: float
) -> Image:
    array = pil_to_array(image)
    rgb = array[:, :, :3]
...
def tint_image(image: Image, color: tuple[int, int, int, int], amount: float) -> Image:
    overlay = Image.new("RGBA", image.size, color)
    return Image.blend(image, overlay, max(0, min(1, amount)))


def threshold_image(image: Image, threshold: int) -> Image:
    gray = np.array(image.convert("L"))
    _, result = cv2.threshold(gray, threshold, 255, cv2.THRESH_BINARY)
    return Image.fromarray(result, "L").convert("RGBA")


def adaptive_threshold_image(image: Image, block_size: int, constant: float) -> Image:
    gray = np.array(image.convert("L"))
...
def negate_image(image: Image) -> Image:
    array = pil_to_array(image)
    array[:, :, :3] = 255 - array[:, :, :3]
    return array_to_pil(array)


def sepia_image(image: Image) -> Image:
    array = pil_to_array(image)
    rgb = array[:, :, :3].astype(np.float32)
    matrix = np.array(
...
def channel_image(image: Image, channel: str) -> Image:
    array = pil_to_array(image)
...
def append_images(
    images: list[Image], vertical: bool, background: tuple[int, int, int, int]
) -> Image:
    if vertical:
...
def apply_operations(image: Image, args: argparse.Namespace) -> Image:
    if args.resize:
