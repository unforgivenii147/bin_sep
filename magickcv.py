...
"""magickcv.py – Magickcv utilities.

This module provides functionality for magickcv."""
...
RESAMPLING = {'nearest': Image.Resampling.NEAREST, 'box': Image.Resampling.BOX, 'bilinear': Image.Resampling.BILINEAR, 'hamming': Image.Resampling.HAMMING, 'bicubic': Image.Resampling.BICUBIC, 'lanczos': Image.Resampling.LANCZOS}

def parse_color(value: str) -> tuple[int, int, int, int]:
    """parse_color – parse color.

Args:
    value: Description of value.

Returns:
    tuple[int, int, int, int]: Description of return value."""
    try:
...
def parse_geometry(value: str) -> tuple[int | None, int | None, str]:
    """parse_geometry – parse geometry.

Args:
    value: Description of value.

Returns:
    tuple[int | None, int | None, str]: Description of return value."""
    match = re.fullmatch('(?:(\\d+)?x?(\\d+)?)?([!%^<>]*)', value.strip())
...
Returns:
    list[Image.Image]: Description of return value."""
    with Image.open(path) as source:
        frames = []
...
def resize_image(image: Image.Image, geometry: tuple[int | None, int | None, str], mode: str, filter_name: str) -> Image.Image:
...
Args:
    ...
    filter_name: Description of filter_name.
...
Returns:
    ...
    if '!' in flags:
        ...
        result_size = (max(1, round(source_width * factor)), max(1, round(source_height * factor)))
    elif mode == 'sample':
        factor = min(width / source_width, height / source_height)
        result_size = (max(1, round(source_width * factor)), max(1, round(source_height * factor)))
...
def extent_image(image: Image.Image, size: tuple[int, int], background: tuple[int, int, int, int], gravity: str) -> Image.Image:
    """extent_image – extent image.

Args:
    image: Description of image.
    size: Description of size.
    background: Description of background.
    gravity: Description of gravity.
...
def gravity_position(container: tuple[int, int], item: tuple[int, int], gravity: str) -> tuple[int, int]:
    """gravity_position – gravity position.
...
Returns:
    ...
    cw, ch = container
    iw, ih = item
    positions = {'northwest': (0, 0), 'north': ((cw - iw) // 2, 0), 'northeast': (cw - iw, 0), 'west': (0, (ch - ih) // 2), 'center': ((cw - iw) // 2, (ch - ih) // 2), 'east': (cw - iw, (ch - ih) // 2), 'southwest': (0, ch - ih), 'south': ((cw - iw) // 2, ch - ih), 'southeast': (cw - iw, ch - ih)}
    return positions.get(gravity.lower(), positions['center'])

def crop_image(image: Image.Image, crop: tuple[int, int, int, int], background: tuple[int, int, int, int]) -> Image.Image:
...
def despeckle_image(image: Image.Image) -> Image.Image:
    """despeckle_image – despeckle image.
...
Args:
    image: Description of image.
    equalize: Description of equalize.
...
def modulate_image(image: Image.Image, brightness: float, saturation: float, hue: float) -> Image.Image:
    """modulate_image – modulate image.

Args:
    image: Description of image.
    brightness: Description of brightness.
    saturation: Description of saturation.
    hue: Description of hue.
...
def adaptive_threshold_image(image: Image.Image, block_size: int, constant: float) -> Image.Image:
    """adaptive_threshold_image – adaptive threshold image.

Args:
    image: Description of image.
    block_size: Description of block_size.
    constant: Description of constant.

Returns:
    Image.Image: Description of return value."""
    gray = np.array(image.convert('L'))
    block_size = max(3, block_size | 1)
    result = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, block_size, constant)
    return Image.fromarray(result, 'L').convert('RGBA')

def negate_image(image: Image.Image) -> Image.Image:
    """negate_image – negate image.
...
def composite_image(base: Image.Image, overlay: Image.Image, offset: tuple[int, int], gravity: str, blend: str, dissolve: float) -> Image.Image:
    """composite_image – composite image.

Args:
    base: Description of base.
    overlay: Description of overlay.
    offset: Description of offset.
    gravity: Description of gravity.
    blend: Description of blend.
    dissolve: Description of dissolve.
...
def draw_text(image: Image.Image, text: str, point: tuple[int, int], fill: tuple[int, int, int, int], stroke: tuple[int, int, int, int], stroke_width: int, font_path: str | None, font_size: int, gravity: str) -> Image.Image:
    """draw_text – draw text.
...
Args:
    ...
    fill: Description of fill.
    stroke: Description of stroke.
    stroke_width: Description of stroke_width.
    font_path: Description of font_path.
    font_size: Description of font_size.
    gravity: Description of gravity.
...
def append_images(images: list[Image.Image], vertical: bool, background: tuple[int, int, int, int]) -> Image.Image:
    """append_images – append images.

Args:
    images: Description of images.
    vertical: Description of vertical.
    background: Description of background.
...
def montage_images(images: list[Image.Image], tile: tuple[int, int], geometry: tuple[int, int], background: tuple[int, int, int, int]) -> Image.Image:
    """montage_images – montage images.
...
def save_image(image: Image.Image, output: Path, quality: int | None, background: tuple[int, int, int, int], compression: int | None) -> None:
...
Returns:
    ...
    parser.add_argument('-compose', choices=['over', 'multiply', 'screen', 'difference', 'add'], default='over')
...
Returns:
    ...
    if args.unsharp:
        ...
        image = sharpen_image(image, radius, sigma, amount / 100.0 if amount > 2 else amount, int(threshold))
    if args.median:
    ...
    if args.annotate:
        ...
        image = draw_text(image, args.annotate[1], offset, args.fill, args.stroke, args.strokewidth, args.font, args.pointsize, args.gravity)
    return image

def apply_draw(image: Image.Image, command: str, fill: tuple[int, int, int, int], stroke: tuple[int, int, int, int], stroke_width: int) -> Image.Image:
    """apply_draw – apply draw.
...
Returns:
    ...
    result = image.copy()
    draw = ImageDraw.Draw(result)
    rect = re.fullmatch('\\s*rectangle\\s+([+-]?\\d+),([+-]?\\d+)\\s+([+-]?\\d+),([+-]?\\d+)\\s*', command, re.IGNORECASE)
    line = re.fullmatch('\\s*line\\s+([+-]?\\d+),([+-]?\\d+)\\s+([+-]?\\d+),([+-]?\\d+)\\s*', command, re.IGNORECASE)
    circle = re.fullmatch('\\s*circle\\s+([+-]?\\d+),([+-]?\\d+)\\s+([+-]?\\d+),([+-]?\\d+)\\s*', command, re.IGNORECASE)
    if rect:
...
Returns:
    ...
    if args.montage:
        ...
        image = montage_images(images, args.tile, parse_size(args.extent) if args.extent else images[0].size, args.background)
