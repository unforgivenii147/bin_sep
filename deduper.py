...
"""deduper.py – Deduper utilities.

This module provides functionality for deduper."""
...
SKIP_DIRS = {'.git', '.hg', '.svn', '__pycache__', '.mypy_cache', '.pytest_cache', '.tox', '.nox', '.venv', 'venv', 'env', '.eggs', 'site-packages'}
...
def get_module_docstring_line_span(tree: ast.Module) -> tuple[int, int | None] | None:
    """get_module_docstring_line_span – get module docstring line span.

Args:
    tree: Description of tree.

Returns:
    tuple[int, int | None] | None: Description of return value."""
    if not tree.body:
        return None
    first = tree.body[0]
...
def is_simple_constant_assign(node: ast.Assign) -> bool:
    """is_simple_constant_assign – is simple constant assign.
...
Args:
    ...
    for node in tree.body:
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            ...
            if not snippet:
                continue
            objects.append({'name': node.name, 'kind': 'function', 'snippet': snippet, 'lineno': node.lineno, 'end_lineno': node.end_lineno})
        elif isinstance(node, ast.ClassDef):
            snippet = source_segment(code, node)
            if not snippet:
                continue
            objects.append({'name': node.name, 'kind': 'class', 'snippet': snippet, 'lineno': node.lineno, 'end_lineno': node.end_lineno})
            ...
            if not snippet:
                continue
            objects.append({'name': name, 'kind': 'constant', 'snippet': snippet, 'lineno': node.lineno, 'end_lineno': node.end_lineno})
    return objects
...
Args:
    ...
    for obj in objs:
        ...
        if not stripped:
            continue
        out.append({'file': str(path), 'name': obj['name'], 'kind': obj['kind'], 'snippet': obj['snippet'], 'hash': sha256(stripped), 'lineno': obj['lineno'], 'end_lineno': obj['end_lineno']})
    return out
...
def find_import_insertion_index(code: str) -> int:
    """find_import_insertion_index – find import insertion index.
...
def add_import_line(code: str, module_name: str, names: str) -> str:
    """add_import_line – add import line.
...
def merge_overlapping_ranges(ranges: Any) -> Any:
    """merge_overlapping_ranges – merge overlapping ranges.
...
def remove_line_ranges(code: str, ranges: Any) -> str:
    """remove_line_ranges – remove line ranges.
...
def file_is_under_base(path: Path, base: Path) -> bool:
    """file_is_under_base – file is under base.
...
def main() -> None:
    ...
    parser.add_argument('-c', '--copy', action='store_true', help='Copy duplicate objects to utils.py without modifying source files')
