...
"""generate_init.py – Generate Init utilities.

This module provides functionality for generate init."""
...
def get_public_names(path: Path) -> list[str]:
    """get_public_names – get public names.
...
def get_python_modules(directory: Path) -> list[Path]:
    """get_python_modules – get python modules.
...
def main() -> None:
    ...
    modules_with_public = [(name, names) for name, names in module_public.items() if names]
