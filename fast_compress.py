...
SKIP_EXTENSIONS_COMPRESS: frozenset[str] = frozenset({'.xz', '.gz', '.7z', '.zip', '.whl', '.lz4', '.zst', '.br', '.bz2', '.lzma', '.z', '.rar', '.tar', '.tgz', '.tbz2', '.bz3', '.jpg', '.dat', '.npz', '.onnx', '.jpeg', '.png', '.gif', '.bmp', '.tiff', '.tif', '.webp', '.svg', '.ico', '.heic', '.heif', '.avif', '.mp4', '.mkv', '.avi', '.mov', '.wmv', '.flv', '.webm', '.m4v', '.mpg', '.mpeg', '.3gp', '.ogv', '.ts', '.m2ts', '.mp3', '.wav', '.flac', '.aac', '.ogg', '.wma', '.m4a', '.opus', '.mid', '.midi', '.aiff', '.pdf', '.docx', '.pptx', '.xlsx', '.odt', '.ods', '.odp', '.epub', '.mobi', '.azw', '.azw3', '.exe', '.dll', '.dylib', '.bin', '.iso', '.img', '.deb', '.rpm', '.pkg', '.msi'})
MEDIA_EXTENSIONS: frozenset[str] = frozenset({'.jpg', '.jpeg', '.png', '.gif', '.bmp', '.tiff', '.tif', '.webp', '.svg', '.ico', '.heic', '.heif', '.avif', '.mp4', '.mkv', '.avi', '.mov', '.wmv', '.flv', '.webm', '.m4v', '.mpg', '.mpeg', '.3gp', '.ogv', '.ts', '.m2ts', '.mp3', '.wav', '.flac', '.aac', '.ogg', '.wma', '.m4a', '.opus', '.mid', '.midi', '.aiff', '.pdf', '.docx', '.pptx', '.xlsx', '.odt', '.ods', '.odp', '.epub', '.mobi', '.azw', '.azw3'})
VALID_DECOMPRESS_EXTENSIONS: frozenset[str] = frozenset({'.zst'})
SKIP_DIRS: frozenset[str] = frozenset({'.git', '__pycache__', '.pytest_cache', '.mypy_cache', '.bin', 'bin', 'dh', 'print_persian', '.dist-info', '.egg-info', 'zstandard'})
...
def compress_file(input_path: Path, output_path: Path, level: int, remove_original: bool) -> tuple[bool, Path, Path | str, int, int]:
...
def decompress_file(input_path: Path, output_path: Path, remove_original: bool) -> tuple[bool, Path, Path | str, int, int]:
...
def process_stream(base_dir: Path, compress: bool, level: int, remove_original: bool) -> None:
    """Walk base_dir and compress or decompress eligible files in parallel."""
    print('{} files (streaming)...', 'Compressing' if compress else 'Decompressing')
    ...
    def _report_progress() -> None:
        ...
        logger.opt(colors=False).info('\rProgress: [{}] {}/{} files', bar, completed, grand_total)
    ...
    with Pool(processes=POOL_WORKERS) as pool:
        ...
        for path, op_out in tasks:
            if compress:
                ar = pool.apply_async(compress_file, (path, op_out, level, remove_original))
            else:
                ar = pool.apply_async(decompress_file, (path, op_out, remove_original))
    ...
    if failed:
        ...
        if success_count > 0:
            logger.success('✅ Successfully {} {} files!', 'compressed' if compress else 'decompressed', success_count)
            if remove_original:
...
def main() -> int:
    ...
    parser.add_argument('--level', type=int, default=3, choices=range(1, 23), help='Compression level (1-22, default: 3)')
    parser.add_argument('--dir', type=str, default='.', help='Directory to process (default: current directory)')
    parser.add_argument('--keep', default=False, action='store_true', help='Keep original files (default: remove on success)')
