...
"""filter_noneng.py – Filter Noneng utilities.

This module provides functionality for filter noneng."""
...
def process_file_lines(input_path: Path, move_mode: bool) -> None:
...
Args:
    ...
    for i, line in enumerate(lines, start=1):
        ...
        if result.language == 'en' and result.is_reliable:
            ...
            lang_info = f'[{result.language.upper()} (Prob: {result.probability:.2f})]' if result.language != 'und' else '[UNKNOWN]'
...
def main() -> None:
    ...
    parser.add_argument('-m', '--move', action='store_true', help='Move identified non-English lines to noneng.txt and drop them from the source file.')
