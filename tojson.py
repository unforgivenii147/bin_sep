...
"""tojson.py – Tojson utilities.

This module provides functionality for tojson."""
...
def file_to_json(path: Path, delimiter: str) -> Any:
    """file_to_json – file to json.
    ...
    try:
        with open(path, encoding='utf-8') as f:
            for line_num, line in enumerate(f, start=1):
                ...
                if len(parts) != 2:
                    print(f"Warning: Line {line_num} doesn't contain delimiter '{delimiter}': {line!r}", file=sys.stderr)
                    continue
                key, value = parts
                key = key.strip()
