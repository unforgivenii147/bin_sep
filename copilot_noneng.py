...
"""copilot_noneng.py – Copilot Noneng utilities.

This module provides functionality for copilot noneng."""
...
def read_text_lines(path: Path) -> Iterator[str]:
    """read_text_lines – read text lines.

Args:
    path: Description of path.

Returns:
    Iterator[str]: Description of return value."""
...
def detect_langdetect(text: str) -> str | None:
    """detect_langdetect – detect langdetect.
...
def combine_votes(g3: str | None, p2: str | None, ld: str | None) -> dict[str, Any]:
    """combine_votes – combine votes.

Args:
    g3: Description of g3.
    p2: Description of p2.
    ld: Description of ld.

Returns:
    dict[str, Any]: Description of return value."""
    votes = {'gcld3': normalize(g3), 'pycld2': normalize(p2), 'langdetect': normalize(ld)}
    print(votes)
...
Returns:
    ...
    if non_en_votes >= 2:
        ...
        if votes['gcld3'] != 'unknown' and (not is_en(votes['gcld3'])) or (votes['pycld2'] != 'unknown' and (not is_en(votes['pycld2']))):
            decision = True
        else:
            decision = False
    else:
...
Returns:
    ...
    if combined['non_english']:
        rec = {'file': str(path), 'line_no': lineno, 'text': text, 'detectors': combined['votes'], 'non_english': True}
        return rec
    return None

def process_file_sequential(path: Path, max_len: int) -> list[dict[str, Any]]:
    """process_file_sequential – process file sequential.
...
def process_file_per_line_parallel(path: Path, max_len: int, workers: int) -> list[dict[str, Any]]:
    """process_file_per_line_parallel – process file per line parallel.
...
Returns:
    ...
    with mp.Pool(processes=8) as ex:
        futures = {ex.apply_async(detect_line, args=(path, lineno, line, max_len)): lineno for lineno, line in enumerate(lines, start=1) if line.strip()}
...
def run_per_file(files: list[Path], workers: int, max_len: int) -> None:
    """run_per_file – run per file.
...
Args:
    ...
    max_len: Description of max_len."""
    with mp.Pool(processes=8) as ex:
        futures = {ex.apply_async(process_file_sequential, args=(f, max_len)): f for f in files}
        for fut in as_completed(futures):
            try:
                res = fut.get()
                if res:
...
def run_per_line(files: list[Path], workers: int, max_len: int) -> None:
    """run_per_line – run per line.
...
Args:
    ...
    with mp.Pool(processes=8) as ex_files:
        futures = {ex_files.apply_async(process_file_per_line_parallel, args=(f, max_len, workers)): f for f in files}
        for fut in as_completed(futures):
            try:
                res = fut.get()
                if res:
...
Returns:
    ...
    parser.add_argument('--parallel-mode', choices=['file', 'line'], default='file', help='Parallelism mode: file (default) or line (inside each file).')
