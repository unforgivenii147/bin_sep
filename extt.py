...
"""extt.py – Extt utilities.

This module provides functionality for extt."""
...
def get_node_name(node: Any) -> Any:
...
Args:
    node: Description of node."""
        if node.type in VALID:
            ...
            definitions.append({'type': node.type.replace('_definition', ''), 'name': name, 'text': node_text, 'line': node.start_point.row + 1})
        for child in node.children:
            traverse(child)
    traverse(root)
    return definitions
