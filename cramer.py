...
try:
    ...
    sys.exit(1)
CODECS = {'snappy': ('.sz', lambda d, l: bytes(cj.snappy.compress(d)), lambda d: bytes(cj.snappy.decompress(d)), None), 'lz4': ('.lz4', lambda d, l: bytes(cj.lz4.compress(d)), lambda d: bytes(cj.lz4.decompress(d)), None), 'gzip': ('.gz', lambda d, l: bytes(cj.gzip.compress(d, level=l)), lambda d: bytes(cj.gzip.decompress(d)), 9), 'deflate': ('.deflate', lambda d, l: bytes(cj.deflate.compress(d, level=l)), lambda d: bytes(cj.deflate.decompress(d)), 9), 'bzip2': ('.bz2', lambda d, l: bytes(cj.bzip2.compress(d, level=l)), lambda d: bytes(cj.bzip2.decompress(d)), 9), 'xz': ('.xz', lambda d, l: bytes(cj.xz.compress(d, level=l)), lambda d: bytes(cj.xz.decompress(d)), 9), 'zstd': ('.zst', lambda d, l: bytes(cj.zstd.compress(d, level=l)), lambda d: bytes(cj.zstd.decompress(d)), 22), 'brotli': ('.br', lambda d, l: bytes(cj.brotli.compress(d, level=l)), lambda d: bytes(cj.brotli.decompress(d)), 11)}
DEFAULT_ALGO = 'snappy'
EXT_TO_ALGO = {ext: name for name, (ext, _, _, _) in CODECS.items()}

def compress_file(path: Path, algo: str, level: int, keep: bool, dry_run: bool) -> dict:
    """compress_file – compress file.

Args:
    path: Description of path.
    algo: Description of algo.
    level: Description of level.
    keep: Description of keep.
...
Returns:
    ...
    result = {'input': str(path), 'output': str(out_path), 'algo': algo, 'level': level, 'ok': False, 'removed': False, 'in_bytes': None, 'out_bytes': None, 'ratio': None, 'error': None}
    try:
...
Returns:
    ...
    result = {'input': str(path), 'output': None, 'algo': algo, 'level': None, 'ok': False, 'removed': False, 'in_bytes': None, 'out_bytes': None, 'ratio': None, 'error': None}
...
def collect_files_for_compress(root: Path, skip_suffixes: Any) -> list[Any]:
    """collect_files_for_compress – collect files for compress.

Args:
    root: Description of root.
    skip_suffixes: Description of skip_suffixes."""
    if root.is_file():
        return [] if root.suffix.lower() in skip_suffixes else [root]
    if root.is_dir():
        return [p for p in sorted(root.rglob('*')) if p.is_file() and p.suffix.lower() not in skip_suffixes]
    return []

def collect_files_for_decompress(root: Path, algo_filter: Any) -> list[Any]:
...
def detect_algo(path: Path, forced: str) -> Any | None:
    """detect_algo – detect algo.

Args:
    path: Description of path.
    forced: Description of forced."""
...
def main() -> None:
    ...
    mode.add_argument('-c', '--compress', dest='mode', action='store_const', const='compress', help='Compress files (default)')
    mode.add_argument('-d', '--decompress', dest='mode', action='store_const', const='decompress', help='Decompress files')
    parser.set_defaults(mode='compress')
    parser.add_argument('-a', '--algo', default=None, choices=sorted(CODECS.keys()), help=f'cramjam algorithm (compress default: {DEFAULT_ALGO}; decompress default: auto by extension)')
    parser.add_argument('-k', '--keep', action='store_true', help='Keep the original files')
    parser.add_argument('--dry-run', action='store_true', help="Show what would happen, don't write or delete")
    ...
    if args.mode == 'compress':
        ...
        skip_suffixes = {'.gz', '.bz2', '.xz', '.zst', '.br', '.lz4', '.sz', '.deflate', '.tmp'}
        ...
        print(f'Files    : {len(files)}' + ('  [dry-run]' if args.dry_run else '') + ('  [keep originals]' if args.keep else '  [originals will be deleted]'))
        print()
        ...
        for path in files:
            ...
            print(f"  OK    {path}  ->  {r['output']}  [{r['in_bytes']:,} -> {r['out_bytes']:,} B, {ratio}]{flag}")
        print()
        if args.dry_run:
        ...
        if total_in:
            print(f'Total: {total_in:,} B -> {total_out:,} B ({total_in / total_out:.2f}x)')
    else:
        ...
        print(f'Target   : {args.path}')
        print(f'Files    : {len(files)}' + ('  [dry-run]' if args.dry_run else '') + ('  [keep originals]' if args.keep else '  [compressed files will be deleted]'))
        print()
        total_in = total_out = 0
        ...
        for path in files:
            ...
            ratio = f"{r['ratio']:.2f}x" if r['ratio'] else '?'
            flag = '  (kept original)' if not r['removed'] else ''
            print(f"  OK    {path}  ->  {r['output']}  [{algo}]  [{r['in_bytes']:,} -> {r['out_bytes']:,} B, {ratio}]{flag}")
        print()
        if args.dry_run:
            print(f'Dry run complete. {len(files)} file(s) would be decompressed.')
            return
        print(f'Done. ok={n_ok}  failed={n_fail}  skipped={n_skipped}  originals_removed={n_removed}')
        if total_in:
            print(f'Total: {total_in:,} B -> {total_out:,} B ({total_out / total_in:.2f}x)')
