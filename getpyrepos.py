...
"""getpyrepos.py – Getpyrepos utilities.

This module provides functionality for getpyrepos."""
        ...
        try:
            ...
            for repo in page_repos:
                if repo.get('language') == 'Python':
                    repos.append({'name': repo['name'], 'full_name': repo['full_name'], 'description': repo.get('description', ''), 'url': repo['html_url'], 'stars': repo['stargazers_count'], 'forks': repo['forks_count'], 'language': repo['language'], 'created_at': repo['created_at'], 'updated_at': repo['updated_at'], 'private': repo['private']})
...
def get_top_trending_users() -> list[dict]:
    """get_top_trending_users – get top trending users.
...
Returns:
    ...
    try:
        ...
        for user in data.get('items', []):
            ...
            trending_users.append({'username': user_data['login'], 'name': user_data.get('name', ''), 'followers': user_data['followers'], 'repos_url': user_data['repos_url'], 'html_url': user_data['html_url'], 'repositories': get_user_repos(user_data['login'])})
