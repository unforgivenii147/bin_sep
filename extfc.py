...
"""extfc.py – Extfc utilities.

This module provides functionality for extfc."""
...
def get_node_text(src: bytes, node: Any) -> str:
...
def extract_functions_and_classes(src: bytes, tree: Tree) -> Any:
    """extract_functions_and_classes – extract functions and classes.
...
def extract_docstring(src: bytes, node: Any) -> str | None:
    """extract_docstring – extract docstring.
...
def format_definition_with_metadata(def_text: str, file_name: str, line_num: int, docstring: str | None=None) -> str:
    """format_definition_with_metadata – format definition with metadata.

Args:
    def_text: Description of def_text.
    file_name: Description of file_name.
    line_num: Description of line_num.
    docstring: Description of docstring.
...
for folder, defs_list in folder_definitions.items():
    ...
    print(f"✅ saved: {out_file} ({folder_def_count} definitions from {len([f for f in defs_list if 'File:' in f])} files)")
