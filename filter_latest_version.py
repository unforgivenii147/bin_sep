...
"""filter_latest_version.py – Filter Latest Version utilities.

This module provides functionality for filter latest version."""
...
def parse_wheel_url(url: str) -> tuple[str, str, tuple[int, ...], str] | None:
    """parse_wheel_url – parse wheel url.

Args:
    url: Description of url.

Returns:
    tuple[str, str, tuple[int, ...], str] | None: Description of return value."""
...
def is_armv7_arch(arch: str) -> bool:
    """is_armv7_arch – is armv7 arch.

Args:
    arch: Description of arch.
...
def filter_latest_for_armv7(urls_file: Path | str | None=None) -> Any:
    """filter_latest_for_armv7 – filter latest for armv7.

Args:
    urls_file: Description of urls_file."""
    urls = []
    ...
    for (package, py_version), arches in sorted(packages.items()):
        for arch, (version, url) in arches.items():
            ...
            print(f'   URL: {url}')
            results.append({'package': package, 'python_version': py_version, 'arch': arch, 'version': version_str, 'url': url})
    print('\n' + '=' * 40)
...
def main() -> None:
    """main – main."""
    import argparse
    parser = argparse.ArgumentParser(description='Find latest ARMv7 wheels from URL list', epilog='Example: python3 filter_armv7.py urls.txt')
