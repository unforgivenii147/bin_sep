...
"""gtransline.py – Gtransline utilities.

This module provides functionality for gtransline."""
...
Args:
    ...
    try:
        ...
        if not target_indices:
            print('  No non-English (Chinese) lines found, skipping.')
            return
        ...
        for i, idx in enumerate(target_indices):
            ...
            translated_count += 1
            if (i + 1) % batch_size == 0:
                print('  Progress: %d/%d lines translated', i + 1, len(target_indices))
...
def main() -> None:
    ...
    parser.add_argument('--extensions', nargs='+', default=['.txt', '.md', '.py', '.js', '.html', '.css', '.json', '.xml', '.csv'], help='File extensions to process')
    parser.add_argument('--workers', type=int, default=mp.cpu_count(), help=f'Number of parallel workers (default: {mp.cpu_count()})')
        ...
        if path.is_file():
            ...
            for ext in args.extensions:
                for path in path.rglob(f'*{ext}'):
                    if path.is_file() and path.resolve() not in exclude_paths and (not any((part.startswith('.') for part in path.parts))):
                        files_to_process.append(path)
