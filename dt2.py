...
"""dt2.py – Dt2 utilities.

This module provides functionality for dt2."""
...
def save_progress(all_lines: list[str], results: dict[str, str], output_path: Path, json_path: Path, failed_path: Path, source_lang: str, target_lang: str, output_type: str='json') -> None:
    """
    ...
    """
    try:
        successful = {line: results[line] for line in all_lines if line in results and results[line] and (results[line] != line)}
        ...
        if output_type == 'text':
            ...
            json_data = {'metadata': {'source_lang': source_lang, 'target_lang': target_lang, 'total_lines': len(all_lines), 'translated_lines': len(successful), 'failed_lines': len(failed), 'untranslated_lines': len(all_lines) - len(successful) - len(failed), 'interrupted': interrupted, 'timestamp': time.strftime('%Y-%m-%d %H:%M:%S')}, 'translations': successful}
...
def main() -> None:
    ...
    parser.add_argument('-o', '--output-type', choices=['text', 'json', 'merged'], default='json', help="Output type: 'text' (translated lines only), 'json' (structured JSON; failed lines go to a separate file), 'merged' (source line + translation line pairs). Default: json")
    ...
    save_progress(all_lines, results, output_path, json_path, failed_path, source_lang, target_lang, args.output_type)
