...
"""repack_pkgs.py – Repack Pkgs utilities.

This module provides functionality for repack pkgs."""
...
Args:
    ...
    def detect_package_type(self, package_dir: Path) -> tuple[bool, bool, bool]:
        """detect_package_type – detect package type.

Args:
    package_dir: Description of package_dir.

Returns:
    tuple[bool, bool, bool]: Description of return value."""
        ...
        has_binary = any((path.suffix.lower() in binary_extensions for path in package_dir.rglob('*') if path.is_file()))
...
Returns:
    ...
    def get_platform_tag() -> str:
        """get_platform_tag – get platform tag.
...
Returns:
    PackageInfo | None: Description of return value."""
        ...
        try:
            ...
            return PackageInfo(name=package_name, version=version, location=str(package_dir), is_pure_python=is_pure, has_c_extension=has_c_ext, has_binary=has_binary, py_version=py_version, abi_tag=abi_tag, platform_tag=platform_tag, wheel_filename=wheel_filename)
...
Args:
    package_info: Description of package_info.
    ...
    tuple[bool, str]: Description of return value."""
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with zipfile.ZipFile(temp_wheel, 'r') as zf_read, zipfile.ZipFile(wheel_path, 'w', zipfile.ZIP_DEFLATED) as zf_write:
...
class VenvRepacker:
    """VenvRepacker – VenvRepacker."""

    def __init__(self, site_packages_dir: str | None=None, output_dir: str | None=None, verbose: bool=False, dry_run: bool=False) -> None:
        """__init__ –   init  .

Args:
    site_packages_dir: Description of site_packages_dir.
    output_dir: Description of output_dir.
    verbose: Description of verbose.
    dry_run: Description of dry_run."""
        ...
        self.stats = {'total_packages': 0, 'pure_python_packages': 0, 'packages_with_c_extensions': 0, 'successfully_repacked': 0, 'failed_packages': 0, 'total_size_before': 0, 'total_size_after': 0}
        self.results = []
...
Returns:
    list[str]: Description of return value."""
        ...
        packages = {item.name for item in self.site_packages.iterdir() if item.is_dir() and (not item.name.startswith('.')) and (not any((item.name.endswith(p) for p in exclude_patterns)))}
...
Returns:
    tuple[bool, str, PackageInfo | None]: Description of return value."""
        try:
    ...
    dict: Description of return value."""
        ...
        for i, package_name in enumerate(packages, 1):
            ...
            if success and package_info:
                ...
                print(f'✓ {message}')
                self.results.append({'package': package_name, 'version': package_info.version, 'is_pure_python': package_info.is_pure_python, 'wheel_filename': package_info.wheel_filename, 'success': True})
            else:
...
Args:
    report_file: Description of report_file."""
        report = {'timestamp': datetime.now().isoformat(), 'site_packages': str(self.site_packages), 'output_directory': str(self.output_dir), 'statistics': self.stats, 'results': self.results}
        try:
...
def main() -> None:
    ...
    try:
        repacker = VenvRepacker(site_packages_dir=args.site_packages, output_dir=args.output, verbose=args.verbose, dry_run=args.dry_run)
        repacker.repack_all()
