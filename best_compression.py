...
"""best_compression.py – Best Compression utilities.

This module provides functionality for best compression."""
...
def copy_chunks(src: Path | str, dst: Path | str, chunk_size: int=1024 * 1024) -> None:
...
Returns:
    str: Description of return value."""
    return {'brotli': '.br', 'huffman': '.hf', 'gz': '.gz', 'bz2': '.bz2', 'lzma': '.xz', 'zip': '.zip', 'snappy': '.snappy', 'zstd': '.zst', '7z': '.7z'}.get(algo, f'.{algo}')

def compress_7z(in_path: Path, out_path: Path) -> None:
    """compress_7z – compress 7z.

Args:
    in_path: Description of in_path.
...
def compress_huffman(in_path: Path, out_path: Path) -> None:
    """compress_huffman – compress huffman.
...
def compress_snappy(in_path: Path, out_path: Path) -> None:
    """compress_snappy – compress snappy.
...
ALGO_SINGLE: dict[str, tuple[str, Any]] = {'7z': ('7z', compress_7z), 'gz': ('gz', compress_gz), 'lzma': ('lzma', compress_lzma), 'bz2': ('bz2', compress_bz2), 'zip': ('zip', compress_zip), 'brotli': ('brotli', compress_brotli), 'huffman': ('huffman', compress_huffman), 'snappy': ('snappy', compress_snappy), 'zstd': ('zstd', compress_zstd)}
...
Returns:
    ...
    try:
        ...
        return Result(algo=algo, input_path=str(in_path), out_path=str(out_path), out_size=out_size, elapsed_s=elapsed, ok=True)
    except Exception as e:
        logger.exception(f'[{algo}] failed: {e}')
        return Result(algo=algo, input_path=str(in_path), out_path='', out_size=0, elapsed_s=0.0, ok=False, error=str(e))
WORKER_ALGOS = {'gz', 'bz2', 'lzma', 'zstd', 'brotli', 'snappy'}

def _chunk_compressor(algo: str) -> Any:
    """_chunk_compressor –  chunk compressor.
...
def mp_compress_chunks(algo: str, in_path: Path, tmpdir: Path, chunk_size: int, processes: int | None) -> Result:
    """mp_compress_chunks – mp compress chunks.
...
Returns:
    ...
    if algo not in WORKER_ALGOS:
        return Result(algo=f'mp_{algo}', input_path=str(in_path), out_path='', out_size=0, elapsed_s=0.0, ok=False, error='Chunk mode not supported')
    try:
        ...
        elapsed = time.perf_counter() - t0
        out_size = out_path.stat().st_size
        return Result(algo=f'mp_{algo}', input_path=str(in_path), out_path=str(out_path), out_size=out_size, elapsed_s=elapsed, ok=True)
    except Exception as e:
        logger.exception(f'[mp_{algo}] failed: {e}')
        return Result(algo=f'mp_{algo}', input_path=str(in_path), out_path='', out_size=0, elapsed_s=0.0, ok=False, error=str(e))
...
Returns:
    Result | None: Description of return value."""
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix='compress_bench_') as td:
        ...
        for algo in single_algos:
            ...
            if r.ok:
                print(f'[{algo}] OK size={fsz(r.out_size)} time={r.elapsed_s:.4f}s out={Path(r.out_path).name}')
            else:
        ...
        for algo in mp_algos:
            ...
            mp_results.append(r)
            if r.ok:
                print(f'[mp_{algo}] OK size={fsz(r.out_size)} time={r.elapsed_s:.4f}s out={Path(r.out_path).name}')
            else:
