...
EXT_IMAGE: Final[frozenset[str]] = frozenset({'.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp', '.tif', '.tiff', '.svg'})
HTTP_SCHEMES: Final[tuple[str, ...]] = ('http://', 'https://')
HTML_ATTR_RE: Final[re.Pattern[str]] = re.compile('(?P<attr>href|src)\\s*=\\s*(?P<q>["\\\'])(?P<url>.*?)(?P=q)', re.IGNORECASE)
MD_IMAGE_RE: Final[re.Pattern[str]] = re.compile('!\\[([^\\]]*?)\\]\\((?P<url>[^)\\s]+)(?:\\s+\\"[^\\"]*\\")?\\)')
MD_LINK_RE: Final[re.Pattern[str]] = re.compile('\\[(?P<text>[^\\]]*?)\\]\\((?P<url>[^)\\s]+)(?:\\s+\\"[^\\"]*\\")?\\)')
...
def _replace_md(md: str, file_dir: Path, timeout: int) -> tuple[str, list[str]]:
    ...
    def replace_md_images(match: re.Match[str]) -> str:
        """replace_md_images – replace md images.
...
Returns:
    ...
    def replace_md_links(match: re.Match[str]) -> str:
        """replace_md_links – replace md links.
