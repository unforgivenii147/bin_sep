...
"""ex_video_link.py – Ex Video Link utilities.

This module provides functionality for ex video link."""
...
def extract_zzztube_link(zzztube_url: str) -> dict:
    """extract_zzztube_link – extract zzztube link.

Args:
    zzztube_url: Description of zzztube_url.
...
Returns:
    ...
    for script in soup.find_all('script', {'type': 'application/json'}):
        try:
            ...
            continue
    video_id = None
    url_match = re.search('zzztube\\.com/(\\d+)', zzztube_url)
    if url_match:
    ...
    if og_video:
        ...
        if 'zzztube' in video_src:
            id_match = re.search('/(\\d+)', video_src)
            if id_match:
    ...
    return {'video_id': video_id, 'title': title, 'playable_url': playable_url, 'iframe_src': iframe_src, 'direct_url': f'https://player.zzztube.com/video/{video_id}' if video_id else None}

def save_results_to_file(results: dict, output_file: str='zzztube_links.json') -> Path:
