...
"""check_missing_scripts.py – Check Missing Scripts utilities.

This module provides functionality for check missing scripts."""
...
Returns:
    ...
    result = {'package': package_name, 'path': str(dist_info_dir), 'missing_scripts': [], 'defined_scripts': [], 'has_entry_points': False}
    ...
    for script_name, module_entry in scripts:
        ...
        if not script_path.exists():
            result['missing_scripts'].append({'name': script_name, 'entry': module_entry, 'expected_path': str(script_path)})
    return result
