...
"""dduputil.py – Dduputil utilities.

This module provides functionality for dduputil."""
...
def is_constant_assign(node: ast.Assign) -> bool:
    """is_constant_assign – is constant assign.
...
def extract_top_level_imports(tree: ast.Module) -> list[ast.stmt]:
    """extract_top_level_imports – extract top level imports.

Args:
    tree: Description of tree.

Returns:
    list[ast.stmt]: Description of return value."""
...
def try_decompress_single(data: bytes, ext: str) -> tuple[bytes, str] | None:
    """try_decompress_single – try decompress single.
...
Returns:
    tuple[bytes, str] | None: Description of return value."""
    try:
        if ext == '.gz':
...
def iter_python_sources(root: Path) -> Iterable[SourceFile]:
    """iter_python_sources – iter python sources.

Args:
    root: Description of root.

Returns:
    Iterable[SourceFile]: Description of return value."""
    root = root.resolve()
    for p in root.rglob('*'):
        if p.is_dir():
            continue
...
def extract_defs_from_source(srcfile: tuple[str, str, str, str]) -> tuple[str, list[dict]]:
    """extract_defs_from_source – extract defs from source.

Args:
    srcfile: Description of srcfile.

Returns:
    tuple[str, list[dict]]: Description of return value."""
...
def partition_by_hash(extracted_map: dict[str, list[tuple[SourceFile, dict]]]) -> dict[str, list[tuple[SourceFile, dict]]]:
    """partition_by_hash – partition by hash.

Args:
    extracted_map: Description of extracted_map.

Returns:
    dict[str, list[tuple[SourceFile, dict]]]: Description of return value."""
    return extracted_map

def ensure_utils_dir() -> None:
    """ensure_utils_dir – ensure utils dir."""
    UTILS_DIR.mkdir(exist_ok=True)

def append_unique_to_file(target: Path, items: list[dict], seen_hashes: set) -> None:
    """append_unique_to_file – append unique to file.

Args:
    target: Description of target.
    items: Description of items.
    seen_hashes: Description of seen_hashes."""
    if not items:
        return
    lines = []
    if target.exists():
...
def safe_write_file(path: Path, content: str) -> None:
    """safe_write_file – safe write file.
...
def remove_nodes_from_source(original_src: str, nodes_to_remove: list[str]) -> str:
    """remove_nodes_from_source – remove nodes from source.

Args:
    original_src: Description of original_src.
    nodes_to_remove: Description of nodes_to_remove.
...
def add_imports_to_source(original_src: str, import_lines: list[str]) -> str:
    """add_imports_to_source – add imports to source.

Args:
    original_src: Description of original_src.
    import_lines: Description of import_lines.
...
def validate_python_source(text: str) -> bool:
    """validate_python_source – validate python source.
...
def main() -> None:
    ...
    parser.add_argument('--min-occurs', type=int, default=2, help='minimum occurrences to consider duplicate (default 2)')
    ...
    print('found {} duplicated code blocks (occurring >= {})', len(dups), args.min_occurs)
    ensure_utils_dir()
    funcs_to_write = []
    classes_to_write = []
    consts_to_write = []
    ...
    for h, instances in dups.items():
        ...
        for sf, _info in instances:
            if args.copy:
                ...
                d = modifications.setdefault(sf.path, {'remove_snippets': [], 'add_imports': set(), 'preserve_imports': []})
    ...
    if args.move:
        print('applying move modifications to original files ({} targets)', len(modifications))
