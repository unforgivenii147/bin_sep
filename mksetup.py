...
"""mksetup.py – Mksetup utilities.

This module provides functionality for mksetup."""
...
def read_entry_points(root: Path) -> dict[str, list[str]]:
    """read_entry_points – read entry points.
...
def extract_wheel(whl: Path, dst: Path) -> None:
    """extract_wheel – extract wheel.

Args:
    whl: Description of whl.
...
Returns:
    ...
    return {'name': meta['Name'], 'version': meta['Version'], 'summary': meta.get('Summary', ''), 'install_requires': meta.get_all('Requires-Dist') or []}

def find_extensions(root: Path) -> list[str]:
    """find_extensions – find extensions.
...
def generate_pyproject_toml() -> str:
    """generate_pyproject_toml – generate pyproject toml.
