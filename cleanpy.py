...
def _comment_should_be_preserved(text: str, is_first_line: bool) -> bool:
    """_comment_should_be_preserved –  comment should be preserved.

Args:
    text: Description of text.
    is_first_line: Description of is_first_line.
...
Args:
    remove_module_docstring: Description of remove_module_docstring."""
...
Returns:
    cst.Comment | RemovalSentinel: Description of return value."""
    ...
    def _handle_docstring(self, body: cst.IndentedBlock, remove: bool) -> cst.IndentedBlock:
        """_handle_docstring –  handle docstring.

Args:
    body: Description of body.
    remove: Description of remove.
...
Returns:
    cst.IndentedBlock: Description of return value."""
        ...
        if not (isinstance(first, cst.SimpleStatementLine) and first.body and isinstance(first.body[0], cst.Expr) and isinstance(first.body[0].value, cst.SimpleString)):
            return body
...
def _remove_blank_lines(data: str) -> str:
    """_remove_blank_lines –  remove blank lines.
...
def parse_args() -> argparse.Namespace:
    ...
    parser: argparse.ArgumentParser = argparse.ArgumentParser(description='Remove Python comments and docstrings recursively, preserving shebangs, # fmt, # type, and module docstrings by default.')
    parser.add_argument('-r', '--remove-module-docstring', action='store_true', help='remove module-level docstrings too')
    parser.add_argument('paths', nargs='*', type=Path, help='files and/or directories to process (default: current directory)')
...
def main() -> int:
    ...
    try:
        async_results: list[tuple[Path, object]] = [(path, pool.apply_async(process_file, (str(path), args.remove_module_docstring))) for path in files]
        pool.close()
        for path, async_result in async_results:
            ...
            if result.changed:
                ...
                print(f'{result.path}: comments removed={result.comments_removed}, docstrings removed={result.docstrings_removed}')
            else:
                print(f'{result.path}: no changes')
        pool.join()
    finally:
        pool.terminate()
    print(f'Summary: files changed={changed_files}, comments removed={total_comments}, docstrings removed={total_docstrings}, errors={errors}')
