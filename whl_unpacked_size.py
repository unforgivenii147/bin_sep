...
"""whl_unpacked_size.py – Whl Unpacked Size utilities.

This module provides functionality for whl unpacked size."""
...
def get_wheel_unpacked_size(wheel_path: Path) -> tuple[Path, int, str | None]:
    """get_wheel_unpacked_size – get wheel unpacked size.

Args:
    wheel_path: Description of wheel_path.

Returns:
    tuple[Path, int, str | None]: Description of return value."""
    try:
...
def find_wheel_files(directory: Path, recursive: bool=False) -> list[Path]:
    """find_wheel_files – find wheel files.
...
def main() -> None:
    ...
    parser.add_argument('-d', '--directory', type=Path, default=Path.cwd(), help='Directory to scan (default: current directory)')
    ...
    parser.add_argument('-s', '--sort', choices=['name', 'size'], default='name', help='Sort output by name or size (default: name)')
    ...
    if args.json:
        output = {'directory': str(args.directory), 'recursive': args.recursive, 'summary': {'total_wheels': len(wheels), 'processed': len(results), 'errors': len(errors), 'total_unpacked_size_bytes': total_unpacked_size, 'total_unpacked_size_formatted': fsz(total_unpacked_size)}, 'wheels': []}
        for wheel_path, size in results:
            output['wheels'].append({'name': wheel_path.name, 'path': str(wheel_path), 'size_bytes': size, 'size_formatted': fsz(size)})
        if errors:
            output['errors'] = []
