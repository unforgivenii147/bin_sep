...
"""shufeler.py – Shufeler utilities.

This module provides functionality for shufeler."""
...
def get_line_offsets(path: Path) -> Any:
    """get_line_offsets – get line offsets.
...
def crypto_shuffle_offsets(offsets: Any) -> None:
    """crypto_shuffle_offsets – crypto shuffle offsets.
...
def shuffle3_offsets(offsets: Any) -> None:
    """shuffle3_offsets – shuffle3 offsets.
...
def weighted_shuffle_offsets(offsets: Any) -> None:
    """weighted_shuffle_offsets – weighted shuffle offsets.
...
def enhanced_shuffle_large_file(input_path: Path, output_path: Path) -> bool:
    """enhanced_shuffle_large_file – enhanced shuffle large file.
...
Returns:
    ...
    try:
        with input_path.open('rb') as infile, mmap.mmap(infile.fileno(), 0, access=mmap.ACCESS_READ) as mm, output_path.open('wb') as outfile:
...
def enhanced_shuffle_small_file(input_path: Path, output_path: Path) -> bool:
    """enhanced_shuffle_small_file – enhanced shuffle small file.
...
Returns:
    ...
    try:
        ...
        print(f"MemoryError: File '{input_path}' is too large to load into memory. Consider increasing 1mb or using a system with more RAM.", file=sys.stderr)
        return False
    except Exception as e:
...
def weighted_shuffle(lst: list[str]) -> None:
    """weighted_shuffle – weighted shuffle.
