...
"""afk2.py – Afk2 utilities.

This module provides functionality for afk2."""
...
def _is_under_type_checking(node: ast.AST, type_checking_blocks: set[int]) -> bool:
    """_is_under_type_checking –  is under type checking.

Args:
    node: Description of node.
    type_checking_blocks: Description of type_checking_blocks.
...
def _collect_type_checking_line_ranges(tree: ast.Module) -> set[int]:
    """_collect_type_checking_line_ranges –  collect type checking line ranges.
...
Returns:
    ...
    for node in ast.walk(tree):
        if not isinstance(node, ast.If):
            continue
        test = node.test
        is_tc = isinstance(test, ast.Name) and test.id == 'TYPE_CHECKING' or (isinstance(test, ast.Attribute) and test.attr == 'TYPE_CHECKING')
        if is_tc:
...
def _collect_all_entries(tree: ast.Module) -> set[str]:
    """_collect_all_entries –  collect all entries.
...
def _collect_used_names(tree: ast.Module) -> set[str]:
    """_collect_used_names –  collect used names.
...
def _import_aliases(node: ast.Import | ast.ImportFrom) -> list[tuple[str, str]]:
    """_import_aliases –  import aliases.
...
Args:
    ...
    is_init: Description of is_init.
    ignore_init: Description of ignore_init.

Returns:
    FileReport: Description of return value."""
    ...
    module_doc = ''
    if tree.body and isinstance(tree.body[0](0), ast.Expr) and isinstance(tree.body[0](0).value, ast.Constant) and isinstance(tree.body[0](0).value.value, str):
    ...
    for node in ast.walk(tree):
        ...
        report.unused.append(UnusedImport(lineno=node.lineno, col_offset=node.col_offset, original_stmt=original_stmt, unused_names=unused_here))
    return report

def _remove_names_from_import_line(line: str, names_to_remove: set[str]) -> str | None:
    """_remove_names_from_import_line –  remove names from import line.

Args:
    line: Description of line.
    names_to_remove: Description of names_to_remove.
...
Returns:
    ...
    if m:
        ...
        kept = [seg.strip() for seg in rest_clean.split(',') if seg.strip() and _alias_local_name(seg.strip()) not in names_to_remove]
        if not kept:
            return None
        return prefix + ', '.join(kept)
    return line
...
def _alias_local_name(segment: str) -> str:
    """_alias_local_name –  alias local name.
...
def _fix_multiline_import(block: str, names_to_remove: set[str]) -> str | None:
    """_fix_multiline_import –  fix multiline import.
...
Args:
    source: Description of source.
    unused_imports: Description of unused_imports.
...
def _iter_whl_sources(archive_path: Path) -> Iterator[tuple[str, str]]:
    """_iter_whl_sources –  iter whl sources.

Args:
    archive_path: Description of archive_path.

Returns:
    Iterator[tuple[str, str]]: Description of return value."""
    try:
...
def _iter_tarzst_sources(archive_path: Path) -> Iterator[tuple[str, str]]:
    """_iter_tarzst_sources –  iter tarzst sources.
...
def _task_analyze_file(path_str: str) -> FileReport:
    """_task_analyze_file –  task analyze file.
...
def _task_analyze_source_str(args: tuple[str, str]) -> FileReport:
    """_task_analyze_source_str –  task analyze source str.
...
def _should_exclude(path: Path, exclude_patterns: list[re.Pattern[str]]) -> bool:
    """_should_exclude –  should exclude.

Args:
    path: Description of path.
    exclude_patterns: Description of exclude_patterns.
...
def collect_tasks(paths: list[Path], exclude_patterns: list[re.Pattern[str]]) -> tuple[list[str], list[tuple[str, str]]]:
...
Returns:
    tuple[list[str], list[tuple[str, str]]]: Description of return value."""
    file_tasks: list[str] = []
...
def print_fix_result(path_str: str, *, fixed: bool, dry_run: bool, error: str | None=None) -> None:
    """print_fix_result – print fix result.

Args:
    path_str: Description of path_str.
    fixed: Description of fixed.
    dry_run: Description of dry_run.
...
def run(paths: list[Path], *, autofix: bool=False, dry_run: bool=False, verbose: bool=False, workers: int=8, exclude_patterns: list[re.Pattern[str]] | None=None, ignore_init: bool=False) -> int:
    """run – run.
...
Returns:
    ...
    if total_unused == 0:
        ...
        print(bold(f'Found {red(str(total_unused))} unused import(s) across {red(str(len(files_with_issues)))} file(s).'))
...
Returns:
    ...
    parser = argparse.ArgumentParser(prog='unused_imports', description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter, epilog=textwrap.dedent('            Examples:\n              %(prog)s src/\n              %(prog)s src/ --autofix\n              %(prog)s src/ --dry-run\n              %(prog)s mypackage.whl\n              %(prog)s . --exclude tests --workers 4 --verbose\n              %(prog)s file.py --autofix --no-color\n            '))
    parser.add_argument('paths', nargs='*', default=['.'], metavar='PATH', help='Files or directories to scan (default: current directory).')
    parser.add_argument('-a', '--autofix', action='store_true', help='Remove unused imports in-place.')
    parser.add_argument('--dry-run', action='store_true', help='Preview changes without writing files (implies --autofix mode).')
    parser.add_argument('-v', '--verbose', action='store_true', help='Detailed output with per-name breakdown and progress.')
    parser.add_argument('--workers', type=int, default=8, metavar='N', help='Number of parallel worker processes (default: 8).')
    parser.add_argument('--exclude', action='append', default=[], metavar='PATTERN', help='Regex pattern to exclude files/dirs (repeatable).')
...
Returns:
    ...
    return run(paths, autofix=args.autofix, dry_run=args.dry_run, verbose=args.verbose, workers=args.workers, exclude_patterns=exclude_patterns, ignore_init=args.ignore_init)
