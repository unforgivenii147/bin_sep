...
"""generate_setuppy.py – Generate Setuppy utilities.

This module provides functionality for generate setuppy."""
...
def read_text_if_exists(path: Path) -> str | None:
    """read_text_if_exists – read text if exists.
...
Args:
    toml_path: Description of toml_path.

Returns:
    ProjectFiles: Description of return value."""
    root = toml_path.parent
    return ProjectFiles(root=root, pyproject=toml_path, setup_py=root / 'setup.py', setup_cfg=root / 'setup.cfg', manifest=root / 'MANIFEST.in')
...
Returns:
    ...
    if isinstance(readme, dict):
        readme = dict(readme)
    return {'name': str(project.get('name', '')), 'version': str(project.get('version', '0.0.0')), 'description': str(project.get('description', '')), 'readme': readme, 'requires_python': str(project.get('requires-python', '')), 'license': str(license_value), 'authors': project.get('authors', []), 'maintainers': project.get('maintainers', []), 'keywords': project.get('keywords', []), 'classifiers': project.get('classifiers', []), 'urls': project.get('urls', {}), 'scripts': project.get('scripts', {}), 'gui_scripts': project.get('gui-scripts', {}), 'entry_points': project.get('entry-points', {}), 'dependencies': project.get('dependencies', []), 'optional_dependencies': project.get('optional-dependencies', {}), 'build_backend': build_system.get('build-backend', ''), 'build_requires': build_system.get('requires', []), 'tool': tool}

def parse_setup_cfg(path: Path) -> configparser.ConfigParser:
    """parse_setup_cfg – parse setup cfg.

Args:
    path: Description of path.

Returns:
    configparser.ConfigParser: Description of return value."""
...
def parse_cfg_mapping(parser: configparser.ConfigParser, section: str) -> dict[str, Any]:
    """parse_cfg_mapping – parse cfg mapping.
...
def readme_expression(readme: Any) -> tuple[str, str]:
    """readme_expression – readme expression.
...
Returns:
    ...
    if isinstance(readme, dict):
        ...
        if readme_file:
            return (f'read_text({readme_file!r})', repr(content_type))
        return (repr(readme.get('text', '')), repr(content_type))
    if isinstance(readme, str) and readme:
        suffix = Path(readme).suffix.lower()
        content_type = 'text/markdown' if suffix in MARKDOWN_SUFFIXES else 'text/x-rst' if suffix == '.rst' else 'text/plain'
        return (f'read_text({readme!r})', repr(content_type))
    return ("''", repr('text/plain'))

def author_values(people: Any) -> tuple[str, str]:
    """author_values – author values.
...
def detect_extensions(metadata: dict[str, Any]) -> tuple[list[dict[str, Any]], str]:
    """detect_extensions – detect extensions.

Args:
    metadata: Description of metadata.

Returns:
    tuple[list[dict[str, Any]], str]: Description of return value."""
    ...
    if isinstance(setuptools_tool, dict):
        ...
        if isinstance(raw_extensions, list):
            extensions = [ext for ext in raw_extensions if isinstance(ext, dict) and ext.get('name')]
    if extensions:
...
def build_package_arguments(metadata: dict[str, Any], cfg: configparser.ConfigParser) -> list[str]:
    """build_package_arguments – build package arguments.
...
Returns:
    ...
    if package_find:
        ...
        if find_call.endswith('('):
            ...
            find_call += ')'
        if exclude:
            find_call = f'find_packages(where={where!r}, exclude={python_literal(exclude)})'
        arguments.append(f'packages={find_call}')
    else:
        package_dir = setuptools_tool.get('package-dir', {}) if isinstance(setuptools_tool, dict) else {}
        if package_dir:
...
def build_entry_points(metadata: dict[str, Any]) -> dict[str, list[str]]:
    """build_entry_points – build entry points.
...
Returns:
    ...
    if metadata['scripts']:
        result['console_scripts'] = [f'{name} = {target}' for name, target in metadata['scripts'].items()]
    if metadata['gui_scripts']:
        result['gui_scripts'] = [f'{name} = {target}' for name, target in metadata['gui_scripts'].items()]
...
def generate_setup_py(metadata: dict[str, Any], cfg: configparser.ConfigParser, root: Path) -> str:
...
Returns:
    ...
    setup_args = [f"name={metadata['name']!r}", f"version={metadata['version']!r}", f"description={metadata['description']!r}", 'long_description=long_description', f'long_description_content_type={content_type}', f'author={authors!r}', f'author_email={author_email!r}', f'maintainer={maintainers!r}', f'maintainer_email={maintainer_email!r}', f"license={metadata['license']!r}", f'url={project_url!r}', f"keywords={python_literal(metadata['keywords'])}", f"classifiers={python_literal(metadata['classifiers'])}", f"python_requires={metadata['requires_python']!r}", f"install_requires={python_literal(metadata['dependencies'])}", f"extras_require={python_literal(metadata['optional_dependencies'])}", f'entry_points={python_literal(entry_points)}', 'include_package_data=True', *build_package_arguments(metadata, cfg)]
    if extensions:
        extension_literals = []
        for extension in extensions:
            extension_literals.append(f"Extension({extension['name']!r}, sources={python_literal(extension.get('sources', []))})")
...
Returns:
    ...
    parser.add_argument('toml_path', nargs='?', type=Path, default=Path('pyproject.toml'), help='Path to pyproject.toml.')
