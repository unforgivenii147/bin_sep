...
"""tcs.py – Tcs utilities.

This module provides functionality for tcs."""
...
def send_to_process(txt: str) -> None:
    """send_to_process – send to process.
...
def copy_lines_to_clipboard(path: str | Path, start_line: int | None=None, end_line: int | None=None) -> None:
    """copy_lines_to_clipboard – copy lines to clipboard.

Args:
    path: Description of path.
    start_line: Description of start_line.
    end_line: Description of end_line."""
    content = ''
    path = Path(path)
    if not path.is_file():
...
def main() -> None:
    ...
    if len(sys.argv) < 2 or len(sys.argv) > 5:
        ...
        print('  [start_line]: The first line number to copy (1-based index). If omitted, copies the entire file.', file=sys.stderr)
        print('  [end_line]: The last line number to copy (1-based index). If omitted and start_line is given, copies to the end of the file.', file=sys.stderr)
        sys.exit(1)
