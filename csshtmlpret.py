...
"""csshtmlpret.py – Csshtmlpret utilities.

This module provides functionality for csshtmlpret."""
...
def _compile_props(props_text: str, grouped: bool=False) -> tuple:
    """_compile_props –  compile props.

Args:
    props_text: Description of props_text.
...
def _prioritify(line_of_css: str, css_props_text_as_list: tuple) -> tuple:
    """_prioritify –  prioritify.

Args:
    line_of_css: Description of line_of_css.
    css_props_text_as_list: Description of css_props_text_as_list.
...
def _props_grouper(props: Any, pgs: Any) -> Any:
    """_props_grouper –  props grouper.

Args:
    props: Description of props.
    pgs: Description of pgs."""
    if not props:
        return props
...
Args:
    css_unsorted_string: Description of css_unsorted_string.
...
def remove_empty_rules(css: str) -> str:
    """remove_empty_rules – remove empty rules.
...
def condense_zero_units(css: str) -> str:
    """condense_zero_units – condense zero units.
...
def condense_semicolons(css: str) -> str:
    """condense_semicolons – condense semicolons.
...
def wrap_css_lines(css: str, line_length: int=80) -> str:
    """wrap_css_lines – wrap css lines.

Args:
    css: Description of css.
    line_length: Description of line_length.
...
def split_long_selectors(css: str) -> str:
    """split_long_selectors – split long selectors.
...
Args:
    css: Description of css.
    justify: Description of justify.
    extraline: Description of extraline.
...
if BeautifulSoup:
    ...
    def prettify(self, encoding: str | None=None, formatter: str='minimal', indent_width: int=4) -> str:
        """prettify – prettify.

Args:
    encoding: Description of encoding.
    formatter: Description of formatter.
    indent_width: Description of indent_width.
...
def walk2list(folder: str, target: tuple, omit: tuple=(), showhidden: bool=False, topdown: bool=True, onerror: object=None, followlinks: bool=False) -> list:
    """walk2list – walk2list.

Args:
    folder: Description of folder.
    target: Description of target.
    omit: Description of omit.
    showhidden: Description of showhidden.
    topdown: Description of topdown.
    onerror: Description of onerror.
    followlinks: Description of followlinks.

Returns:
    list: Description of return value."""
    return [p.resolve() for p in Path(folder).rglob('*') if p.is_file() and (not p.name.startswith(() if showhidden else '.')) and (not p.name.endswith(omit)) and p.name.endswith(target)]

def process_multiple_files(path: Path | str) -> None:
...
def prefixer_extensioner(path: str) -> str:
    """prefixer_extensioner – prefixer extensioner.
...
def process_single_css_file(css_path: str) -> str:
    """process_single_css_file – process single css file.
...
def process_single_html_file(html_path: str) -> str:
    """process_single_html_file – process single html file.
...
def make_arguments_parser() -> Namespace:
    """make_arguments_parser – make arguments parser.

Returns:
    Namespace: Description of return value."""
