...
"""wrapper_gh.py – Wrapper Gh utilities.

This module provides functionality for wrapper gh."""
...
def find_real_gh() -> Any:
    """find_real_gh – find real gh."""
    if os.path.isfile(REAL_GH) and os.access(REAL_GH, os.X_OK) and (os.path.realpath(REAL_GH) != os.path.realpath(__file__)):
        return REAL_GH
    path_dirs = os.environ.get('PATH', '').split(':')
    script_path = os.path.realpath(__file__)
    for path_dir in path_dirs:
        candidate = os.path.join(path_dir, 'gh')
        if os.path.isfile(candidate) and os.access(candidate, os.X_OK) and (os.path.realpath(candidate) != script_path):
            return candidate
    return None

def create_log_file() -> Any:
...
def write_log_header(log_file: Path | str, command_args: Any) -> None:
    """write_log_header – write log header.

Args:
    log_file: Description of log_file.
    command_args: Description of command_args."""
...
def write_log_footer(log_file: Path | str, exit_code: Any) -> None:
    """write_log_footer – write log footer.
