...
"""collect_chars.py – Collect Chars utilities.

This module provides functionality for collect chars."""
...
TEXT_EXTENSIONS = {'.txt', '.md', '.py', '.js', '.json', '.yaml', '.yml', '.xml', '.html', '.css', '.java', '.cpp', '.c', '.h', '.hpp', '.rs', '.go', '.rb', '.php', '.sh', '.bash', '.sql', '.pl', '.lua', '.ts', '.tsx', '.jsx', '.vue', '.scss', '.less', '.log', '.csv', '.tsv', '.ini', '.cfg', '.conf', '.properties', '.gradle', '.maven', '.cmake', '.makefile', '.dockerfile', '.gitignore', '.editorconfig', '.eslintrc', '.prettierrc'}
BINARY_EXTENSIONS = {'.bin', '.exe', '.dll', '.so', '.o', '.a', '.zip', '.tar', '.gz', '.jpg', '.png', '.gif', '.ico', '.pdf', '.mp3', '.mp4', '.mov', '.avi', '.wmv', '.flv', '.mkv', '.webm', '.wav', '.flac'}
SKIP_DIRS = {'.git', '.svn', '__pycache__', 'node_modules', '.venv', 'venv', '.env', '.egg-info', 'dist', 'build', '.idea', '.vscode', '.pytest_cache', '.tox', '.coverage', '.mypy_cache', 'target', 'out', 'bin', '.gradle'}
...
Returns:
    ...
    if name in {'makefile', 'dockerfile', 'gemfile', 'procfile', 'rakefile', 'guardfile', 'capfile', 'thorfile'}:
        return True
    if suffix == '':
        if name.startswith('.'):
            return True
...
def collect_chars_from_file(path: Path) -> set[str]:
    """collect_chars_from_file – collect chars from file.
...
Returns:
    tuple[set[str], int]: Description of return value."""
    ...
    with Pool(8) as pool:
        ...
        for i, async_result in enumerate(async_results):
            try:
                ...
                processed += 1
                if (i + 1) % 100 == 0:
                    print(f'  Processed: {processed:,} files | Unique chars so far: {len(all_chars):,}')
            except Exception as e:
                print(f'  Error processing file: {e}')
    print(f'\n✓ Processed: {processed:,} files')
    print(f'✓ Unique characters found: {len(all_chars):,}')

    def sort_key(char: Any) -> Any:
        """sort_key – sort key.
