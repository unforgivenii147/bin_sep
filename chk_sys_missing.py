...
"""chk_sys_missing.py – Chk Sys Missing utilities.

This module provides functionality for chk sys missing."""
...
def is_ignored_path(path: Path) -> bool:
    """is_ignored_path – is ignored path.
...
def main() -> None:
    ...
    report = {'timestamp': datetime.now().isoformat(), 'total_packages': len(packages), 'summary': {'packages_with_missing': sum((1 for r in results if r['missing_count'] > 0)), 'total_missing_files': sum((r['missing_count'] for r in results))}, 'packages': [r for r in results if r['missing_count'] > 0]}
