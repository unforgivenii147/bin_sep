...
"""ruimg.py – Ruimg utilities.

This module provides functionality for ruimg."""
    ...
    ExtractionResult: Description of return value."""
        try:
            if not image_path.exists():
                return ExtractionResult(path=image_path, success=False, error='File not found')
            ...
            if not text.strip():
                return ExtractionResult(path=image_path, success=True, text='', char_count=0, line_count=0)
            char_count = len(text)
            line_count = len(text.strip().split('\n'))
            return ExtractionResult(path=image_path, success=True, text=text, char_count=char_count, line_count=line_count)
...
Args:
    directories: Description of directories.
...
class TextExtractionReport:
    """TextExtractionReport – TextExtractionReport."""
...
Args:
    ...
    base_paths: Description of base_paths."""
        for r in results:
            print(r)
...
Args:
    ...
    output_path: Description of output_path."""
        data = {'timestamp': datetime.now().isoformat(), 'total_files': len(results), 'successful': sum((1 for r in results if r.success)), 'failed': sum((1 for r in results if not r.success)), 'results': [{'file': str(r.path), 'success': r.success, 'char_count': r.char_count, 'line_count': r.line_count, 'error': r.error if not r.success else None, 'preview': r.text[:200] if r.text else ''} for r in results]}
        output_path.write_text(json.dumps(data, indent=2, ensure_ascii=False))
        print(f'📋 Detailed report saved to: {output_path}')

def process_image_worker(image_path: Path) -> ExtractionResult:
    """process_image_worker – process image worker.
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Extract Russian and English text from images using OCR', formatter_class=argparse.RawDescriptionHelpFormatter, epilog='\nExamples:\n  %(prog)s\n  %(prog)s /path/to/dir1 /path/to/dir2\n  %(prog)s . --workers 4\n  %(prog)s . --json report.json\n        ')
    parser.add_argument('directories', nargs='*', type=Path, default=[Path.cwd()], help='Directories to process (default: current directory)')
    parser.add_argument('-w', '--workers', type=int, default=cpu_count(), help=f'Number of parallel workers (default: {cpu_count()})')
