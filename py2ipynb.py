...
"""py2ipynb.py – Py2Ipynb utilities.

This module provides functionality for py2ipynb."""
...
def py_to_ipynb(input_file: Path | str, output_file: Path | str | None=None) -> bool:
    """py_to_ipynb – py to ipynb.
...
Returns:
    ...
    while i < len(lines):
        line = lines[i]
        if i > 0 and (line.startswith(('def ', 'class ')) or (line.startswith(('import ', 'from ')) and (not current_cell[-1].startswith(('import ', 'from ')))) or (line.strip() == '' and current_cell and (i + 1 < len(lines)) and lines[i + 1].strip() and (not lines[i + 1].startswith((' ', '\t'))))) and current_cell:
