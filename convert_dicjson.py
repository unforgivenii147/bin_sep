...
"""convert_dicjson.py – Convert Dicjson utilities.

This module provides functionality for convert dicjson."""
...
def process_file(json_file: Path | str) -> Any:
...
def main() -> None:
    """main – main."""
    cwd = Path.cwd()
    json_files = [p for p in cwd.glob('*.json') if p.name != f'{cwd.name}.json']
    if not json_files:
