...
"""normalize_print_sep.py – Normalize Print Sep utilities.

This module provides functionality for normalize print sep."""
...
def normalize_separators(content: str) -> tuple[str, int]:
    """normalize_separators – normalize separators.
...
Returns:
    ...
    pattern = '(cprint|print)\\s*\\(\\s*[\'\\"](.)[\'\\"](\\s*\\*\\s*)(\\d+)([^)]*)\\)'
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Normalize print separators in Python files.', epilog='Examples:\n  %(prog)s                  # Dry-run current directory\n  %(prog)s -a src/          # Autofix files in src/ directory\n  %(prog)s --autofix file.py other.py  # Autofix specific files', formatter_class=argparse.RawDescriptionHelpFormatter)
