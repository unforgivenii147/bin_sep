...
"""mendic.py – Mendic utilities.

This module provides functionality for mendic."""
...
Args:
    ...
    persian_values = sum((1 for v in sample_values if isinstance(v, str) and is_persian_word(v)))
    is_correct = english_keys >= len(sample_keys) * 0.5 and persian_values >= len(sample_values) * 0.5
    return (is_correct, sample_keys, sample_values)

def merge_json_files(directory: str='.', output_file: str='enfa.json') -> None:
    ...
    for json_file in json_files:
        try:
            ...
            if not is_correct_format:
                print(f'❌ Skipping {json_file.name}: Wrong format (Persian keys detected)')
            ...
            for english_word, persian_translation in data.items():
                ...
                if english_word in merged_data:
                    ...
                    duplicate_keys[english_word].append({'existing': merged_data[english_word], 'duplicate': persian_translation, 'file': json_file.name})
                else:
