...
"""pkgs_with_entry_points.py – Pkgs With Entry Points utilities.

This module provides functionality for pkgs with entry points."""
...
Returns:
    ...
    common_paths = [Path(sys.prefix) / 'lib' / f'python{sys.version_info.major}.{sys.version_info.minor}' / 'site-packages', Path(sys.prefix) / 'local' / 'lib' / f'python{sys.version_info.major}.{sys.version_info.minor}' / 'site-packages']
...
def get_package_name_from_path(path: Path) -> str:
    """get_package_name_from_path – get package name from path.
...
def is_pure_python_package(pkg_name: str, site_dir: Path) -> bool:
    """is_pure_python_package – is pure python package.

Args:
    pkg_name: Description of pkg_name.
    site_dir: Description of site_dir.

Returns:
    bool: Description of return value."""
    try:
        dist_info_patterns = [f'{pkg_name}*.dist-info', f"{pkg_name.replace('-', '_')}*.dist-info", f"{pkg_name.replace('_', '-')}*.dist-info"]
                ...
                if dist_info.is_dir():
                    ...
                    if record_file.exists():
                        try:
                            ...
                            pass
        egg_info_patterns = [f'{pkg_name}*.egg-info', f"{pkg_name.replace('-', '_')}*.egg-info", f"{pkg_name.replace('_', '-')}*.egg-info"]
        ...
        for pattern in package_dir_patterns:
            for package_dir in site_dir.glob(pattern):
                if package_dir.is_dir() and (not package_dir.name.endswith('.dist-info')) and (not package_dir.name.endswith('.egg-info')):
...
def parse_entry_points(entry_points_file: Path) -> dict[str, list[str]]:
    """parse_entry_points – parse entry points.

Args:
    entry_points_file: Description of entry_points_file.
...
Args:
    package_path: Description of package_path.
    site_dir: Description of site_dir.

Returns:
    dict[str, any]: Description of return value."""
    pkg_name = get_package_name_from_path(package_path)
    result = {'name': pkg_name, 'has_entry_points': False, 'entry_points_data': None, 'is_pure_python': True, 'error': None}
    try:
        entry_points_file = None
        dist_info_patterns = [f'{pkg_name}*.dist-info', f"{pkg_name.replace('-', '_')}*.dist-info", f"{pkg_name.replace('_', '-')}*.dist-info", f'{package_path.name}*.dist-info']
        for pattern in dist_info_patterns:
            for dist_info in site_dir.glob(pattern):
                if dist_info.is_dir():
        ...
        if not entry_points_file:
            egg_info_patterns = [f'{pkg_name}*.egg-info', f"{pkg_name.replace('-', '_')}*.egg-info", f"{pkg_name.replace('_', '-')}*.egg-info", f'{package_path.name}*.egg-info']
            for pattern in egg_info_patterns:
                for egg_info in site_dir.glob(pattern):
                    if egg_info.is_dir():
...
def find_packages_categorized(site_dir: Path) -> tuple[list[str], list[str], list[str], list[str]]:
    """find_packages_categorized – find packages categorized.

Args:
    site_dir: Description of site_dir.

Returns:
    tuple[list[str], list[str], list[str], list[str]]: Description of return value."""
    pure_without_ep = []
    nonpure_without_ep = []
    pure_with_ep = []
    nonpure_with_ep = []
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Find Python packages and categorize by entry_points.txt and purity (Linux/Termux optimized)')
    parser.add_argument('--pure-noep-output', default='noep_pure.txt', help='Output file for pure Python packages without entry_points (default: noep_pure.txt)')
    parser.add_argument('--nonpure-noep-output', default='noep_nopure.txt', help='Output file for non-pure packages without entry_points (default: noep_nopure.txt)')
    parser.add_argument('--pure-ep-output', default='ep_pure.txt', help='Output file for pure Python packages with entry_points (default: ep_pure.txt)')
    parser.add_argument('--nonpure-ep-output', default='ep_nopure.txt', help='Output file for non-pure packages with entry_points (default: ep_nopure.txt)')
    parser.add_argument('--ep-details-output', default='ep_details.txt', help='Output file for detailed entry_points information (default: ep_details.txt)')
    parser.add_argument('-j', '--json', action='store_true', help='Output in JSON format')
    parser.add_argument('-p', '--processes', type=int, default=None, help=f'Number of processes to use (default: {cpu_count()})')
...
Args:
    ...
    if args.json:
        data = {'timestamp': datetime.now().isoformat(), 'site_directories': [str(d) for d in site_dirs], 'platform': sys.platform, 'total': len(unique_pure_noep), 'packages': unique_pure_noep}
        Path(args.pure_noep_output).write_text(json.dumps(data, indent=2))
    else:
        Path(args.pure_noep_output).write_text('\n'.join(unique_pure_noep))
    if args.json:
        data = {'timestamp': datetime.now().isoformat(), 'site_directories': [str(d) for d in site_dirs], 'platform': sys.platform, 'total': len(unique_nonpure_noep), 'packages': unique_nonpure_noep}
    ...
    if args.json:
        data = {'timestamp': datetime.now().isoformat(), 'site_directories': [str(d) for d in site_dirs], 'platform': sys.platform, 'total': len(unique_pure_ep), 'packages': unique_pure_ep}
        Path(args.pure_ep_output).write_text(json.dumps(data, indent=2))
    else:
        Path(args.pure_ep_output).write_text('\n'.join(unique_pure_ep))
    if args.json:
        data = {'timestamp': datetime.now().isoformat(), 'site_directories': [str(d) for d in site_dirs], 'platform': sys.platform, 'total': len(unique_nonpure_ep), 'packages': unique_nonpure_ep}
    ...
    if args.json:
        data = {'timestamp': datetime.now().isoformat(), 'site_directories': [str(d) for d in site_dirs], 'platform': sys.platform, 'total': len(unique_ep_details), 'packages': unique_ep_details}
    ...
    print()
    print(f'Grand total: {len(unique_pure_noep) + len(unique_nonpure_noep) + len(unique_pure_ep) + len(unique_nonpure_ep)}')
    print(f"{'=' * 40}")
    if args.verbose:
        print('\nOutput files:')
...
def scan_for_entry_points(site_dir: Path) -> list[dict]:
    """scan_for_entry_points – scan for entry points.
