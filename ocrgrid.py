...
"""ocrgrid.py – Ocrgrid utilities.

This module provides functionality for ocrgrid."""
...
def main() -> None:
    ...
    image_variants: dict[str, Image.Image] = {'original': base_img, 'grayscale': cv_to_pil(to_grayscale(cv_img)), 'rescaled': cv_to_pil(rescale(cv_img)), 'deskewed': cv_to_pil(deskew(cv_img)), 'rotated_90': cv_to_pil(rotate(cv_img, 90))}
    psm_values = [3, 4, 6, 11]
    oem_values = [1, 3]
    dpi_values = [150, 300]
        ...
        for psm in psm_values:
            for oem in oem_values:
                for dpi in dpi_values:
                    ...
                    meta_path.write_text(json.dumps({'image_variant': variant_name, 'source_file': str(args.fname), 'tesseract': result}, indent=2), encoding='utf-8')
                    report_index.append({'variant': variant_name, 'psm': psm, 'oem': oem, 'dpi': dpi, 'text_file': str(txt_path)})
