...
"""dtransline_chinese.py – Dtransline Chinese utilities.

This module provides functionality for dtransline chinese."""
...
CHINESE_PATTERN: Final[re.Pattern] = re.compile('[\\u4e00-\\u9fff\\u3400-\\u4dbf\\u20000-\\u2a6df\\u2a700-\\u2b73f\\u2b740-\\u2b81f\\u2b820-\\u2ceaf\\uf900-\\ufaff]')

def is_chinese_text(text: str, threshold: float=0.3) -> bool:
    """is_chinese_text – is chinese text.
...
Returns:
    ...
    stats = {'file': str(path), 'total_lines': 0, 'chinese_lines': 0, 'translated_lines': 0, 'errors': 0}
            ...
            if is_chinese_text(line, threshold):
                ...
                if dry_run:
                    ...
                    if stats['translated_lines'] % 10 == 0:
                        print('  Progress: %d Chinese lines translated', stats['translated_lines'])
            else:
                new_lines.append(line)
...
def main() -> None:
    ...
    parser.add_argument('--extensions', '-e', nargs='+', default=['.txt', '.md', '.py', '.js', '.html', '.css', '.json', '.xml', '.csv'], help='Extensions to process')
    parser.add_argument('--workers', '-w', type=int, default=mp.cpu_count(), help=f'Number of workers (default: {mp.cpu_count()})')
    ...
    if not files_to_process:
        ...
        return
    print('Found %d files. Using %d workers (Threshold: %.0f%%)', len(files_to_process), args.workers, args.threshold * 40)
