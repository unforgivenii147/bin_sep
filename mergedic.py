...
"""mergedic.py – Mergedic utilities.

This module provides functionality for mergedic."""
...
ENGLISH_RE = re.compile("^[A-Za-z][A-Za-z\\s\\-'.]*$")

def looks_english(text: str) -> bool:
    """looks_english – looks english.
...
def detect_direction(data: dict, path: Path) -> str:
    ...
    if ratio <= 0.1:
        return 'fa'
    raise ValueError(f'{path} has mixed key languages ({en_count}/{total} English-looking keys). Refusing to merge ambiguous file.')
...
def normalize_value(value: str) -> list:
...
def main() -> None:
    ...
    for path in input_paths:
        ...
        if direction is None:
            ...
            msg = f"{path.name} looks like a '{file_dir}' dict but we already committed to '{direction}' (from {direction_source.name}). Skipping."
            logger.error(msg)
    ...
    logger.success(f'Merged {merged_files} file(s) (direction: {direction}) → {output_path} ({len(merged)} unique keys)')
