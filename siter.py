...
"""siter.py – Siter utilities.

This module provides functionality for siter."""
...
class WheelBuilder:
    ...
    def __init__(self, site_packages: Path, output_dir: Path, force_all: bool=False, parallel: bool=True, max_workers: int | None=None) -> None:
        """__init__ –   init  .

Args:
    site_packages: Description of site_packages.
    output_dir: Description of output_dir.
    force_all: Description of force_all.
    parallel: Description of parallel.
    ...
    def _find_venv_root(self) -> Path | None:
        """_find_venv_root –  find venv root.
...
Returns:
    ...
    def _read_record(self, dist_info: Path) -> dict[str, dict[str, str]]:
        """_read_record –  read record.

Args:
    dist_info: Description of dist_info.

Returns:
    dict[str, dict[str, str]]: Description of return value."""
    ...
    def _find_scripts_for_package(self, records: dict) -> list[Path]:
        """_find_scripts_for_package –  find scripts for package.
...
Returns:
    ...
    def _find_data_for_package(self, package_name: str) -> list[tuple[Path, str]]:
        """_find_data_for_package –  find data for package.

Args:
    package_name: Description of package_name.

Returns:
    list[tuple[Path, str]]: Description of return value."""
    ...
    def _get_wheel_tags(self) -> tuple[str, str, str]:
        """_get_wheel_tags –  get wheel tags.
...
Returns:
    ...
    def _build_wheel_worker(self, dist_info: Path) -> tuple[str, Path | None]:
        """_build_wheel_worker –  build wheel worker.

Args:
    dist_info: Description of dist_info.

Returns:
    tuple[str, Path | None]: Description of return value."""
        try:
...
Returns:
    ...
    def _build_serial(self, dist_infos: list[Path]) -> int:
        """_build_serial –  build serial.

Args:
    dist_infos: Description of dist_infos.
...
Returns:
    ...
    def _build_parallel(self, dist_infos: list[Path]) -> int:
        """_build_parallel –  build parallel.
...
Returns:
    ...
    parser.add_argument('--workers', '-w', type=int, default=None, help='Number of worker processes (default: CPU count, max 8)')
    parser.add_argument('--site-packages', '-s', type=Path, help='Path to site-packages directory (default: current directory)')
