...
"""trydecomp.py – Trydecomp utilities.

This module provides functionality for trydecomp."""
...
def try_decompress(filename: str) -> None:
    """try_decompress – try decompress.

Args:
    filename: Description of filename."""
    print(f'Attempting to decompress: {filename}\n')
    compression_methods = {'zlib': zlib.decompress, 'bz2': bz2.decompress, 'gzip': gzip.decompress, 'lzma': lzma.decompress, 'pickle': pickle.loads}
    if brotli:
        compression_methods['brotli'] = brotli.decompress
    if zstd_available:

        def zstd_decompress_all(data: str) -> bytes:
            """zstd_decompress_all – zstd decompress all.
    ...
    if tarfile.is_tarfile(filename):
        try:
            ...
            with tarfile.open(filename, 'r') as tar:
                ...
                if members:
                    print(f'  SUCCESS: Opened as tar archive with {len(members)} members. First member: {members[0].name}\n')
                    success = True
                else:
