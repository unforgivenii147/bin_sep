...
"""no_entry_point.py – No Entry Point utilities.

This module provides functionality for no entry point."""
...
def find_packages_without_entry_points(site_dir: Path) -> tuple[list[str], list[str]]:
    """find_packages_without_entry_points – find packages without entry points.

Args:
    site_dir: Description of site_dir.

Returns:
    tuple[list[str], list[str]]: Description of return value."""
    pure_packages = []
    non_pure_packages = []
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Find Python packages without entry_points.txt in system site directories (Linux/Termux optimized)')
    parser.add_argument('--pure-output', default='noep_pure.txt', help='Output file for pure Python packages (default: noep_pure.txt)')
    parser.add_argument('--nonpure-output', default='noep_nopure.txt', help='Output file for non-pure packages (default: noep_nopure.txt)')
    ...
    unique_nonpure.sort()
    if args.json:
        pure_data = {'timestamp': datetime.now().isoformat(), 'site_directories': [str(d) for d in site_dirs], 'platform': sys.platform, 'total': len(unique_pure), 'packages': unique_pure}
        Path(args.pure_output).write_text(json.dumps(pure_data, indent=2))
    else:
        Path(args.pure_output).write_text('\n'.join(unique_pure))
    if args.json:
        nonpure_data = {'timestamp': datetime.now().isoformat(), 'site_directories': [str(d) for d in site_dirs], 'platform': sys.platform, 'total': len(unique_nonpure), 'packages': unique_nonpure}
