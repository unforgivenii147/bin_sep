...
"""cf2mp4.py – Cf2Mp4 utilities.

This module provides functionality for cf2mp4."""
...
FUTURE_RESULT_RECEIVERS = {'future', 'f', 'fut', 'done', 'system_future', 'user_future', 'future1', 'future2', 'result_future'}
...
def position_to_index(source: str, offsets: list[int], lineno: int, byte_column: int) -> int:
    """position_to_index – position to index.

Args:
    source: Description of source.
    offsets: Description of offsets.
    lineno: Description of lineno.
    byte_column: Description of byte_column.
...
Returns:
    ...
    return (position_to_index(source, offsets, node.lineno, node.col_offset), position_to_index(source, offsets, node.end_lineno, node.end_col_offset))

def has_top_level_pool_binding(tree: ast.Module) -> bool:
    """has_top_level_pool_binding – has top level pool binding.
...
Returns:
    ...
    for node in tree.body:
        if isinstance(node, ast.ImportFrom) and node.module in {'multiprocessing', 'multiprocessing.pool'} and any(((alias.asname or alias.name) == 'Pool' for alias in node.names)):
            return True
        if isinstance(node, ast.Import) and any(((alias.asname or alias.name) == 'Pool' for alias in node.names)):
            return True
...
def top_level_max_workers(tree: ast.Module) -> list[ast.AST]:
    """top_level_max_workers – top level max workers.

Args:
    tree: Description of tree.

Returns:
    list[ast.AST]: Description of return value."""
...
def import_end_index(source: str, offsets: list[int], tree: ast.Module) -> int:
    """import_end_index – import end index.
...
def is_concurrent_attribute(node: ast.AST, attribute: str) -> bool:
    """is_concurrent_attribute – is concurrent attribute.
...
Returns:
    bool: Description of return value."""
    return isinstance(node, ast.Attribute) and node.attr == attribute and isinstance(node.value, ast.Attribute) and (node.value.attr == 'futures') and isinstance(node.value.value, ast.Name) and (node.value.value.id == 'concurrent')

def build_submit_replacement(call: ast.Call) -> str | None:
    """build_submit_replacement – build submit replacement.
...
def build_map_replacement(call: ast.Call) -> str | None:
    """build_map_replacement – build map replacement.
...
Args:
    edits: Description of edits.
    edit: Description of edit."""
...
Returns:
    tuple[str, list[str], bool]: Description of return value."""
...
def has_executor_import(source: str) -> bool:
    """has_executor_import – has executor import.
