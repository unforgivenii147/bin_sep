...
"""Mechanically migrate direct concurrent.futures executor calls to Pool.apply_async."""
...
def position_to_index(source: str, offsets: list[int], lineno: int, byte_column: int) -> int:
...
def has_top_level_pool_binding(tree: ast.Module) -> bool:
    for node in tree.body:
        if isinstance(node, ast.ImportFrom) and node.module in {"multiprocessing", "multiprocessing.pool"}:
            if any((alias.asname or alias.name) == "Pool" for alias in node.names):
                return True
        if isinstance(node, ast.Import):
            if any((alias.asname or alias.name) == "Pool" for alias in node.names):
                return True
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)) and node.name == "Pool":
            return True
    return False
...
def import_end_index(source: str, offsets: list[int], tree: ast.Module) -> int:
    ...
    if not imports:  # Preserve a shebang and encoding declaration if the file has either.
...
def add_edit(edits: list[Edit], edit: Edit) -> None:
    """Append a non-overlapping edit, preferring the larger containing edit."""
...
def rewrite_source(source: str) -> tuple[str, list[str], bool]:
  ...
  for node in tree.body:
    if isinstance(node, ast.ImportFrom) and node.module == "concurrent.futures":
      executor_aliases = [ alias.asname or alias.name for alias in node.names if alias.name in EXECUTOR_CLASS_NAMES ]
      helper_aliases = [ alias.asname or alias.name for alias in node.names if alias.name in HELPER_NAMES ]
      ...
      helpers_to_import.update(alias.name for alias in node.names if alias.name in HELPER_NAMES)
      replacement_lines: list[str] = []

      if executor_aliases:
        replacement_lines.append("from multiprocessing.pool import Pool")
      if helper_aliases:
        imported = ", ".join( f"{alias.name} as {alias.asname}" if alias.asname else alias.name for alias in node.names if alias.name in HELPER_NAMES )
        replacement_lines.append(f"from pool_helpers import {imported}")
        start, end = node_span(source, offsets, node)
        add_edit(edits, Edit(start, end, "\n".join(replacement_lines), "replace futures import"))
      ...
      if concurrent_aliases:
        ...
        start, end = node_span(source, offsets, node) 
        add_edit( edits, Edit( start, end, "from multiprocessing.pool import Pool\nfrom pool_helpers import as_completed", "replace futures module import", ), )
        requires_pool = True
      ...
      if pool_already_bound:
        for edit in list(edits):
          if edit.reason == "replace futures import" and edit.replacement.startswith("from multiprocessing.pool import Pool\n"):
    ...
    if isinstance(node, ast.With):
      for item in node.items:
        if isinstance(item.context_expr, ast.Call):
          ...
          is_executor = ( isinstance(func, ast.Name) and func.id in imported_executor_names ) or any(is_concurrent_attribute(func, name) for name in EXECUTOR_CLASS_NAMES)
        if is_executor and isinstance(item.optional_vars, ast.Name):
          ...
          for item in node.items:
            if isinstance(item.context_expr, ast.Call) and isinstance(item.optional_vars, ast.Name):
              func = item.context_expr.func
            if isinstance(func, ast.Name) and func.id in imported_executor_names:
              pool_variables.add(item.optional_vars.id)
  ...
  for node in ast.walk(tree):
    if not isinstance(node, ast.Call):
      continue
    func = node.func
    is_executor_constructor = ( isinstance(func, ast.Name) and func.id in imported_executor_names ) or any(is_concurrent_attribute(func, name) for name in EXECUTOR_CLASS_NAMES)
    if is_executor_constructor:
      start, end = node_span(source, offsets, node)
      add_edit( edits, Edit(start, end, "Pool(processes=MAX_WORKERS)", "fixed pool constructor"), )
      requires_pool = True
      continue
    if is_concurrent_attribute(func, "as_completed"):
      start, end = node_span(source, offsets, func)
      add_edit(edits, Edit(start, end, "as_completed", "unqualify completion helper"))
      helpers_to_import.add("as_completed")
      continue
    ...
    if func.attr == "result" and receiver in FUTURE_RESULT_RECEIVERS:
      ...
      start, end = node_span(source, offsets, node) add_edit(edits, Edit(start, end, "None", "Pool jobs cannot be cancelled"))
  if requires_pool:
  ...
  if assignments:
    ...
    insertion = import_end_index(source, offsets, tree) add_edit(edits, Edit(insertion, insertion, "\nMAX_WORKERS = 8", "insert worker constant")) # If an import replacement references Pool but an existing binding was not # present, the replacement already provides it. Files using a direct module # import always receive it above. if not edits: return source, [], requires_pool output = source for edit in sorted(edits, key=lambda current: (current.start, current.end), reverse=True): output = output[: edit.start] + edit.replacement + output[edit.end :] return output, [edit.reason for edit in edits], requires_pool


def has_executor_import(source: str) -> bool: tree = ast.parse(source) for node in tree.body: if isinstance(node, ast.ImportFrom) and node.module == "concurrent.futures": if any(alias.name in EXECUTOR_CLASS_NAMES for alias in node.names): return True if isinstance(node, ast.Import) and any(alias.name == "concurrent.futures" for alias in node.names): return True return False
"""
...
def rewrite_source(source: str) -> tuple[str, list[str], bool]:
    ...
    for node in tree.body:
        if isinstance(node, ast.ImportFrom) and node.module == "concurrent.futures":
            executor_aliases = [
                alias.asname or alias.name for alias in node.names if alias.name in EXECUTOR_CLASS_NAMES
            ]
            helper_aliases = [alias.asname or alias.name for alias in node.names if alias.name in HELPER_NAMES]
            imported_executor_names.update(executor_aliases)
            imported_helper_names.update(helper_aliases)
        ...
        if pool_already_bound:
            for edit in list(edits):
                if edit.reason == "replace futures import" and edit.replacement.startswith(
                    "from multiprocessing.pool import Pool\n"
                ):
                    replacement = edit.replacement.replace("from multiprocessing.pool import Pool\n", "", 1)
                    edits.remove(edit)
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.With):
            for item in node.items:
                if isinstance(item.context_expr, ast.Call):
                    func = item.context_expr.func
                    is_executor = (isinstance(func, ast.Name) and func.id in imported_executor_names) or any(
                        is_concurrent_attribute(func, name) for name in EXECUTOR_CLASS_NAMES
                    )
                    if is_executor and isinstance(item.optional_vars, ast.Name):
                        pool_variables.add(item.optional_vars.id)
        elif isinstance(node, ast.AsyncWith):
    ...
    for node in ast.walk(tree):
        ...
        func = node.func
        is_executor_constructor = (isinstance(func, ast.Name) and func.id in imported_executor_names) or any(
            is_concurrent_attribute(func, name) for name in EXECUTOR_CLASS_NAMES
        )
        ...
        if is_executor_constructor:
            ...
            add_edit(
                edits,
                Edit(start, end, "Pool(processes=MAX_WORKERS)", "fixed pool constructor"),
            )
            requires_pool = True
            continue
    ...
    if requires_pool:
        ...
        if assignments:
            ...
            add_edit(edits, Edit(insertion, insertion, "\nMAX_WORKERS = 8", "insert worker constant"))

    if not edits:
        return source, [], requires_pool

    output = source
    for edit in sorted(edits, key=lambda current: (current.start, current.end), reverse=True):
    ...
    for node in tree.body:
        if isinstance(node, ast.ImportFrom) and node.module == "concurrent.futures":
            if any(alias.name in EXECUTOR_CLASS_NAMES for alias in node.names):
                return True
        if isinstance(node, ast.Import) and any(alias.name == "concurrent.futures" for alias in node.names):
            return True
    return False
