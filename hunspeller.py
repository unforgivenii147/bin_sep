...
def process_line(line: str, autofix: bool = False) -> tuple:
    """
    Process a single line for spelling errors using Hunspell.
    Returns: (updated_line, misspelled_count, fixed_count, suggestions_dict)
    """
    # Initialize Hunspell for each process
...
def process_file(filepath: str, autofix: bool = False, num_processes: int = None, 
    ...
    """
    Process a file for spelling errors using Hunspell with multiprocessing.
    
    Args:
        filepath: Path to the text file
        autofix: Whether to automatically correct misspelled words
        num_processes: Number of processes to use (defaults to CPU count)
        dic_path: Path to Hunspell dictionary file
    ...
    # Verify Hunspell dictionary files exist
    import os
    ...
    if autofix:
        if total_fixed > 0:
            try:
                ...
                if total_misspelled > total_fixed:
                    ...
                    # Show skipped words with no suggestions
                    no_suggestion_words = [word for word, suggs in all_suggestions.items() 
        ...
        if total_misspelled == 0:
            ...
            # Sort suggestions by word for better readability
...
def find_hunspell_dicts():
    """Find available Hunspell dictionaries on the system."""
    import os
    import glob
...
if __name__ == "__main__":
    ...
    # List available dictionaries if requested
    if args.list_dicts:
    ...
    if args.dictionary and args.affix:
        ...
        # Try to find first available dictionary
        dicts = find_hunspell_dicts()
        if dicts:
            dic_path, aff_path = dicts[0]  # Use first available
        else:
            # Default paths
