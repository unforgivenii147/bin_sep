...
"""hunspeller.py – Hunspeller utilities.

This module provides functionality for hunspeller."""
...
Returns:
    ...
    def check_and_replace(match: Any) -> bool:
        """check_and_replace – check and replace.
    ...
    match: Description of match."""
        ...
        if not h.spell(clean_word):
            ...
            if autofix and suggestions:
                ...
                return word
        return word
    updated_line = re.sub("[a-zA-Z']+", check_and_replace, line)
    return (updated_line, misspelled_count, fixed_count, suggestions_dict)

def process_file(path: str, autofix: bool=False, num_processes: int | None=None, dic_path: str='/usr/share/hunspell/en_US.dic', aff_path: str='/usr/share/hunspell/en_US.aff') -> None:
...
Args:
    ...
    dic_path: Description of dic_path.
    aff_path: Description of aff_path."""
    try:
        ...
        if total_fixed > 0:
            try:
                ...
                if total_misspelled > total_fixed:
                    ...
                    no_suggestion_words = [word for word, suggs in all_suggestions.items() if suggs == ['No suggestions']]
...
def find_hunspell_dicts() -> Any:
    """find_hunspell_dicts – find hunspell dicts."""
    import glob
    import os
    common_paths = ['/usr/share/hunspell/', '/usr/share/myspell/', '/usr/local/share/hunspell/', '/data/data/com.termux/files/usr/share/hunspell/']
    found_dicts = []
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Detect and optionally autofix misspelled words in a file using Hunspell with parallel processing.', epilog='Example: %(prog)s file.txt -a -p 4')
    parser.add_argument('file', help='Path to the text file to check')
    parser.add_argument('-a', '--autofix', action='store_true', help='Automatically correct misspelled words in the file')
    parser.add_argument('-p', '--processes', type=int, default=None, help='Number of processes to use (defaults to CPU count)')
    parser.add_argument('-d', '--dictionary', help='Path to Hunspell dictionary file (.dic)')
    parser.add_argument('--affix', help='Path to Hunspell affix file (.aff)')
    parser.add_argument('--list-dicts', action='store_true', help='List available Hunspell dictionaries and exit')
