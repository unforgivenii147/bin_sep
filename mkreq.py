...
"""mkreq.py – Mkreq utilities.

This module provides functionality for mkreq."""
...
def extract_imports(path: Path) -> Any:
...
def get_local_modules(cwd: str='.') -> Any:
...
def collect_requirements(cwd: str='.', exclude_dirs: Path | str | None=None, verbose: bool=False) -> Any:
...
Args:
    ...
    if exclude_dirs is None:
        exclude_dirs = {'.venv', 'venv', '__pycache__', '.git', '.pytest_cache', 'env', '.env', 'node_modules', '.idea', 'build', 'dist'}
    root = Path(cwd)
    all_imports = set()
...
def write_requirements(packages: Any, output_file: str='requirements.txt') -> Path:
    """write_requirements – write requirements.
