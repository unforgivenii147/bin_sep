...
IMPORT_HINTS: dict[str, str] = {'List': 'from typing import List', 'Dict': 'from typing import Dict', 'Optional': 'from typing import Optional', 'Tuple': 'from typing import Tuple', 'Set': 'from typing import Set', 'Any': 'from typing import Any', 'Union': 'from typing import Union', 'Callable': 'from typing import Callable', 'Type': 'from typing import Type', 'ClassVar': 'from typing import ClassVar', 'Final': 'from typing import Final', 'Literal': 'from typing import Literal', 'Generator': 'from typing import Generator', 'Iterator': 'from typing import Iterator', 'Iterable': 'from typing import Iterable', 'Sequence': 'from typing import Sequence', 'Mapping': 'from typing import Mapping', 'TypeVar': 'from typing import TypeVar', 'overload': 'from typing import overload', 'cast': 'from typing import cast', 'TYPE_CHECKING': 'from typing import TYPE_CHECKING', 'dataclass': 'from dataclasses import dataclass', 'field': 'from dataclasses import field', 'Path': 'from pathlib import Path', 'datetime': 'from datetime import datetime', 'date': 'from datetime import date', 'timedelta': 'from datetime import timedelta', 're': 'import re', 'json': 'import json', 'os': 'import os', 'sys': 'import sys', 'math': 'import math', 'time': 'import time', 'copy': 'import copy', 'abc': 'import abc', 'ABC': 'from abc import ABC', 'abstractmethod': 'from abc import abstractmethod', 'functools': 'import functools', 'itertools': 'import itertools', 'collections': 'import collections', 'defaultdict': 'from collections import defaultdict', 'OrderedDict': 'from collections import OrderedDict', 'Counter': 'from collections import Counter', 'deque': 'from collections import deque', 'namedtuple': 'from collections import namedtuple', 'contextmanager': 'from contextlib import contextmanager', 'asynccontextmanager': 'from contextlib import asynccontextmanager', 'partial': 'from functools import partial', 'wraps': 'from functools import wraps', 'lru_cache': 'from functools import lru_cache', 'cached_property': 'from functools import cached_property', 'Enum': 'from enum import Enum', 'auto': 'from enum import auto', 'IntEnum': 'from enum import IntEnum', 'logging': 'import logging', 'getLogger': 'import logging', 'traceback': 'import traceback', 'threading': 'import threading', 'asyncio': 'import asyncio', 'subprocess': 'import subprocess', 'tempfile': 'import tempfile', 'hashlib': 'import hashlib', 'base64': 'import base64', 'struct': 'import struct', 'io': 'import io', 'StringIO': 'from io import StringIO', 'BytesIO': 'from io import BytesIO', 'socket': 'import socket', 'uuid': 'import uuid', 'warnings': 'import warnings', 'weakref': 'import weakref', 'inspect': 'import inspect', 'textwrap': 'import textwrap', 'string': 'import string', 'random': 'import random', 'heapq': 'import heapq', 'bisect': 'import bisect', 'array': 'import array', 'pickle': 'import pickle', 'shelve': 'import shelve', 'sqlite3': 'import sqlite3', 'csv': 'import csv', 'configparser': 'import configparser', 'argparse': 'import argparse', 'shutil': 'import shutil', 'glob': 'import glob', 'fnmatch': 'import fnmatch', 'stat': 'import stat', 'platform': 'import platform', 'signal': 'import signal', 'atexit': 'import atexit', 'pprint': 'import pprint', 'unittest': 'import unittest', 'pytest': 'import pytest', 'dataclasses': 'import dataclasses', 'typing_extensions': 'import typing_extensions', 'Protocol': 'from typing import Protocol', 'runtime_checkable': 'from typing import runtime_checkable'}
...
class EntityVisitor(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> None:
        ...
        entity: Entity = Entity(name=node.name, full_name=node.name, entity_type='class', source=class_source, imports=imports_in_class, source_path=self.source_path)
    ...
    def _visit_method(self, node: ast.FunctionDef | ast.AsyncFunctionDef, class_name: str) -> None:
        ...
        entity: Entity = Entity(name=node.name, full_name=full_name, entity_type='function', source=method_source, imports=imports_for_method, source_path=self.source_path)
        self.entities.append(entity)
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        ...
        entity: Entity = Entity(name=node.name, full_name=node.name, entity_type='function', source=func_source, imports=imports_for_func, source_path=self.source_path)
        self.entities.append(entity)
    ...
    def visit_Assign(self, node: ast.Assign) -> None:
        ...
        for target in node.targets:
            if isinstance(target, ast.Name) and CONST_RE.match(target.id):
                ...
                entity: Entity = Entity(name=target.id, full_name=target.id, entity_type='const', source=const_source, imports=[], source_path=self.source_path)
                self.entities.append(entity)
...
def process_python_file(path: Path, virtual_path: str | None=None, source_override: str | None=None) -> tuple[list[Entity], list[str]]:
    ...
    source: str | None = source_override if source_override is not None else read_py_file(path)
    if source is None:
        return ([], [])
    if not source.strip():
        return ([], [])
...
TYPE_SUBDIR: dict[str, str] = {'function': 'function', 'class': 'class', 'const': 'const'}
...
def write_entity(entity: Entity, output_dir: Path) -> Path | None:
    ...
    all_imports: list[str] = sorted(set(entity.imports), key=lambda s: 0 if s.startswith(('import ', 'from ')) else 1)
    import_block: str = '\n'.join(all_imports)
    if import_block:
        import_block += '\n\n'
...
def main(argv: list[str] | None=None) -> int:
    ...
    Returns:
        Process exit code.
    """
    parser: argparse.ArgumentParser = argparse.ArgumentParser(description='Extract Python code entities from files and archives.')
    parser.add_argument('-t', '--tmp', action='store_true', help='Write output to ~/tmp/output/ instead of ./output/')
    parser.add_argument('-w', '--workers', type=int, default=FIXED_WORKERS, help=f'Number of parallel workers (default: {FIXED_WORKERS})')
    parser.add_argument('-d', '--dir', type=Path, default=Path.cwd(), help='Root directory to scan (default: current directory)')
    ...
    all_entities: list[Entity] = []
    all_imports: list[str] = []
    tasks: list[tuple[Callable[[Path], tuple[list[Entity], list[str], str]], Path]] = [(_worker_py, p) for p in py_files] + [(_worker_archive, p) for p in archives]
    print('Processing {} file(s) with {} worker(s)…', len(tasks), args.workers)
    with mp.Pool(processes=args.workers) as pool:
        async_results: list[tuple[Callable[[Path], tuple[list[Entity], list[str], str]], Path, Any]] = []
