...
"""p45.py – P45 utilities.

This module provides functionality for p45."""
...
def format_python_file(path: Path) -> None:
    """format_python_file – format python file.
...
Args:
    ...
    try:
        with path.open('r', encoding='utf-8') as f_in, backup_path.open('w', encoding='utf-8') as f_bak:
            content = f_in.read()
            f_bak.write(content)
    except OSError as e:
    ...
    for _i, line in enumerate(lines):
        ...
        if stripped_line.startswith((DOC_TH1, DOC_TH2)):
            if not in_multiline_string:
                ...
                if line.strip().endswith(string_type) and len(line.strip()) > len(string_type):
                    ...
                    wrapped_content = textwrap.fill(content_to_wrap, width=35, initial_indent=string_type, subsequent_indent=string_type + ' ' * (len(string_type) - 1), break_long_words=False, break_on_hyphens=False)
        ...
        if comment_index != -1:
            ...
            if comment_part:
                ...
                wrapped_comment = textwrap.fill(comment_content, width=35, initial_indent=comment_indent + '# ', subsequent_indent=comment_indent + '# ', break_long_words=False, break_on_hyphens=False)
    ...
    try:
        ...
        print(f'Error: Formatted code is not parsable by AST. Aborting write operation for {path}.', file=sys.stderr)
