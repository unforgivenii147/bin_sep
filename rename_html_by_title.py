...
logger.add('rename_html_by_title_{time:YYYYMMDD}.log', rotation='500 MB', retention='10 days', compression='zip', format='{time:YYYY-MM-DD HH:mm:ss} | {level: <8} | {name}:{function}:{line} - {message}', level='DEBUG', backtrace=True, diagnose=True)
PERSIAN_MAP: dict[str, str] = {'ا': 'a', 'ب': 'b', 'پ': 'p', 'ت': 't', 'ث': 's', 'ج': 'j', 'چ': 'ch', 'ح': 'h', 'خ': 'kh', 'د': 'd', 'ذ': 'z', 'ر': 'r', 'ز': 'z', 'ژ': 'zh', 'س': 's', 'ش': 'sh', 'ص': 's', 'ض': 'z', 'ط': 't', 'ظ': 'z', 'ع': 'a', 'غ': 'gh', 'ف': 'f', 'ق': 'q', 'ک': 'k', 'گ': 'g', 'ل': 'l', 'م': 'm', 'ن': 'n', 'و': 'o', 'ه': 'h', 'ی': 'i', 'ء': '', 'ة': 'e', 'َ': '', 'ِ': '', 'ُ': '', 'ّ': '', 'ْ': '', 'ً': '', 'ٌ': '', 'ٍ': ''}
ARABIC_MAP: dict[str, str] = {'ا': 'a', 'ب': 'b', 'ت': 't', 'ث': 'th', 'ج': 'j', 'ح': 'h', 'خ': 'kh', 'د': 'd', 'ذ': 'dh', 'ر': 'r', 'ز': 'z', 'س': 's', 'ش': 'sh', 'ص': 's', 'ض': 'd', 'ط': 't', 'ظ': 'z', 'ع': 'a', 'غ': 'gh', 'ف': 'f', 'ق': 'q', 'ك': 'k', 'ل': 'l', 'م': 'm', 'ن': 'n', 'ه': 'h', 'و': 'w', 'ي': 'i', 'أ': 'a', 'إ': 'i', 'آ': 'aa', 'ؤ': 'u', 'ئ': 'i', 'ة': 'a'}
CYRILLIC_MAP: dict[str, str] = {'а': 'a', 'б': 'b', 'в': 'v', 'г': 'g', 'д': 'd', 'е': 'e', 'ё': 'yo', 'ж': 'zh', 'з': 'z', 'и': 'i', 'й': 'y', 'к': 'k', 'л': 'l', 'м': 'm', 'н': 'n', 'о': 'o', 'п': 'p', 'р': 'r', 'с': 's', 'т': 't', 'у': 'u', 'ф': 'f', 'х': 'h', 'ц': 'ts', 'ч': 'ch', 'ш': 'sh', 'щ': 'sch', 'ъ': '', 'ы': 'y', 'ь': '', 'э': 'e', 'ю': 'yu', 'я': 'ya', 'А': 'a', 'Б': 'b', 'В': 'v', 'Г': 'g', 'Д': 'd', 'Е': 'e', 'Ё': 'yo', 'Ж': 'zh', 'З': 'z', 'И': 'i', 'Й': 'y', 'К': 'k', 'Л': 'l', 'М': 'm', 'Н': 'n', 'О': 'o', 'П': 'p', 'Р': 'r', 'С': 's', 'Т': 't', 'У': 'u', 'Ф': 'f', 'Х': 'h', 'Ц': 'ts', 'Ч': 'ch', 'Ш': 'sh', 'Щ': 'sch', 'Ъ': '', 'Ы': 'y', 'Ь': '', 'Э': 'e', 'Ю': 'yu', 'Я': 'ya'}
GREEK_MAP: dict[str, str] = {'α': 'a', 'β': 'b', 'γ': 'g', 'δ': 'd', 'ε': 'e', 'ζ': 'z', 'η': 'h', 'θ': 'th', 'ι': 'i', 'κ': 'k', 'λ': 'l', 'μ': 'm', 'ν': 'n', 'ξ': 'x', 'ο': 'o', 'π': 'p', 'ρ': 'r', 'σ': 's', 'τ': 't', 'υ': 'u', 'φ': 'ph', 'χ': 'ch', 'ψ': 'ps', 'ω': 'o', 'Α': 'a', 'Β': 'b', 'Γ': 'g', 'Δ': 'd', 'Ε': 'e', 'Ζ': 'z', 'Η': 'h', 'Θ': 'th', 'Ι': 'i', 'Κ': 'k', 'Λ': 'l', 'Μ': 'm', 'Ν': 'n', 'Ξ': 'x', 'Ο': 'o', 'Π': 'p', 'Ρ': 'r', 'Σ': 's', 'Τ': 't', 'Υ': 'u', 'Φ': 'ph', 'Χ': 'ch', 'Ψ': 'ps', 'Ω': 'o'}
CHINESE_MAP: dict[str, str] = {'中': 'zhong', '国': 'guo', '人': 'ren', '大': 'da', '小': 'xiao', '文': 'wen', '字': 'zi', '日': 'ri', '月': 'yue', '年': 'nian', '天': 'tian', '地': 'di', '水': 'shui', '火': 'huo', '木': 'mu'}
TRANSLITERATION_MAPS: list[dict[str, str]] = [PERSIAN_MAP, ARABIC_MAP, CYRILLIC_MAP, GREEK_MAP, CHINESE_MAP]
...
class LanguageTransliterator:
    ...
    def detect_script(text: str) -> str:
        ...
        total: int = len(text)
        script_counts: dict[str, int] = {'persian': persian_count, 'cyrillic': cyrillic_count, 'greek': greek_count, 'chinese': chinese_count, 'hangul': hangul_count, 'japanese': hiragana_count + katakana_count, 'hebrew': hebrew_count, 'devanagari': devanagari_count}
...
Returns:
    ...
    def _extract_with_regex(html_content: str) -> str | None:
        ...
        Returns:
            The extracted title, or None if no title was found.
        """
        patterns: list[str] = ['<title[^>]*>(.*?)</title>', '<TITLE[^>]*>(.*?)</TITLE>', '<Title[^>]*>(.*?)</Title>']
...
class FilenameNormalizer:
    ...
    PUNCTUATION_PATTERN: re.Pattern = re.compile('[^\\w\\s-]', re.UNICODE)
    SPACE_PATTERN: re.Pattern = re.compile('\\s+')
    UNDERSCORE_PATTERN: re.Pattern = re.compile('_+')
    DASH_PATTERN: re.Pattern = re.compile('-+')
    LEADING_TRAILING_PATTERN: re.Pattern = re.compile('^[\\s_-]+|[\\s_-]+$')
    RESERVED_NAMES: set[str] = {'con', 'prn', 'aux', 'nul', 'com1', 'com2', 'com3', 'com4', 'com5', 'com6', 'com7', 'com8', 'com9', 'lpt1', 'lpt2', 'lpt3', 'lpt4', 'lpt5', 'lpt6', 'lpt7', 'lpt8', 'lpt9'}
...
class HtmlFileProcessor:
    ...
    def process_file(self, path: Path) -> ProcessingResult:
        ...
        try:
            if not path.exists():
                return ProcessingResult(path=path, original_name=original_name, new_name=None, title=None, success=False, error=f'File not found: {path}', duration=(datetime.now() - start_time).total_seconds())
            if not path.is_file():
                return ProcessingResult(path=path, original_name=original_name, new_name=None, title=None, success=False, error=f'Not a file: {path}', duration=(datetime.now() - start_time).total_seconds())
            if path.stat().st_size > MAX_FILE_SIZE:
                return ProcessingResult(path=path, original_name=original_name, new_name=None, title=None, success=False, error=f'File too large: {path.stat().st_size} bytes', duration=(datetime.now() - start_time).total_seconds())
            title: str | None = self.title_extractor.extract_title(path)
            if not title:
                return ProcessingResult(path=path, original_name=original_name, new_name=None, title=None, success=False, error='No valid title found in HTML', duration=(datetime.now() - start_time).total_seconds())
            new_name: str = FilenameNormalizer.normalize(title, path.suffix)
            if new_name == original_name:
                return ProcessingResult(path=path, original_name=original_name, new_name=new_name, title=title, success=True, error='Filename already matches title', duration=(datetime.now() - start_time).total_seconds())
            ...
            try:
                ...
                return ProcessingResult(path=new_path, original_name=original_name, new_name=new_path.name, title=title, success=True, duration=(datetime.now() - start_time).total_seconds())
            except Exception as e:
                return ProcessingResult(path=path, original_name=original_name, new_name=new_name, title=title, success=False, error=f'Rename failed: {e}', duration=(datetime.now() - start_time).total_seconds())
        except Exception as e:
            logger.error(f'Unexpected error processing {path}: {e}')
            return ProcessingResult(path=path, original_name=original_name, new_name=None, title=None, success=False, error=str(e), duration=(datetime.now() - start_time).total_seconds())

class FileDiscovery:
    """Discovers HTML files in directories."""
    SKIP_DIRS: set[str] = {'.git', '.svn', '__pycache__', 'node_modules', '.venv', 'venv', '.env', '.egg-info', 'dist', 'build', '.idea', '.vscode', '.pytest_cache', '.tox', '.coverage', '.mypy_cache', 'target', 'out', 'bin', '.gradle', '.next', '.cache'}
