    ...
    for item in paths:
        ...
        if item.is_file():
            if item.suffix == ".js":
                ...
                print(f"warning: {item} is not a .js file, skipping", file=sys.stderr)
        elif item.is_dir():
            base = item
            files.extend((p, base) for p in sorted(base.rglob("*.js")))
        else:
            print(
...
def main() -> int:
    ...
    workers = args.jobs
    print(
        f"Processing {len(files)} JavaScript file(s) with {workers or 'all'} worker(s)...\n"
    )
    t0 = time.monotonic()
    total_files = 0
    total_comments = 0
