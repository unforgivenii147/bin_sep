...
"""ex_const.py – Ex Const utilities.

This module provides functionality for ex const."""
...
Returns:
    list[tuple[str, str, str]]: Description of return value."""
    constants = []
    try:
            ...
            if isinstance(node, ast.Assign):
                ...
                if isinstance(node.target, ast.Name) and node.value is not None:
                    if node.target.id.isupper():
                        ...
                        const_value = ast.unparse(node.value)
                        const_type = type(node.value.value).__name__ if isinstance(node.value, ast.Constant) else 'unknown'
                        constants.append((const_name, const_value, const_type))
...
Returns:
    tuple[str, list[tuple[str, str, str]] | None]: Description of return value."""
