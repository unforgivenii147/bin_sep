...
"""mfadic.py – Mfadic utilities.

This module provides functionality for mfadic."""
...
def is_persian_word(word: str) -> bool:
    """is_persian_word – is persian word.

Args:
    word: Description of word."""
    persian_pattern = re.compile('[\\u0600-\\u06FF\\u0750-\\u077F\\u08A0-\\u08FF]')
    return bool(persian_pattern.search(word))

def is_english_word(word: str) -> bool:
    """is_english_word – is english word.
...
def check_file_format(data: str) -> bool:
    """check_file_format – check file format.
...
Args:
    ...
    english_values = sum((1 for v in sample_values if isinstance(v, str) and is_english_word(v)))
    is_correct = persian_keys >= len(sample_keys) * 0.5 and english_values >= len(sample_values) * 0.5
    return (is_correct, sample_keys, sample_values)

def merge_json_files(directory: str='.', output_file: str='faen.json') -> None:
    ...
    for json_file in json_files:
        try:
            ...
            if not is_correct_format:
                cprint(f'❌ Skipping {json_file.name}: Wrong format (English keys detected)')
            ...
            for persian_word, english_translation in data.items():
                ...
                if persian_word in merged_data:
                    ...
                    duplicate_keys[persian_word].append({'existing': merged_data[persian_word], 'duplicate': english_translation, 'file': json_file.name})
                else:
