...
"""oxip.py – Oxip utilities.

This module provides functionality for oxip."""
...
def optimize_png(path: Path | str) -> int:
...
Returns:
    ...
    try:
        ...
        subprocess.run(['oxipng', '-o', 'max', '--quiet', '--strip', 'safe', str(path), '--force'], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
