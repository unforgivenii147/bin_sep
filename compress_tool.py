...
"""compress_tool.py – Compress Tool utilities.

This module provides functionality for compress tool."""
...
def setup_compressors() -> None:
    """setup_compressors – setup compressors."""
    global COMPRESSORS
    COMPRESSORS = {'zstd': {'ext': '.zst', 'tar_ext': '*.tar.zst', 'settings': {'level': 22}, 'available': True}, 'xz': {'ext': '.xz', 'tar_ext': '*.tar.xz', 'settings': {'preset': 9}, 'available': True}, 'py7zr': {'ext': '.7z', 'tar_ext': '*.7z', 'settings': {'compression_level': 7}, 'available': True}, 'gzip': {'ext': '.gz', 'tar_ext': '*.tar.gz', 'settings': {'compresslevel': 9}, 'available': True}, 'bz2': {'ext': '.bz2', 'tar_ext': '*.tar.bz2', 'settings': {'compresslevel': 9}, 'available': True}, 'brotli': {'ext': '.br', 'tar_ext': '*.tar.br', 'settings': {'quality': 11, 'lgwin': 22}, 'available': True}, 'lz4': {'ext': '.lz4', 'tar_ext': '*.tar.lz4', 'settings': {'compression_level': 12}, 'available': True}}
...
Args:
    ...
    compressor: Description of compressor.
...
def compress_in_memory(path: Path, out_path: Path, compressor: str) -> bool:
    """compress_in_memory – compress in memory.
    ...
    try:
        ...
        with open(out_path, 'wb') as f:
            if compressor == 'zstd':
                ...
                f.write(gzip.compress(data, compresslevel=COMPRESSORS['gzip']['settings']['compresslevel']))
            elif compressor == 'bz2':
                f.write(bz2.compress(data, compresslevel=COMPRESSORS['bz2']['settings']['compresslevel']))
            elif compressor == 'brotli':
                f.write(brotli.compress(data, quality=COMPRESSORS['brotli']['settings']['quality'], lgwin=COMPRESSORS['brotli']['settings']['lgwin']))
            elif compressor == 'lz4':
                f.write(lz4.frame.compress(data, compression_level=COMPRESSORS['lz4']['settings']['compression_level']))
            elif compressor == 'xz':
                import lzma
...
def compress_chunked(path: Path, out_path: Path, original_size: int, compressor: str) -> bool:
    """compress_chunked – compress chunked.
...
Returns:
    ...
    try:
        if compressor == 'zstd':
            ...
            with open(path, 'rb') as inf, gzip.open(out_path, 'wb', compresslevel=COMPRESSORS['gzip']['settings']['compresslevel']) as outf:
                while True:
                    chunk = inf.read(CHUNK_SIZE)
                    if not chunk:
                        break
            ...
            with open(path, 'rb') as inf, bz2.open(out_path, 'wb', compresslevel=COMPRESSORS['bz2']['settings']['compresslevel']) as outf:
                while True:
                    chunk = inf.read(CHUNK_SIZE)
                    if not chunk:
                        break
            ...
            with open(path, 'rb') as inf, open(out_path, 'wb') as outf:
                compressor_obj = brotli.Compressor(quality=COMPRESSORS['brotli']['settings']['quality'], lgwin=COMPRESSORS['brotli']['settings']['lgwin'])
                while True:
                    chunk = inf.read(CHUNK_SIZE)
                    if not chunk:
                        break
            ...
            with open(path, 'rb') as inf, open(out_path, 'wb') as outf:
                while True:
                    ...
                    if not chunk:
                        break
                    compressed = lz4.frame.compress_chunk(context, chunk, compression_level=COMPRESSORS['lz4']['settings']['compression_level'])
                    outf.write(compressed)
        elif compressor == 'xz':
            import lzma
            with open(path, 'rb') as inf, lzma.open(out_path, 'wb', preset=COMPRESSORS['xz']['settings']['preset']) as outf:
                while True:
                    chunk = inf.read(CHUNK_SIZE)
                    if not chunk:
                        break
...
def decompress_archive(archive_path: Path, compressor: str) -> bool:
    """decompress_archive – decompress archive.
...
async def compress_folder_async(dir_path: Path, archive_path: str, compressor: str) -> bool:
    """compress_folder_async – compress folder async.
...
async def process_compress(compressor: str) -> None:
    """process_compress – process compress.
...
async def process_decompress(compressor: str) -> None:
    """process_decompress – process decompress.
...
def check_compressor_availability(compressor: str) -> bool:
    """check_compressor_availability – check compressor availability.
