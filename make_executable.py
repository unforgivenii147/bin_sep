...
"""make_executable.py – Make Executable utilities.

This module provides functionality for make executable."""
...
def check_and_make_executable(path: Path) -> dict:
    """check_and_make_executable – check and make executable.
...
Returns:
    ...
    result = {'path': path, 'is_shebang': False, 'permission_changed': False, 'error': None}
    try:
        if not path.is_file():
            return result
