...
"""fix_escape.py – Fix Escape utilities.

This module provides functionality for fix escape."""
...
def walk_python_files(root: Path) -> Iterator[Path]:
    """walk_python_files – walk python files.
...
Returns:
    ...
    for warning in caught_warnings:
        if issubclass(warning.category, SyntaxWarning) and 'invalid escape sequence' in str(warning.message):
            result['has_issues'] = True
            line_number = getattr(warning, 'lineno', 'Unknown')
            result['messages'].append(f'Line {line_number}: SyntaxWarning: {warning.message}')
    ...
    try:
        ...
        for token in tokens:
            ...
            prefix_end = 0
            while prefix_end < len(string_text) and string_text[prefix_end].lower() in 'frub':
                prefix_end += 1
            ...
            with warnings.catch_warnings(record=True) as token_warnings:
                ...
                with contextlib.suppress(SyntaxError):
                    compile(f'_{prefix}{literal}', '<string>', 'exec')
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Recursively scan and optionally fix Python files containing invalid escape sequences.')
    parser.add_argument('-a', '--auto-fix', action='store_true', help='Automatically fix invalid escape sequences by converting affected string literals to raw strings.')
