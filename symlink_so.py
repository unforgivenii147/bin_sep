...
"""symlink_so.py – Symlink So utilities.

This module provides functionality for symlink so."""
...
def should_skip(so_path: Path | str) -> bool:
    """should_skip – should skip.

Args:
    so_path: Description of so_path."""
    if so_path.is_symlink():
        return True
    name = so_path.name
    if name.endswith(('.0', '.1')):
        return True
    return bool(re.search('\\.so\\.\\d+(\\.\\d+)+$', name))

def get_base_name(so_path: Path | str) -> Any:
    """get_base_name – get base name.
