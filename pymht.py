...
"""pymht.py – Pymht utilities.

This module provides functionality for pymht."""
...
def split_data_url(src: str) -> Any | None:
    """split_data_url – split data url.
...
Args:
    ...
    def get_name_from_headers(part: Any) -> str:
        """get_name_from_headers – get name from headers.
...
Args:
    ...
    html_text = re.sub('(src|href)=[\\"\']cid:([^\\"\']+)[\\"\']', lambda m: f'{m.group(1)}="{os.path.basename(out_dir)}/{cid_to_file.get(m.group(2), m.group(2))}"' if m.group(2) in cid_to_file else m.group(0), html_text, flags=re.IGNORECASE)

    def data_uri_replacer(match: Any) -> Any:
        """data_uri_replacer – data uri replacer.
