...
"""move_tests_dirs.py – Move Tests Dirs utilities.

This module provides functionality for move tests dirs."""
...
def move_tests_folder(tests_path: Path, base_src: Path, base_dst: Path) -> tuple[bool, str]:
    """move_tests_folder – move tests folder.

Args:
    tests_path: Description of tests_path.
    base_src: Description of base_src.
    base_dst: Description of base_dst.
...
def move_tests_recursive(source_dir: str=SRC, max_workers: int=4) -> int:
    """move_tests_recursive – move tests recursive.
...
Returns:
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(move_tests_folder, tests_path, source, destination): tests_path for tests_path in tests_folders}
