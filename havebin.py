...
"""havebin.py – Havebin utilities.

This module provides functionality for havebin."""
...
def find_packages_with_bin_scripts(output_file: str='have_scripts.txt') -> None:
    """find_packages_with_bin_scripts – find packages with bin scripts.
...
Args:
    ...
    try:
        ...
        for i, package_name in enumerate(installed_packages):
            ...
            try:
                result = subprocess.run(['pip', 'show', '-f', package_name], capture_output=True, text=True, check=True, encoding='utf-8', errors='ignore')
                ...
                for line in lines:
                    ...
                    for indicator in bin_indicators:
                        if indicator in line.lower() and (line.endswith('.py') or os.path.splitext(line)[1] == '' or os.path.splitext(line)[1] == '.exe') and (not any((exclude_part in line for exclude_part in ['__pycache__', '.dist-info', '.egg-info', '.pth']))):
