...
"""tscleaner.py – Tscleaner utilities.

This module provides functionality for tscleaner."""
...
WORKERS = 8
ROOT = Path.cwd()
SKIP_DIRECTORY_NAMES = frozenset({'.git', '.hg', '.svn', '.idea', '.mypy_cache', '.pytest_cache', '.ruff_cache', '.tox', '.venv', '__pycache__', 'node_modules', 'target', 'build', 'dist', 'vendor'})
...
SUFFIX_SPECS: tuple[tuple[str, LanguageSpec], ...] = (('.d.ts', LanguageSpec('tree_sitter_typescript', 'language_typescript')), ('.tsx', LanguageSpec('tree_sitter_typescript', 'language_tsx')), ('.mts', LanguageSpec('tree_sitter_typescript', 'language_typescript')), ('.cts', LanguageSpec('tree_sitter_typescript', 'language_typescript')), ('.ts', LanguageSpec('tree_sitter_typescript', 'language_typescript')), ('.jsx', LanguageSpec('tree_sitter_javascript', 'language')), ('.mjs', LanguageSpec('tree_sitter_javascript', 'language')), ('.cjs', LanguageSpec('tree_sitter_javascript', 'language')), ('.js', LanguageSpec('tree_sitter_javascript', 'language')), ('.cpp', LanguageSpec('tree_sitter_cpp', 'language')), ('.cxx', LanguageSpec('tree_sitter_cpp', 'language')), ('.cc', LanguageSpec('tree_sitter_cpp', 'language')), ('.c++', LanguageSpec('tree_sitter_cpp', 'language')), ('.hpp', LanguageSpec('tree_sitter_cpp', 'language')), ('.hxx', LanguageSpec('tree_sitter_cpp', 'language')), ('.hh', LanguageSpec('tree_sitter_cpp', 'language')), ('.h++', LanguageSpec('tree_sitter_cpp', 'language')), ('.css', LanguageSpec('tree_sitter_css', 'language')), ('.html', LanguageSpec('tree_sitter_html', 'language')), ('.htm', LanguageSpec('tree_sitter_html', 'language')), ('.xhtml', LanguageSpec('tree_sitter_html', 'language')), ('.lua', LanguageSpec('tree_sitter_lua', 'language')), ('.rs', LanguageSpec('tree_sitter_rust', 'language')), ('.vim', LanguageSpec('tree_sitter_vim', 'language')), ('.c', LanguageSpec('tree_sitter_c', 'language')), ('.h', LanguageSpec('tree_sitter_c', 'language')), ('.sh', LanguageSpec('tree_sitter_bash', 'language')), ('.bash', LanguageSpec('tree_sitter_bash', 'language')), ('.zsh', LanguageSpec('tree_sitter_bash', 'language')), ('.ksh', LanguageSpec('tree_sitter_bash', 'language')))

def language_spec_for(path: Path) -> LanguageSpec | None:
    """language_spec_for – language spec for.

Args:
    path: Description of path.

Returns:
    LanguageSpec | None: Description of return value."""
...
def iter_source_files(root: Path) -> list[Path]:
    """iter_source_files – iter source files.
...
Returns:
    ...
    files: list[Path] = []
    try:
        for directory, directory_names, file_names in root.walk(top_down=True, follow_symlinks=False):
