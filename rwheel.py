...
"""rwheel.py – Rwheel utilities.

This module provides functionality for rwheel."""
...
def get_package_info(pkg_path: Path | str) -> Any:
    """get_package_info – get package info.

Args:
    pkg_path: Description of pkg_path."""
...
def collect_package_files(pkg_path: Path | str, verbose: bool=False) -> Any:
...
Args:
    ...
    if bin_path.exists():
        for script_path in bin_path.glob('*'):
            if script_path.is_file() and (script_path.name.startswith(pkg_path.name) or script_path.name == pkg_path.name):
...
def create_wheel(pkg_path: Path | str, wheels_dir: Path | str, dryrun: bool=False, verbose: bool=False) -> bool:
    """create_wheel – create wheel.

Args:
    pkg_path: Description of pkg_path.
    wheels_dir: Description of wheels_dir.
    dryrun: Description of dryrun.
    verbose: Description of verbose.
...
def get_packages(site_packages_dir: Path, package_names: str | None=None) -> Any:
    """get_packages – get packages.

Args:
    site_packages_dir: Description of site_packages_dir.
    package_names: Description of package_names."""
    packages = []
    ...
    for item in site_packages_dir.iterdir():
        ...
        if item.name.startswith('_'):
            continue
        if item.is_dir():
            if (item / '__init__.py').exists() or any(item.parent.glob(f'{item.name}*.dist-info')):
...
def repack_sequential(packages: Any, wheels_dir: Path, dryrun: Any, verbose: bool) -> Any:
    """repack_sequential – repack sequential.
...
def repack_parallel(packages: Any, wheels_dir: Path, dryrun: Any, verbose: bool) -> list[bool]:
    """repack_parallel – repack parallel.
...
Returns:
    list[bool]: Description of return value."""
...
def main() -> None:
    ...
    possible_paths = [Path(sys.executable).parent / 'lib' / f'python{sys.version_info.major}.{sys.version_info.minor}' / 'site-packages', Path(sys.prefix) / 'lib' / f'python{sys.version_info.major}.{sys.version_info.minor}' / 'site-packages', Path(__file__).parent if '__file__' in dir() else Path.cwd()]
    site_packages = None
