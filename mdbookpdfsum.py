...
"""mdbookpdfsum.py – Mdbookpdfsum utilities.

This module provides functionality for mdbookpdfsum."""
...
Args:
    child: Description of child."""
        self.children.append(child)

    def path_to_root(self) -> Any:
...
Args:
    prefix_path: Description of prefix_path.
    ...
    for idx, line in enumerate(lines):
        if line.startswith('# '):
            ...
            if not title.startswith(node.title):
                all_matched = False
                print(f'[ERROR] Title not matched: source_file:{source_file}, line num:{idx}, title:{title}, title in `SUMMARY.md`:{node.title}')
                break
...
def add_outline(html_root: Any, reader: pypdf.PdfReader, writer: pypdf.PdfWriter, node: Section) -> None:
    """add_outline – add outline.

Args:
    html_root: Description of html_root.
    reader: Description of reader.
    writer: Description of writer.
    node: Description of node."""
    if not node.is_root():
        id = get_dom_id(node)
        try:
...
def main() -> None:
    ...
    parser.add_argument('--html_path', type=str, help='path of the `print.html` generated `mdbook-pdf`', default='print.html')
    parser.add_argument('--pdf_path', type=str, help='path of the `output.pdf` generated `mdbook-pdf`', default='output.pdf')
...
def print_section_tree(root: Section) -> None:
    """print_section_tree – print section tree.
...
def parse_section_tree(md_text: str) -> Section:
    """parse_section_tree – parse section tree.

Args:
    md_text: Description of md_text.

Returns:
    Section: Description of return value."""
