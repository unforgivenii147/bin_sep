...
"""check_duplicate_packages.py – Check Duplicate Packages utilities.

This module provides functionality for check duplicate packages."""
...
def get_site_directories() -> tuple[list[pathlib.Path], list[pathlib.Path]]:
    """get_site_directories – get site directories.

Returns:
    tuple[list[pathlib.Path], list[pathlib.Path]]: Description of return value."""
...
def scan_directory_for_packages(directory: pathlib.Path) -> dict[str, pathlib.Path]:
    """scan_directory_for_packages – scan directory for packages.

Args:
    directory: Description of directory.

Returns:
    dict[str, pathlib.Path]: Description of return value."""
    packages = {}
...
def find_duplicate_packages(system_packages: dict[str, pathlib.Path], user_packages: dict[str, pathlib.Path]) -> dict[str, tuple[pathlib.Path, pathlib.Path]]:
    """find_duplicate_packages – find duplicate packages.

Args:
    system_packages: Description of system_packages.
    user_packages: Description of user_packages.

Returns:
    dict[str, tuple[pathlib.Path, pathlib.Path]]: Description of return value."""
    duplicates = {}
...
def process_system_directories(system_dirs: list[pathlib.Path]) -> dict[str, pathlib.Path]:
    """process_system_directories – process system directories.

Args:
    system_dirs: Description of system_dirs.

Returns:
    dict[str, pathlib.Path]: Description of return value."""
    system_packages = {}
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(system_dirs) or 1) as executor:
        future_to_dir = {executor.submit(scan_directory_for_packages, directory): directory for directory in system_dirs}
...
def process_user_directories(user_dirs: list[pathlib.Path]) -> dict[str, pathlib.Path]:
    """process_user_directories – process user directories.

Args:
    user_dirs: Description of user_dirs.
...
def analyze_package_versions(package_name: str, system_location: pathlib.Path, user_location: pathlib.Path) -> dict[str, str]:
    """analyze_package_versions – analyze package versions.

Args:
    package_name: Description of package_name.
    system_location: Description of system_location.
    user_location: Description of user_location.
...
def main() -> None:
    ...
    if duplicates:
        ...
        for pkg_name, (system_loc, user_loc) in duplicates.items():
            ...
            if (versions['system_version'] != 'unknown' and versions['user_version'] != 'unknown') and versions['system_version'] != versions['user_version']:
