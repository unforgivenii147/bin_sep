...
"""imzzz.py – Imzzz utilities.

This module provides functionality for imzzz."""
...
Args:
    ...
    return path.suffix == '.py' or (not path.suffix and any((line.startswith(('import ', 'from ', '#!/usr/bin/env python')) for line in Path(path).open(encoding='utf-8', errors='ignore'))))

def extract_compressed(path: Path | str, extract_to: Any) -> None:
    """extract_compressed – extract compressed.
...
def get_imports(path: Path | str) -> Any:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                ...
                if module not in STD_LIB and (not module.startswith('.')) and (not path.parent.match(f'*{module}*')):
                    imports.add(MAPPING.get(module, module))
        elif isinstance(node, ast.ImportFrom):
            module = node.module.split('.')[0] if node.module else ''
            if module and module not in STD_LIB and (not module.startswith('.')) and (not path.parent.match(f'*{module}*')):
                imports.add(MAPPING.get(module, module))
    return imports
