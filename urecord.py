...
"""urecord.py – Urecord utilities.

This module provides functionality for urecord."""
...
def find_site_packages() -> list[str]:
    """find_site_packages – find site packages.
...
def update_record_file(record_path: Path) -> bool:
    """update_record_file – update record file.
...
def scan_and_update(site_packages_dirs: Path | str) -> tuple[int, int]:
    """scan_and_update – scan and update.

Args:
    site_packages_dirs: Description of site_packages_dirs.
...
def main() -> None:
    """main – main."""
    parser = argparse.ArgumentParser(description='Remove .pyc and direct_url.json references from RECORD files in site-packages')
    parser.add_argument('--site-dir', '-s', action='append', help='Specific site-packages directory to scan (can be used multiple times)')
