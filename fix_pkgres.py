...
"""fix_pkgres.py – Fix Pkgres utilities.

This module provides functionality for fix pkgres."""
...
PATTERNS = {'import_stmt': re.compile('^(import pkg_resources|from pkg_resources import .*)', re.MULTILINE), 'get_dist': re.compile('pkg_resources\\.get_distribution\\((.*?)\\)\\.version'), 'parse_version': re.compile('pkg_resources\\.parse_version\\('), 'resource_filename': re.compile('pkg_resources\\.resource_filename\\('), 'requirement': re.compile('pkg_resources\\.Requirement\\.parse\\(')}

def fix_content(content: str) -> Any:
...
Args:
    ...
    if 'pkg_resources.get_distribution' in new_content:
        new_content = PATTERNS['get_dist'].sub('importlib.metadata.version(\\1)', new_content)
    ...
    new_content = re.sub('^import pkg_resources\\n?', '', new_content, flags=re.MULTILINE)
    return new_content
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--autofix', action='store_true', help='Attempt to automatically replace simple patterns.')
