...
"""vitrans.py – Vitrans utilities.

This module provides functionality for vitrans."""
...
def split_into_chunks(text: str, max_chars: int=MAX_CHUNK_CHARS) -> list[str]:
    """split_into_chunks – split into chunks.
...
@retry(reraise=True, stop=stop_after_attempt(MAX_RETRIES), wait=wait_exponential_jitter(initial=3, max=90, jitter=4), retry=retry_if_exception_type((RateLimitError, TranslationError)), before_sleep=before_sleep_log(logger, logging.DEBUG))
def _translate_chunk(text: str) -> str:
    """_translate_chunk –  translate chunk.
...
def translate_chunk_safe(text: str, idx: int) -> tuple[str, bool]:
    """translate_chunk_safe – translate chunk safe.
...
Args:
    src: Description of src.
    done: Description of done.
    total: Description of total."""
    try:
        state = {'source': str(src), 'saved_at': datetime.now().isoformat(), 'total_chunks': total, 'chunks': {str(k): v for k, v in done.items()}}
...
Returns:
    dict[int, str]: Description of return value."""
