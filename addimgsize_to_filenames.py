...
"""addimgsize_to_filenames.py – Addimgsize To Filenames utilities.

This module provides functionality for addimgsize to filenames."""
...
class ImageDimensionRenamer:
    """ImageDimensionRenamer – ImageDimensionRenamer."""
...
Args:
    ...
    separator: Description of separator."""
...
Returns:
    ...
    def has_dimensions_in_name(filename: str) -> bool:
        """has_dimensions_in_name – has dimensions in name.
...
Args:
    image_paths: Description of image_paths."""
        ...
        with Pool(processes=cpu_count()) as pool:
            results = list(tqdm(pool.imap_unordered(self.rename_image, args_list), total=len(image_paths), desc='Processing', unit='img', ncols=80))
        print('\n[RESULTS]')
        print('-' * 40)
        ...
        print('-' * 40)
        print(f'[SUMMARY] Renamed: {successful} | Skipped: {already_processed} | Failed: {failed} | Total: {len(image_paths)}')
