...
"""ssdiper.py – Ssdiper utilities.

This module provides functionality for ssdiper."""
...
def calculate_ssdeep_hash(path: Path, min_file_size: int=1) -> Any | None:
    """calculate_ssdeep_hash – calculate ssdeep hash.

Args:
    path: Description of path.
    min_file_size: Description of min_file_size."""
    try:
...
def compare_files(paths: list[Path], similarity_threshold: int=70) -> Any:
        ...
        for j in range(i + 1, len(paths_list)):
            ...
            try:
                ...
                if score >= similarity_threshold:
                    similarities.append({'file1': str(Path(path1_str).relative_to(cwd)), 'file2': str(Path(path2_str).relative_to(cwd)), 'similarity_score': score})
...
def save_to_json(data: str, filename: str='simz.json') -> None:
