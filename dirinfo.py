...
"""dirinfo.py – Dirinfo utilities.

This module provides functionality for dirinfo."""
...
def scan_directory(path: str='.') -> Any:
...
Args:
    chart_type: Description of chart_type.
    output_filename: Description of output_filename."""
    _, _, _, _, size_by_ext = scan_directory()
    sorted_items = sorted([(ext, size) for ext, size in size_by_ext.items() if size > 0], key=operator.itemgetter(1), reverse=True)
    if not sorted_items:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--save', action='store_true', help='Save the report to a file named .dirinfo in the current directory.')
    parser.add_argument('-i', '--image', metavar='FILENAME', type=str, help='Save a Matplotlib bar chart of file types and sizes to the specified image file (e.g., chart.png).')
    parser.add_argument('-t', '--type', choices=['persian', 'english'], default='english', help='Specify the language/type of the Matplotlib chart title and labels (default: english).')
    parser.add_argument('path', metavar='PATH', type=str, nargs='?', default='.', help='The directory to scan (default: current directory).')
