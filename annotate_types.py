...
"""annotate_types.py – Annotate Types utilities.

This module provides functionality for annotate types."""
...
Returns:
    cst.CSTNode: Description of return value."""
        if original_node.module and cst.helpers.get_full_name_for_node(original_node.module) == '_typeshed':
            return cst.ImportFrom(module=cst.Name('typing'), names=[cst.ImportAlias(name=cst.Name('Any'))])
        return updated_node
...
Returns:
    ...
    def leave_Attribute(self, original_node: cst.Attribute, updated_node: cst.Attribute) -> cst.CSTNode:
        """leave_Attribute – leave Attribute.
...
Returns:
    cst.CSTNode: Description of return value."""
        if (isinstance(original_node.value, cst.Name) and original_node.value.value == '_typeshed') and original_node.attr.value == 'Incomplete':
...
def sanitize_stub_cst(stub_cst: cst.Module) -> cst.Module:
    """sanitize_stub_cst – sanitize stub cst.

Args:
    stub_cst: Description of stub_cst.
...
def generate_stub(py_path: Path, output_stub_path: Path, verbose: bool=False) -> None:
    """generate_stub – generate stub.

Args:
    py_path: Description of py_path.
    output_stub_path: Description of output_stub_path.
    ...
    with tempfile.TemporaryDirectory() as tmp_out_dir:
        cmd = [sys.executable, '-c', 'from mypy.stubgen import main; main()', '--include-private', '--include-docstrings', '-o', tmp_out_dir, str(py_path)]
        try:
        ...
        if not generated_stubs:
            raise RuntimeError(f'stubgen finished but no .pyi file was generated in output directory. Output: {result.stdout.strip()}')
        target_stub = next((s for s in generated_stubs if s.stem == py_path.stem), generated_stubs[0])
...
def apply_type_annotations(source_code: str, stub_code: str, overwrite_existing: bool=True, use_future_annotations: bool=False) -> str:
    """apply_type_annotations – apply type annotations.

Args:
    source_code: Description of source_code.
    stub_code: Description of stub_code.
    overwrite_existing: Description of overwrite_existing.
    use_future_annotations: Description of use_future_annotations.
...
Returns:
    ...
    ApplyTypeAnnotationsVisitor.store_stub_in_context(context=context, stub=stub_cst, overwrite_existing_annotations=overwrite_existing, use_future_annotations=use_future_annotations)
...
Args:
    ...
    try:
        ...
        raise SyntaxError(f'Resulting code has invalid Python syntax at line {e.lineno}, column {e.offset}: {e.msg}\nCode snippet:\n{e.text}') from e
...
Args:
    ...
    modified: Description of modified.
    filename: Description of filename.

Returns:
    str: Description of return value."""
    diff = difflib.unified_diff(original.splitlines(keepends=True), modified.splitlines(keepends=True), fromfile=f'{filename} (original)', tofile=f'{filename} (annotated)')
    return ''.join(diff)

def annotate_file(target_file: str | Path, stub_file: str | Path | None=None, overwrite_existing: bool=True, use_future_annotations: bool=False, dry_run: bool=False, show_diff: bool=False, verbose: bool=True) -> tuple[bool, str]:
    """annotate_file – annotate file.

Args:
    target_file: Description of target_file.
    stub_file: Description of stub_file.
    overwrite_existing: Description of overwrite_existing.
    use_future_annotations: Description of use_future_annotations.
...
Returns:
    ...
    annotated_code = apply_type_annotations(source_code=original_code, stub_code=stub_code, overwrite_existing=overwrite_existing, use_future_annotations=use_future_annotations)
...
Returns:
    ...
    parser = argparse.ArgumentParser(description='Add type annotations to a Python (.py) file using its .pyi stub file and LibCST.\nIf the stub file does not exist beside the original file, it is created using stubgen.\nUpdates the file in-place after validating syntax with ast.parse.', formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument('file', help='Path to the .py source file to annotate (input=sys.argv[1]).')
    parser.add_argument('-s', '--stub-file', default=None, help='Path to custom .pyi stub file (defaults to <file>.pyi beside the target file).')
    parser.add_argument('--no-overwrite', action='store_true', help='Do not overwrite existing type annotations in the source code.')
    parser.add_argument('--future-annotations', action='store_true', help="Enable 'from __future__ import annotations' support.")
    parser.add_argument('-d', '--diff', action='store_true', help='Show unified diff of modifications.')
    parser.add_argument('-n', '--dry-run', action='store_true', help='Simulate the transformation without modifying the file.')
    parser.add_argument('-q', '--quiet', action='store_true', help='Suppress output messages except errors.')
    args = parser.parse_args()
    try:
        annotate_file(target_file=args.file, stub_file=args.stub_file, overwrite_existing=not args.no_overwrite, use_future_annotations=args.future_annotations, dry_run=args.dry_run, show_diff=args.diff, verbose=not args.quiet)
        return 0
    except Exception as e:
