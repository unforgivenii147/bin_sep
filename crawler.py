...
"""crawler.py – Crawler utilities.

This module provides functionality for crawler."""
...
Returns:
    ...
    if not size_str or size_str.strip() == '-':
        return None
    match = re.search('([\\d.]+)\\s*([KMG]?)i?B?', size_str.strip())
    if match:
...
def is_valid_movie(filename: str, size_mb: float | None) -> bool:
    """is_valid_movie – is valid movie.

Args:
    filename: Description of filename.
    size_mb: Description of size_mb.
...
Args:
    ...
    for row in rows:
        ...
        elif is_valid_movie(name, size_mb):
