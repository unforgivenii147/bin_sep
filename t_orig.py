...
"""t_orig.py – T Orig utilities.

This module provides functionality for t orig."""
...
def contains_cyrillic(text: str) -> bool:
    ...
    return bool(re.search('[\\u0400-\\u04FF\\u0500-\\u052F\\u2DE0-\\u2DFF\\uA640-\\uA69F\\u1C80-\\u1C8F]', text))
...
def translate_chunk_factory(source_lang: str, target_lang: str) -> Any:
    ...
    def translate_chunk(chunk: list[str]) -> tuple[list[str], str | None]:
        ...
        for attempt in range(1, RETRY_ATTEMPTS + 1):
            try:
                ...
                logger.warning("Translate attempt %d/%d failed for chunk starting '%s...': %s. Retrying in %.2fs", attempt, RETRY_ATTEMPTS, chunk[0][:60] + '...' if chunk else '', e, sleep_time)
...
def main() -> None:
    ...
    parser.add_argument('--max-workers', type=int, default=MAX_WORKERS, help=f'Max worker threads (default: {MAX_WORKERS})')
    ...
    if source_lang.lower() == 'ru' or source_lang.lower().startswith('ru'):
        ...
        skipped_lines = []
    print('Loaded %d lines: %d flagged for translation, %d skipped', len(all_lines), len(to_translate_raw), len(skipped_lines))
    ...
    print('Deduplicated: %d unique lines to translate (from %d total flagged)', len(to_translate_unique), len(to_translate_raw))
    ...
    if remaining_to_translate:
        ...
        print('Created %d chunk(s) from %d remaining lines (max %d chars per chunk), using %d worker(s)', len(chunks), len(remaining_to_translate), args.max_chunk_size, num_workers)
                ...
                try:
                    ...
                    if translated_text:
                        ...
                        if len(translated_lines) == len(original_lines):
                            ...
                            logger.warning('Line-count mismatch in chunk (%d original vs %d translated). Falling back to per-line translation for this chunk.', len(original_lines), len(translated_lines))
                            for line in original_lines:
                                try:
                                    ...
                                    results[line] = line
                                    to_cache[line] = line
                        print("Translated chunk %d/%d (sample: '%s' → '%s')", completed, total, original_lines[0][:40] + ('...' if len(original_lines[0]) > 40 else ''), results.get(original_lines[0], '')[:60])
                    else:
                        logger.error('Failed to translate chunk starting with: %s', chunk[0][:60] + '...' if chunk else '')
                        for line in chunk:
                            try:
                    ...
                    logger.error("Unexpected error processing chunk starting with '%s': %s", chunk[0][:60] + '...' if chunk else '', e)
            if to_cache:
        ...
        with output_path.open('w', encoding='utf-8') as f:
            ...
            for line in all_lines:
                if line in results:
                    ...
                    f.write(f'{line}\n')
        print('Wrote %s: translated %d lines, kept %d lines unchanged', output_path.name, translated_count, len(all_lines) - translated_count)
    except Exception as e:
