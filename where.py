...
"""where.py – Where utilities.

This module provides functionality for where."""
...
def parse_csv_exts(s: str | None) -> set[str] | None:
    """parse_csv_exts – parse csv exts.

Args:
    s: Description of s.

Returns:
    set[str] | None: Description of return value."""
    if not s:
        return None
...
def file_matches_extensions(path: Path, allowed_exts: set[str] | None) -> bool:
    """file_matches_extensions – file matches extensions.

Args:
    path: Description of path.
    allowed_exts: Description of allowed_exts.
...
def file_matches_exclude(path: Path, excluded_exts: set[str] | None) -> bool:
    """file_matches_exclude – file matches exclude.

Args:
    path: Description of path.
    excluded_exts: Description of excluded_exts.
...
def safe_copy_file(src: Path, dst_root: Path, rel_path: Path, errors: list[str]) -> None:
    """safe_copy_file – safe copy file.

Args:
    src: Description of src.
    dst_root: Description of dst_root.
    rel_path: Description of rel_path.
...
class ChangeHandler(FileSystemEventHandler):
    ...
    def __init__(self, cwd: Path, copy_enabled: bool, dest_dir: Path, allowed_exts: set[str] | None, excluded_exts: set[str] | None, interval_sec: float, print_lock: Any | None=None) -> None:
        """__init__ –   init  .

Args:
    cwd: Description of cwd.
    copy_enabled: Description of copy_enabled.
    dest_dir: Description of dest_dir.
    allowed_exts: Description of allowed_exts.
    excluded_exts: Description of excluded_exts.
    interval_sec: Description of interval_sec.
    print_lock: Description of print_lock."""
        super().__init__()
        self.cwd = cwd
...
Returns:
    ...
    def _should_process(self, src_path: Path) -> bool:
        """_should_process –  should process.
...
Returns:
    ...
    p = argparse.ArgumentParser(description='Watch a folder recursively, print changes, and optionally copy changed/created files.')
    p.add_argument('folder', nargs='?', default=str(Path.cwd()), help='Folder to watch (default: current working directory).')
    p.add_argument('-c', '--copy', action='store_true', help='Copy changed/created files to destination.')
    p.add_argument('-d', '--dest', default=str(Path.home() / 'tmp' / 'tgz'), help='Destination folder for copies (default: ~/tmp/tgz).')
    p.add_argument('-e', '--extensions', default=None, help="Comma-separated allowlist of file extensions to copy, e.g. 'svg,png,txt'. If omitted, copy all changed/created file types.")
    p.add_argument('-x', '--exclude', default=None, help="Comma-separated list of file extensions to exclude from copying, e.g. 'log,tmp'.")
    p.add_argument('-i', '--interval', type=float, default=1.0, help='Watch interval (seconds) for batching/printing and copying (default: 1.0).')
    p.add_argument('--no-recursive', action='store_true', help='Disable recursive watching (watch only top-level directory).')
    return p
...
def main() -> None:
    ...
    handler = ChangeHandler(cwd=cwd, copy_enabled=bool(args.copy), dest_dir=dest_dir, allowed_exts=allowed_exts, excluded_exts=excluded_exts, interval_sec=interval_sec)
    observer = Observer()
